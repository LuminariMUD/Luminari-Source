/* Production-linked tests for spells converted from Realms of Luminari. */
...
#include "../../src/magic/rol_spells.h"
#include "../../src/magic/spells.h"

#include <string.h>

static const int foundational_rol_spells[] = {
    SPELL_FARSEE,
...
void TestRolFoundationalSpellsAreRegisteredWithoutClassAssignments(CuTest *tc)
{
  size_t index;
  int class_num;
  int domain_num;
  ...
  for (index = 0; index < sizeof(foundational_rol_spells) / sizeof(foundational_rol_spells[0]);
       index++)
  {
    spellnum = foundational_rol_spells[index];
    CuAssertTrue(tc, is_rol_gap_spell(spellnum));
...
void TestRolComprehendLanguagesDoesNotGrantSpeech(CuTest *tc)
{
  struct char_data ch;
...
void TestRolMinorRejuvenationChangesDisplayedAgeTemporarily(CuTest *tc)
{
  struct char_data ch;
  struct player_special_data specials;
  int original_age;
...
void TestRolAreaShieldsUseTheStrongestQuarterReduction(CuTest *tc)
{
  ...
  CuAssertIntEquals(tc, 100, rol_spell_adjust_area_damage(&victim, 100));

  add_test_affect(&victim, SPELL_ANCESTRAL_SHIELD, APPLY_NONE, 0);
  CuAssertIntEquals(tc, 75, rol_spell_adjust_area_damage(&victim, 100));
...
void TestRolCreatureWardsReduceOnlyMatchingAttackers(CuTest *tc)
{
  ...
  CuAssertIntEquals(tc, 75, rol_spell_adjust_incoming_damage(&animal, &victim, 100));
  CuAssertIntEquals(tc, 100, rol_spell_adjust_incoming_damage(&undead, &victim, 100));

  add_test_affect(&victim, SPELL_PROT_FROM_UNDEAD, APPLY_AC_NEW, 2);
  CuAssertIntEquals(tc, 75, rol_spell_adjust_incoming_damage(&undead, &victim, 100));
  CuAssertIntEquals(tc, 100, rol_spell_adjust_incoming_damage(&humanoid, &victim, 100));
