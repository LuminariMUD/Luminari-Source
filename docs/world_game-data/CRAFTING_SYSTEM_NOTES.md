...
runtime `lib/etc/config` file or through `cedit`. The runtime file is ignored by
Git, so source control does not prove which value production uses.
...
| `restring`, `redesc` | Change item descriptions when enabled/registered. |
| `augment`, `disenchant` | Work with crystal enchantments. |
| `bonearmor`, `reforge` | Transform eligible equipment. |
| `autocraft` | Produce items for the older supply-order quest. |
...
| Subcommand | Purpose |
| --- | --- |
| `itemtype` / `type` | Select weapon, armor, miscellaneous, instrument, or golem type. Jewelry is under `misc`. |
...
| `bonuses` | Configure up to six object affects and mote costs. |
| `instrument` | Configure instrument quality, effectiveness, and breakability. |
| `keywords`, `shortdesc`, `roomdesc`, `extradesc` | Configure descriptions; the first three are required. |
...
| `reset [part]` | Refund/reset all or `motes`, `materials`, `enhancement`, `instrument`, `bonuses`, `descriptions`, `refine`, or `resize`. |
...
| `golem` | Enter golem construction; this subcommand requires mode `2`. |

A complete ordinary setup includes item type, specific type, variant, allocated
materials, `keywords`, `shortdesc`, and `roomdesc`. Enhancement, bonuses,
instrument options, and `extradesc` are conditional or optional. The compiled
no-argument editor banner still lists `jewelry` and omits `golem`; that banner
is stale relative to the parser and corrected help topic.
...
construction. Other variants require an `ITEM_CRAFTING_TOOL` object with the
appropriate wear slot. Tracked source defines and validates such objects but
does not establish that a deployment's world data grants any, so operators
must verify their world prototypes. Without those objects, ordinary equipment
projects cannot pass the tool check.
...
An ordinary item starts with 60 seconds before speed modifiers. At completion,
the base DC is recomputed as `10 + object level - level_adjust`, then the level
adjustment modifier is applied. The roll adds the proficient-talent bonus and,
where applicable, +5 from Craft Wondrous Item or Craft Magical Arms and Armor.
...
- potion creation uses `brew`, scroll creation uses `scribe`, and no registered
  command creates wands. `recharge` uses the Craft Wand feat but is not wand
  creation.

Player-reachable timed materials-and-motes work is ordinary creation, golem
work in mode `2`, and `supplyorder`. The dormant handlers still matter to save
compatibility and source maintenance but must not be advertised as commands.
...
craft duration: losing the activity owner preserves the remainder. However,
`resume_craft_activity()` has no production login, reconnect, copyover, or
player-load caller, so a loaded saved timer is not reconstructed automatically.
The persistence exceptions below impose additional method-specific limits.
...
`craftmaterials` (alias `craftmat`) lists these crafting balances, and
`craftmaterials store <item>` deposits a physical `ITEM_MATERIAL` object into
them. The meaning of the separate `materials` command is compile-time selected:
...
explicit older room node. The `WILDERNESS_HARVEST_CRAFTING` environment toggle
defaults true:
...
`gather` and `mine` are separate wilderness resource commands implemented in
`src/wilderness/resource_system.c`. Explicit older nodes retain their own
harvest behavior and may produce physical `ITEM_MATERIAL` objects.

`salvage <item>` can return materials and possible motes from eligible
equipment, but the character must have `FEAT_SALVAGE` or `TALENT_SCAVENGER`.
...
that inactive design and is historical rather than a valid command checklist.
...
Golem type, size, selected golem materials, and required golem motes are not
saved. A loaded character can retain a golem method and duration without the
selection needed to complete; resumed golem work therefore cannot complete
...
For other saved methods, the remaining duration is serialized and does not
count down offline, but current production routing does not automatically hand
it back to the activity manager after a reload.
...
Production-linked tests cover command abbreviations, activity ownership and
interruption, golem resource/lifecycle contracts, wilderness reward mapping,
tool quality, delayed harvest completion, harvested-resource consumption,
legacy crafting skills, supply-order skills, and crafted instruments. Tests do
not establish a deployment's selected mode or world-object availability.
