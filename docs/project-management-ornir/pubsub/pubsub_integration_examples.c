 ...
 */
void integrate_combat_events_example(struct char_data *ch, struct char_data *victim, int damage,
                                     int weapon_type)
{
  char event_data[MAX_STRING_LENGTH];

  /* Publish detailed combat event */
  snprintf(event_data, sizeof(event_data), "Combat: %s attacks %s with %s for %d damage",
           GET_NAME(ch), GET_NAME(victim),
           weapon_type >= 0 ? weapon_list[weapon_type].name : "bare hands", damage);

  pubsub_trigger_event("events.combat.action", ch, event_data, PUBSUB_PRIORITY_LOW);

  /* Critical hit events */
  if (damage > GET_MAX_HIT(victim) / 4)
  { /* More than 25% of max HP */
    snprintf(event_data, sizeof(event_data), "Critical hit: %d damage to %s", damage,
             GET_NAME(victim));
  ...
  if (GET_HIT(victim) <= 0)
  {
    snprintf(event_data, sizeof(event_data), "Player death: %s killed by %s", GET_NAME(victim),
             GET_NAME(ch));
                                    ...
                                    struct char_data *target)
{
  ...
  snprintf(event_data, sizeof(event_data), "Spell %d level %d", spellnum, level);
  ...
  if (spell_info[spellnum].targets & TAR_AREA_ATTACK)
  {
    ...
    snprintf(spatial_data, sizeof(spatial_data), "%d,%d,%d,%d", X_LOC(ch), Y_LOC(ch), 0,
             3); /* 3-room radius */

    struct pubsub_message *spatial_msg = pubsub_create_message(
        pubsub_find_topic_by_name("events.spell.area")->topic_id, GET_NAME(ch), event_data,
        PUBSUB_MESSAGE_TYPE_SPATIAL, PUBSUB_MESSAGE_CATEGORY_GAME_EVENT, PUBSUB_PRIORITY_NORMAL);
  ...
  {
    snprintf(event_data, sizeof(event_data), "Powerful magic energies ripple from %s's %s spell",
             GET_NAME(ch), spell_info[spellnum].name);

    pubsub_publish_wilderness_audio(X_LOC(ch), Y_LOC(ch), 0, "Magical Energies", event_data,
...
void integrate_levelup_events_example(struct char_data *ch, int old_level, int new_level)
{
  ...
  snprintf(event_data, sizeof(event_data), "Level %d to %d", old_level, new_level);
  ...
  {
    snprintf(event_data, sizeof(event_data), "Milestone level %d reached", new_level);
  ...
  case CLASS_WIZARD:
    if (new_level % 2 == 0)
    { /* Every even level */
      snprintf(event_data, sizeof(event_data), "Wizard spell learning opportunity: level %d",
               new_level);
  ...
  case CLASS_FIGHTER:
    if (new_level % 3 == 0)
    { /* Every 3rd level */
      snprintf(event_data, sizeof(event_data), "Fighter feat selection: level %d", new_level);
                                       ...
                                       int amount_harvested, int remaining)
{
  ...
  snprintf(event_data, sizeof(event_data), "Harvested %d units of resource type %d, %d remaining",
  ...
  {
    snprintf(event_data, sizeof(event_data), "Resource %d depleted to %d", resource_type,
             remaining);
    ...
    msg = pubsub_create_message(pubsub_find_topic_by_name("events.resource.depletion")->topic_id,
                                GET_NAME(ch), event_data, PUBSUB_MESSAGE_TYPE_ENVIRONMENTAL,
                                PUBSUB_MESSAGE_CATEGORY_ENVIRONMENTAL, PUBSUB_PRIORITY_NORMAL);
  ...
  if (remaining <= 10)
  {
    snprintf(event_data, sizeof(event_data), "CRITICAL: Resource %d depleted to %d at (%d,%d)",
 ...
 */
void integrate_guild_events_example(struct char_data *ch, const char *action, const char *details)
{
  char event_data[MAX_STRING_LENGTH];
  char topic_name[128];

  /* General guild event */
  snprintf(event_data, sizeof(event_data), "Guild action: %s - %s", action, details);
 ...
 */
void integrate_environmental_events_example(int zone_vnum, const char *effect, int severity)
{
  char event_data[MAX_STRING_LENGTH];
  char topic_name[128];
  int priority;
  ...
  {
  case 1:
    priority = PUBSUB_PRIORITY_LOW;
    break;
  case 2:
    priority = PUBSUB_PRIORITY_NORMAL;
    break;
  case 3:
    priority = PUBSUB_PRIORITY_HIGH;
    break;
  case 4:
    priority = PUBSUB_PRIORITY_URGENT;
    break;
  default:
    priority = PUBSUB_PRIORITY_NORMAL;
    break;
  ...
  snprintf(event_data, sizeof(event_data), "Environmental effect in zone %d: %s (severity %d)",
  ...
  {
    snprintf(event_data, sizeof(event_data), "Weather change: %s", effect);
  ...
  {
    snprintf(event_data, sizeof(event_data), "Fire event: %s", effect);
...
void setup_event_driven_topics(void)
{
  ...
  pubsub_create_topic("events.combat.action", "Combat action events", PUBSUB_CATEGORY_SYSTEM,
                      PUBSUB_ACCESS_PUBLIC, "System");
  pubsub_create_topic("events.combat.critical", "Critical hit events", PUBSUB_CATEGORY_SYSTEM,
                      PUBSUB_ACCESS_PUBLIC, "System");
  pubsub_create_topic("events.player.death", "Player death events", PUBSUB_CATEGORY_SYSTEM,
                      PUBSUB_ACCESS_PUBLIC, "System");
  pubsub_create_topic("events.player.levelup", "Player level advancement", PUBSUB_CATEGORY_SYSTEM,
                      PUBSUB_ACCESS_PUBLIC, "System");
  ...
  pubsub_create_topic("events.spell.cast", "Spell casting events", PUBSUB_CATEGORY_SYSTEM,
                      PUBSUB_ACCESS_PUBLIC, "System");
  pubsub_create_topic("events.spell.area", "Area effect spell events", PUBSUB_CATEGORY_SYSTEM,
                      PUBSUB_ACCESS_PUBLIC, "System");
  ...
  pubsub_create_topic("events.guild.activity", "General guild activities", PUBSUB_CATEGORY_GUILD,
                      PUBSUB_ACCESS_PUBLIC, "System");
  pubsub_create_topic("events.guild.promotion", "Guild promotions", PUBSUB_CATEGORY_GUILD,
                      PUBSUB_ACCESS_PUBLIC, "System");
  pubsub_create_topic("events.environmental.global", "Global environmental events",
                      PUBSUB_CATEGORY_SYSTEM, PUBSUB_ACCESS_PUBLIC, "System");
  pubsub_create_topic("events.weather.change", "Weather change events", PUBSUB_CATEGORY_SYSTEM,
                      PUBSUB_ACCESS_PUBLIC, "System");
