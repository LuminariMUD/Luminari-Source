...
| `craft/crafting_new.c:start_craft_activity` | Existing primary activity manager and native event_runtime; only active crafters have owned steps | No descriptor scan. Committed movement, combat, damage and target loss cancel work. Owner loss preserves the saved remainder; production login, reconnect and copyover paths invoke `resume_craft_activity()` to attempt reconstruction. The direct test seeds the unreachable survey adapter rather than a reachable craft. Materials stay with the project until existing completion/reset routines consume or refund them. |
| `magic/buff_sequence.c` | Native next-cast deadline and exact casting activity transition | No descriptor scan or duplicate cast countdown. Normal cast/manifest admission and costs remain authoritative. Movement, target loss, cast interruption, disconnect and shutdown cancel the sequence; saved lists remain. |
| `vessels/transport_jobs.c:transport.arrival` | Native character-owned arrival deadline; no descriptor scan | Admit before fare/departure; generation-checked destination; versioned vnum save and offline pause/reconstruction; committed relocation cancels. |
| `craft/crafting_new.c:generate_available_contracts` | Lazy wall-clock refresh on supply list/status access; offline time counts | No global scan or idle timer. Retains existing last/next refresh timestamps, active offers and slot cooldowns. The former online-only inventory description was incorrect. |
| `vessels/moving_room_events.c:moving-room.relocate` | Live-room-owned native deadline, resetZonePulse times ten seconds | Startup admission after world load; no global countdown scan. Existing spec gateway and door facts remain authoritative. OLC preserves live deadlines, editor clones have none, deletion cancels ownership, and stale/reindexed rooms resolve by identity. |
| `quest/staff_event_agenda.c` | Native expiry, delay-ended, jackalope-population and prisoner-presence agendas | Only active event/delay reasons are scheduled. Shared 75-second mud-hour phase; expiry precedes maintenance; admission before announcements/spawning; incarnation checks and shutdown cancellation. Status derives time from deadlines. |
