...
| `craft/crafting_new.c:start_craft_activity` | Existing primary activity manager and native event_runtime; only active crafters have owned steps | No descriptor scan. Committed movement, combat, damage and target loss cancel work. Offline time pauses the saved remainder; login/copyover reconstruct owned work. Materials stay with the project until existing completion/reset routines consume or refund them. |
