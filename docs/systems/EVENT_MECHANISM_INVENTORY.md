...
| `craft/crafting_new.c:generate_available_contracts` | Lazy wall-clock refresh when `supplyorder list`, `supplyorder select`, or `supplyorder cooldown` runs; offline time counts | No global scan or idle timer. Retains existing last/next refresh timestamps, active offers and slot cooldowns. The former online-only inventory description was incorrect. |
