...
| `craft/crafting_new.c:start_craft_activity` | Existing primary activity manager and native event_runtime; only active crafters have owned steps | No descriptor scan. Committed movement, combat, damage and target loss cancel work. Owner loss preserves the saved remainder, but no production login/copyover path invokes `resume_craft_activity()` to reconstruct it. Materials stay with the project until existing completion/reset routines consume or refund them. |
