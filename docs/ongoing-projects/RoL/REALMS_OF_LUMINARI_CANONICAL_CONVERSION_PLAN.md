...
- 1,614 resolved and 107 pending live static bindings, with 706 resolved and 89 pending
...
- 822 resolved and 26 pending records in the independent 848-record `ACT_SPEC`
  cross-check.
...
`rol-phase6-special-837b309c0b219cc5`. The older 1,112/1,147 binding, 538/562 handler,
...
remaining measured scope is 107 live static bindings across 89 direct handler names in
...
The thirty-first reconciled three Undermountain combat callbacks through the typed monster
runtime. Essra 2092043 preserves her successful-hit one-to-10 combat roll, five authored speech
branches, ordered double speech, targeted look, and laugh. Rust monster 2093303 preserves its
critical-only hammer-tail attempt, source 32-slot scan, independent one-in-33 roll for each
occupied slot, first-success ordering, container immunity, miss output, and permanent destruction
of the selected worn item. Gherias'Tuk 2093310 preserves activity-pulse combat gating, exact
2093325 hammer-head recognition, value-zero `say faith` priority, and the later one-in-ten mortal
vampire drain. The target safely handles an absent offhand where the source dereferenced it and
uses typed target validation before lethal behavior.
The thirty-second reconciled five Undermountain lifecycle callbacks through the typed monster
combat and utility-object runtimes. Flying dagger 2092047 suppresses its corpse, becomes golden
dagger 2092044 on death, and preserves its critical heart strike with target-safe invalidation and
a staff safety gate. Ochre jelly 2092058 consumes the first ordinary NPC corpse while spilling
its contents, skips player corpses without rolling, and independently gives each ordinary ground
object the source one-in-101 destruction chance. Vortex Guardian 2093002 preserves its seven
one-to-10 warnings and, on death, blocks the north exit while creating one-hour portal 2093005.
Shrieker 2093019 requires a mortal PC witness, always emits its shriek, and has a one-in-ten chance
to attract one uniformly selected mobile from 2093050 through 2093065. Black orchid 2093243 uses
the target's automatic object pulse to schedule one nonresetting 72-MUD-hour decay, equivalent to
the source three-day initialization event. Missing spawned prototypes and an absent north exit
now log explicit errors instead of dereferencing invalid state; Phase 7 must stage the referenced
objects and attracted mobiles before live testing.
...
sessions, or 20-80 focused engineering hours. Thirty-two corrected batches have closed 368
bindings across 168 handlers, leaving an arithmetic binding projection near 10 sessions and a
handler-diversity projection near 17. The published range allows shared runtime families to
outperform the handler projection while recognizing that 78 of the remaining 89 handlers are
