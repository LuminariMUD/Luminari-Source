...
The current artifact roster has three confirmed omissions of identity-defining
combat mechanics from its executable source material:

1. Fade has no life-draining combat strike.
...
Port the identity, not the legacy HP mutation. Use the normal damage pipeline,
cap healing at missing HP, state target exclusions explicitly, and test both a
hit and a rejection case.
...
| 169903 | Fade | Confirmed gap | Four called effects exist, but the executable 1-in-16 life drain does not (ART-AUD-002). Its source passives are also unresolved (ART-AUD-007). |
...
- Fade, Doombringer, and Avernus each produce their approved named behavior in a
