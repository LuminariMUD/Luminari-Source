...
| 5 s | Luminari pulse | Mixed character and affected-room work | **Correction in progress:** affected rooms retain explicit owners. Full-population character admission was rejected at production scale; regeneration, hazards, afflictions, weapon effects, mount/grapple cleanup, and other work require state-specific admission and retirement. |
| 6 s | NPC thinking | Previously every character | **Correction in progress:** the former event for every AI-enabled NPC was distributed polling. Wander, patrol, hunt, echo, special activity, resource recovery, and one-shot room reactions become explicit work reasons. Player absence never suspends configured off-screen behavior. |
...
| 6 s | damage/effects and player misc | Character/descriptor state | **Correction in progress:** connected-player work may remain connection-owned; NPC/character damage, effects, cooldowns, and recovery require concrete state or deadlines and retire when no work remains. |
