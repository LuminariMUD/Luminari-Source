...
This checkpoint restores the flat-file Luminari world only. The development database and
player/house stores still contain conversion-era persistent references, and the shared-runtime
mechanics isolation audit is still outstanding. The production world also contains legacy
validation findings; for example, the zone 1507 validator reports existing HLQ reference and
object-payload errors. Those production-source findings were recorded and were not changed during
this restoration.
