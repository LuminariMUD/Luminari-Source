...
Checkpoints 1-23 extracted the complete audited general object section to
...
Quicksand, Tower of Kenjin, Hive of Passion, Fey-Branche, Abyssal Vortex, House Agrach-Dyrr, and
House Shobalar packages now live in dedicated `src/spec/spec_zone_*` owners. The
...
`unequip_char()` in `src/handler.c`. `src/spec_procs.c` is 1,472 lines, down 10,740 lines from the
Phase 03 baseline.
...
| Root `make test` and `make install` pass with the server installed at `bin/circle`. | Standing gate; passed at Phase 03 Checkpoint 23 (574 tests). |
| Builder, help, system, and architecture documentation matches every implemented phase. | Standing gate; met through Phase 03 Checkpoint 23. |
...
crafting-mold purchase and construction. Checkpoints 1-23 have moved every item in that list, the
...
House Agrach-Dyrr, and House Shobalar packages have also moved intact from the legacy files. The
remaining callbacks in `src/spec_procs.c` are cohesive zone content; its former
...
Shipped content ownership (Phase 03 Checkpoints 1-23):
