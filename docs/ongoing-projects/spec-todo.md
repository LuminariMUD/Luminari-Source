...
Checkpoints 1-10 extracted the complete audited general object section to
...
Flame, Prisoner, Celestial Leviathan, and Fire Giant packages now live in dedicated
`src/spec/spec_zone_*` owners; `zone_procs.c` is 1,608 lines, down 2,594 lines from its Phase 03
...
| Root `make test` and `make install` pass with the server installed at `bin/circle`. | Standing gate; passed at Phase 03 Checkpoint 10 (574 tests). |
| Builder, help, system, and architecture documentation matches every implemented phase. | Standing gate; met through Phase 03 Checkpoint 10. |
...
crafting-mold purchase and construction. Checkpoints 1-10 have moved every item in that list, the
traced general mobile/room slice, reusable combat/companion archetypes, and clan services. The
King's Castle, Abyss, Crimson Flame, Prisoner, Celestial Leviathan, and Fire Giant packages have
also moved intact from the legacy files. The remaining legacy callbacks are cohesive zone content.
...
Shipped content ownership (Phase 03 Checkpoints 1-10):
...
src/spec/spec_zone_<package>.c     (Jot, Mad Drow, TTF)
```
