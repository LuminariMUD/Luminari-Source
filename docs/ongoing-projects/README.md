...
| [mudlet-msdp-json-decoder-investigation.md](mudlet-msdp-json-decoder-investigation.md) | Server remediation in progress | Server scope covers plain-text `ALIGNMENT`, `AREA_NAME`, and `ROOM_NAME` values, valid MSDP-over-GMCP JSON, regression tests, and protocol documentation. Mudlet and LuminariGUI changes are documented external findings and are out of scope. |
