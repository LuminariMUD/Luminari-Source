...
then split by fix ownership. This file contains 20 actionable code items drawn
from 22 production records. `LIB-BUGS.md` contains 14 production-owned items
...
Across both files, 37 unique records remain from the original 146-record
snapshot. The other 109 were removed because they are resolved, intentional,
...
- **Group Heal does not cure blindness** - Group Heal invokes Heal's point
  restoration for each target but omits Heal's unaffect path, which is what
  removes blindness. Reporter: Arithon (L30, R109560, 2023-02-10; #036).
...
- **`cexchange` unnecessarily removes Hide** - A hidden and sneaking
  ...
  R132262, 2023-02-19; #039).
- **Aasimar Healing Hands is not charged against its daily limit** - The
  racial-magic counter is initialized and checked, but successful Healing
  Hands uses never decrement it despite the documented three uses per day.
  Reporter: Lothelye (L10, R103009, 2023-02-19; #040).
