...
|----------|--------|--------------|
| [EVENT_DRIVEN_CORE_REFACTOR_SPEC.md](EVENT_DRIVEN_CORE_REFACTOR_SPEC.md) | Controlling specification v1.2; Phases 1 through 5 accepted, Phase 6a next | Introduce the typed synchronous domain-event core with bounded nesting, deterministic order, immutable borrowed payloads, typed handles, diagnostics, and independently reversible publishers. |
