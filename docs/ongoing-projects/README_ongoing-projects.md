...
|----------|--------|--------------|
| [CAMPAIGN_VARIANT_RETIREMENT_SCOPE.md](CAMPAIGN_VARIANT_RETIREMENT_SCOPE.md) | Scoped; implementation not started | Use a compiler-led remove/build/fix loop to retire the DragonLance and Forgotten Realms implementations while preserving current Luminari behavior and persisted identifiers. Campaign-framework preservation or redesign is explicitly out of scope. Supersedes `CAMPAIGN_VARIANT_REMOVAL_SCOPE.md`. |
