...
|----------|--------|--------------|
| [EVENT_DRIVEN_CORE_REFACTOR_SPEC.md](EVENT_DRIVEN_CORE_REFACTOR_SPEC.md) | Controlling specification v0.9; Phases 1, 2, and 2.5 accepted, Phase 3 next | Implement the `libevent` compatibility reactor and its monotonic pacing, fd/signal ownership, descriptor and copyover lifecycle, dependency contract, boot-time driver rollback, and 2x2 driver/backend test matrix without changing gameplay cadence. |
