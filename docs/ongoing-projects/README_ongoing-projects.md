...
|----------|--------|--------------|
| [EVENT_DRIVEN_CORE_REFACTOR_SPEC.md](EVENT_DRIVEN_CORE_REFACTOR_SPEC.md) | Draft architecture specification v0.4; inert Phase 1 scheduler core landed, Phase 1 gate open | Review and accept the scheduler invariants, timing-wheel geometry, lifecycle and ownership contract, migration gates, and encounter semantics; then complete the Phase 0 event inventory and performance baseline before writing runtime code. |
| [EVENT_DRIVEN_CORE_REFACTOR_SPEC_REVIEW.md](EVENT_DRIVEN_CORE_REFACTOR_SPEC_REVIEW.md) | Adversarial specification review complete 2026-08-30; findings open | Twenty-seven findings against specification v0.4 and the shipped Phase 1 core. Five are blocking for version 1.0: the published owner contract the core does not implement, a self-contradictory cascade rule, the unassigned monotonic-clock mandate, the unstated libevent/copyover `execl` hazard, and a Phase 6 rollback that cannot roll back. |
