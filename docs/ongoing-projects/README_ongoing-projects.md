...
| [BINARY_RENAME_CIRCLE_TO_LUMINARI.md](BINARY_RENAME_CIRCLE_TO_LUMINARI.md) | Audited plan; implementation not started | Rename the built server executable from `circle` to `luminari`. Phase A retains `bin/circle -> luminari`; Phase B waits for old-to-new and new-to-new copyovers plus a verified autorun supervisor restart. Includes the complete tracked active-file inventory, mixed-release rollback, historical/reference-tree exclusions, help SQL/file parity, and validation gates. |
