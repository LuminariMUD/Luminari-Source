...
|----------|--------|--------------|
| [CAMPAIGN_VARIANT_RETIREMENT_LIVE_TEST_REPORT.md](CAMPAIGN_VARIANT_RETIREMENT_LIVE_TEST_REPORT.md) | Live regression test passed | Gameplay evidence, test limits, state restoration, and six non-blocking follow-up findings for the retirement branch. |
| [BINARY_RENAME_CIRCLE_TO_LUMINARI.md](BINARY_RENAME_CIRCLE_TO_LUMINARI.md) | Phase A implemented; compatibility link still active | Rename the built server executable from `circle` to `luminari`. Phase A retains `bin/circle` as a symlink to `luminari`; Phase B waits for old-to-new and new-to-new copyovers plus a verified autorun supervisor restart. Includes the complete tracked active-file inventory, mixed-release rollback, historical/reference-tree exclusions, help SQL/file parity, and validation gates. |
