...
| [BINARY_RENAME_CIRCLE_TO_LUMINARI.md](BINARY_RENAME_CIRCLE_TO_LUMINARI.md) | Clean cutover in progress; compatibility implementation removed | Rename the server executable to `luminari` in one clean cutover. There is no compatibility symlink, mixed release layout, or old-to-new copyover. Covers the canonical release layout, the static regression, help SQL and file parity, the development rehearsal, and the production maintenance window. |
