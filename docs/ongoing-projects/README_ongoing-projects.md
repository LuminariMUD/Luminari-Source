...
| [BINARY_RENAME_CIRCLE_TO_LUMINARI.md](BINARY_RENAME_CIRCLE_TO_LUMINARI.md) | Plan written; implementation not started | Rename the built server executable from `circle` to `luminari`. Two-phase deploy gated on a successful copyover, because the running server's compiled-in `EXE_FILE` still points at `bin/circle` during the transition. Records the CircleMUD attribution and `circle_*` identifier exclusions, and two adjacent findings. |
