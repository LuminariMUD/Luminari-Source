...
| [SRC_LAYOUT_PHASE3.md](SRC_LAYOUT_PHASE3.md) | Complete | Added `src/obj/` (14 files: `act.item`, `item.h`, `objsave`, `treasure*`, `spec_artifacts`, `shop`, `trade`, `house`). Flat `src/` went from 134 to 120 across fifteen directories. Every predicted include-rewrite count held; the sweep missed `util/shopconv.c`, which broke the first build. One item remains: the in-game house load and `hsort` check from section 7. |
