...
ACMDU(do_devote)
{
      char arg1[MEDIUM_STRING] = {'\0'}, arg2[MEDIUM_STRING] = {'\0'}, dname[SMALL_STRING] = {'\0'};
      ...
      if (is_abbrev(arg1, "list"))
      {
            ...
            for (i = 0; i < NUM_DEITIES; i++)
            {
                  ...
                  if (listtype == DEITY_LIST_GOOD && deity_list[i].alignment != ALIGNMENT_GOOD)
                        continue;
                  else if (listtype == DEITY_LIST_NEUTRAL && deity_list[i].alignment != ALIGNMENT_GOOD && deity_list[i].ethos != ETHOS_NEUTRAL)
                        continue;
