...
int main(void)
{
  ...
  if (!mysql_query(
          conn,
          "SELECT keyword, help_tag FROM help_keywords WHERE LOWER(keyword) LIKE LOWER('score%')"))
  {
    res = mysql_store_result(conn);
    if (res)
    {
  ...
  if (!mysql_query(conn, "SELECT DISTINCT he.tag, LEFT(he.entry, 50), he.min_level "
