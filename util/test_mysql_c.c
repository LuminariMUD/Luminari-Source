...
void test_help_search(MYSQL *conn)
{
  ...
  const char *query = "SELECT he.tag, he.entry, he.min_level, he.last_updated, "
  ...
  while ((fetch_result = mysql_stmt_fetch(pstmt.stmt)) == 0 || fetch_result == MYSQL_DATA_TRUNCATED)
  {
    ...
    if (fetch_result == MYSQL_DATA_TRUNCATED)
    {
      ...
      if (tag_error)
        printf("  - tag truncated\n");
      if (entry_error)
        printf("  - entry truncated\n");
      if (min_level_error)
        printf("  - min_level truncated\n");
      if (last_updated_error)
        printf("  - last_updated truncated\n");
      if (keywords_error)
        printf("  - keywords truncated (length=%lu)\n", keywords_length);
    }
...
void test_soundex(MYSQL *conn)
{
  ...
  const char *query = "SELECT hk.help_tag, hk.keyword "
...
int main()
{
  ...
  /* Connect to database */
  if (!mysql_real_connect(conn, "localhost", "luminari_mud", "vZ$eO}fD-4%7", "luminari_mudprod", 0,
                          NULL, 0))
  {
