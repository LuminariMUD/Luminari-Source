...
int main()
{
  ...
  if (mysql_query(conn,
                  "SELECT DISTINCT keyword FROM help_keywords WHERE keyword LIKE '%score%'") == 0)
  {
  ...
  test_query(conn, "TEST 1: No LOWER(), exact match",
             "SELECT COUNT(*) FROM help_keywords WHERE keyword = ?", "Score");

  test_query(conn, "TEST 2: No LOWER(), LIKE with wildcard",
             "SELECT COUNT(*) FROM help_keywords WHERE keyword LIKE ?", "Score%");

  test_query(conn, "TEST 3: LOWER() on column only",
             "SELECT COUNT(*) FROM help_keywords WHERE LOWER(keyword) LIKE ?", "score%");

  test_query(conn, "TEST 4: LOWER() on both sides",
             "SELECT COUNT(*) FROM help_keywords WHERE LOWER(keyword) LIKE LOWER(?)", "score%");
  ...
  const char *full_query = "SELECT COUNT(*) "
  ...
  /* Test without LOWER() */
  const char *no_lower_query = "SELECT COUNT(*) "
                               "FROM help_entries he "
                               "LEFT JOIN help_keywords hk ON he.tag = hk.help_tag "
