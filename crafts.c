  ...
  if ((fp = fopen(CRAFT_FILE, "r")) == NULL) {
    ...
    while((line = fread_line(fp)) != NULL && line[0] != '\0' && !done) {
      if (!in_craft && !strcmp(line, "NEW")) {
        ...
        switch (*tag) {
           case 'I':
             if (!strcmp(tag, "Id  ")) craft->craft_id  = atoi(line);
           break;
           case 'E':
             if (!strcmp(tag, "End ")) {
               in_craft = FALSE;
           ...
           break;
           case 'M':
             if (!strcmp(tag, "Mroo")) craft->craft_msg_room = strdup(line);
             else if (!strcmp(tag, "Mslf")) craft->craft_msg_self = strdup(line);
           break;
           case 'N':
             if (!strcmp(tag, "Name"))  craft->craft_name  = strdup(line);
           break;
           case 'R':
             if (!strcmp(tag, "Req "))  {
           ...
           break;
           case 'T':
             if (!strcmp(tag, "Time"))  craft->craft_timer  = atoi(line);
           break;
           case 'V':
             if (!strcmp(tag, "Vnum"))  craft->craft_object_vnum  = atoi(line);
           break;
           default:
