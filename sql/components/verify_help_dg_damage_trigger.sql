...
FROM help_entries
WHERE
  tag = 'MOB-TRIGGERS' AND min_level = 31 AND auto_generated = FALSE
...
JOIN (
  ...
  UNION ALL
  SELECT 'No explicit return preserves'
  UNION ALL
  SELECT 'A wait cannot delay the current hit'
  UNION ALL
  SELECT '%attackid% / %attackname%'
  UNION ALL
  SELECT '%damagetype% / %damagetypename%'
  UNION ALL
  SELECT '%attackmodeid% / %attackmode%'
  UNION ALL
  SELECT 'direct DG damage commands'
  UNION ALL
  SELECT 'player victims'
  UNION ALL
  SELECT 'damage cap'
  UNION ALL
  SELECT 'training dummy'
) AS expected_content
WHERE
  h.tag = 'TRIGEDIT-MOB-DAMAGE'
