...
SELECT
  'campaign_merchant' AS check_name,
  COUNT(*) AS actual,
  1 AS expected
...
WHERE
  merchant.name = 'Vailand Ironwind Trader'
...
SELECT
  'campaign_market_gradient' AS check_name,
  COUNT(*) AS actual,
  2 AS expected
...
WHERE
  commodity.name = 'iron'
  AND (
    (market.port_vnum = 1000360 AND market.supply = 320)
