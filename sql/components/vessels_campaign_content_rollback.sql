...
WHERE
  port_vnum IN (1000360, 1000362)
  AND commodity_id = (
    SELECT commodity_id FROM trade_commodities
    WHERE name = 'iron'
  )
  AND (
    (port_vnum = 1000360 AND supply = 320)
...
WHERE (
  (vnum = 1000013 AND name = 'North Vailand Territorial Waters')
