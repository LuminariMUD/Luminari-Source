...
JOIN (
  SELECT
    'PERFORM' AS tag,
    '{Used As Instrument}' AS required_text
  UNION ALL
  SELECT
    'PERFORM',
    'slot shown by EQUIPMENT'
  UNION ALL
  SELECT
    'PERFORM',
  ...
  SELECT
    'PERFORM',
    'non-ideal instrument applies -2 effectiveness'
  UNION ALL
  SELECT
    'PERFORM',
    'chance in 11,111 per verse'
  UNION ALL
  SELECT
    'INSTRUMENT',
  ...
  SELECT
    'INSTRUMENT',
    'summoned instrument appears in your possession'
  UNION ALL
  SELECT
    'INSTRUMENT',
    'requires more than 20 hit points'
  UNION ALL
  SELECT
    'INSTRUMENT',
    'costs exactly 20 hit points'
  UNION ALL
  SELECT
    'INSTRUMENT',
    'cannot reduce you below 1'
