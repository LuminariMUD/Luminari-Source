...
SELECT
  'race_wemic_entry' AS check_name,
  COUNT(*) AS actual,
  1 AS expected,
  IF(COUNT(*) = 1, 'PASS', 'FAIL') AS result
FROM help_entries
WHERE
  tag = 'RACE-WEMIC' AND min_level = 0 AND auto_generated = FALSE
  AND CHAR_LENGTH(TRIM(entry)) > 0;

SELECT
  'race_wemic_keywords' AS check_name,
  COUNT(*) AS actual,
  4 AS expected,
...
SELECT
  'race_wemic_content' AS check_name,
  COUNT(*) AS actual,
  8 AS expected,
...
JOIN (
  ...
  UNION ALL
  SELECT 'ankle equipment'
) AS expected_content ON INSTR(h.entry, expected_content.required_text) > 0
WHERE h.tag = 'RACE-WEMIC';

SELECT
  'race_wemic_keyword_conflicts' AS check_name,
  COUNT(*) AS actual,
  0 AS expected,
  IF(COUNT(*) = 0, 'PASS', 'FAIL') AS result
FROM help_keywords
WHERE
  UPPER(keyword) IN ('WEMIC', 'RACE-WEMIC', 'BARBARIAN', 'RACE-BARBARIAN')
