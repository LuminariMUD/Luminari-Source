...
SELECT
  'vessel_narrative_hints' AS check_name,
  COUNT(*) AS actual,
  8 AS expected
FROM region_hints
WHERE
...
SELECT
  'vessel_narrative_regions' AS check_name,
  COUNT(DISTINCT region_vnum) AS actual,
  4 AS expected
FROM region_hints
WHERE
  region_vnum IN (1000013, 1000014, 1000015, 1000016)
...
SELECT
  'vessel_narrative_all_weather' AS check_name,
  COUNT(*) AS actual,
  4 AS expected
FROM region_hints
WHERE
...
SELECT
  'vessel_narrative_bad_rows' AS check_name,
  COUNT(*) AS actual,
  0 AS expected
FROM region_hints
WHERE
