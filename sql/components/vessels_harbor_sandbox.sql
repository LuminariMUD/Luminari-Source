...
SELECT
  ...
  0
WHERE NOT EXISTS (
  SELECT 1 FROM ship_waypoints
  WHERE name = 'harbor_west_dock'
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM ship_waypoints
  WHERE name = 'harbor_channel_turn'
);
...
SELECT
  ...
  1
WHERE NOT EXISTS (
  SELECT 1 FROM ship_routes
  WHERE name = 'harbor_ferry_loop'
);

SET @harbor_route_id = (
  SELECT MIN(route_id) FROM ship_routes
  WHERE name = 'harbor_ferry_loop'
);
SET @harbor_west_id = (
...
SELECT
  'Harbor Sandbox Merchant',
  1,
  @harbor_ferry_prototype_id,
  @harbor_route_id,
  70001,
  -66,
  92,
  ...
  1
WHERE
  @harbor_ferry_prototype_id IS NOT NULL
...
AND NOT EXISTS (
  SELECT 1 FROM region_data
  WHERE name = 'Harbor Sandbox Free Seas'
);
...
AND NOT EXISTS (
  SELECT 1 FROM region_data
  WHERE name = 'Harbor Sandbox Pirate Cove'
);
...
SELECT
  7000004,
  'Harbor Admiralty hunter patrol',
  0,
  0,
  0,
  0,
...
WHERE
  @harbor_hunter_prototype_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
...
SELECT
  @harbor_hunter_encounter_id,
  @harbor_hunter_prototype_id,
  70002,
  2000,
  5,
  300,
  ...
  1
WHERE
  @harbor_hunter_encounter_id IS NOT NULL
