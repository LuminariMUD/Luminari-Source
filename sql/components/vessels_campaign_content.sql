...
SELECT
  @vailand_north_region_vnum,
  10000,
  'North Vailand Territorial Waters',
  1,
  ST_GeomFromText(
...
WHERE NOT EXISTS (
  ...
  WHERE
    vnum = @vailand_north_region_vnum
...
SELECT
  @vailand_central_region_vnum,
  10000,
  'Central Vailand Territorial Waters',
  1,
  ST_GeomFromText(
...
WHERE NOT EXISTS (
  SELECT 1 FROM region_data
  WHERE
    vnum = @vailand_central_region_vnum
...
WHERE NOT EXISTS (
  SELECT 1 FROM region_data
  WHERE
    vnum = @vailand_passage_region_vnum
...
SET
  x = -599, y = 455, z = 0, tolerance = 0.5,
  wait_time = 30, flags = 0
WHERE name = 'vailand_north_port';
UPDATE ship_waypoints
SET
  x = -573, y = 405, z = 0, tolerance = 0.5,
  wait_time = 0, flags = 0
WHERE name = 'vailand_northing';
UPDATE ship_waypoints
SET
  x = -533, y = 330, z = 0, tolerance = 0.5,
  wait_time = 0, flags = 0
WHERE name = 'blackwake_anchorage';
UPDATE ship_waypoints
SET
  x = -480, y = 229, z = 0, tolerance = 0.5,
  wait_time = 0, flags = 0
WHERE name = 'vailand_central_approach';
UPDATE ship_waypoints
SET
  x = -495, y = 229, z = 0, tolerance = 0.5,
  wait_time = 0, flags = 0
WHERE name = 'vailand_coast_turn';
UPDATE ship_waypoints
SET
...
WHERE
  name IN ('vailand_outer_passage', 'vailand_southing')
  AND NOT EXISTS (
...
SELECT
  @vailand_merchant_name,
  1,
  @vailand_prototype_id,
  @vailand_route_id,
  31810,
  -599,
  455,
  ...
  1
WHERE
  @vailand_prototype_id IS NOT NULL
