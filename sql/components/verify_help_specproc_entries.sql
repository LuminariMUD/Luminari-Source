...
WHERE BINARY tag = 'spec-proc'
  ...
  AND INSTR(entry, 'Forty-five converted mobiles use converter-owned death profiles') > 0
  ...
  AND INSTR(entry, 'RoL Toll Keeper is mobile-owned') > 0
  AND INSTR(entry, 'ticketed ship entries') > 0
  ...
  AND INSTR(entry, 'Forty-five converted') > 0
  AND INSTR(entry, 'four Elemental Tower bosses compose') > 0
  AND INSTR(entry, 'pit-fiend tails compose') > 0
  ...
  AND INSTR(entry, 'RoL Waterdeep Peacekeeper is mobile-owned') > 0
  ...
  AND INSTR(entry, 'Fifty converted weapon') > 0
  ...
  AND INSTR(entry, '266 source') > 0
  AND INSTR(entry, 'Casino owner 2003206 independently rolls both') > 0
