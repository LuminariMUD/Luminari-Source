...
class SemanticTests(unittest.TestCase):
    ...
    def test_color_stripping_placeholder_and_runtime_flags(self) -> None:
        ...
        findings = self.validate([zone(1, 100, 199)], [test_room], [mobile], [test_object])
    ...
    def test_exit_consistency_no_exit_fallback_and_unreachable_codes(self) -> None:
        ...
        self.assertTrue({"SEM005", "SEM007", "SEM008", "SEM009", "SEM010", "SEM011"} <= codes)
    ...
    def test_rol_object_trap_value_contract(self) -> None:
        ...
        findings = self.validate([zone(1, 100, 199)], [room(100, 1)], objects=[valid, invalid])
        ...
        for field_name, minimum, maximum in scalar_cases:
            ...
            for value in (minimum - 1, maximum + 1):
                with self.subTest(field=field_name, invalid=value):
                    candidate = replace(base, **{field_name: value})
                    self.assertIn("SEM025", {item.code for item in self.quest_findings(candidate)})
    ...
    def test_qst_type_flags_and_type_specific_domains(self) -> None:
        ...
        unavailable = quest_record(98, -1, questmaster_vnum=None, previous_quest_vnum=98)
        self.assertFalse(
        ...
        for value in (1, ability_limit):
            skill = quest_record(106, self.quest_types["AQ_SKILL_SUCCESS"], target=value)
        ...
        phenomenon_limit = self.manifest["limits"]["NUM_DOMAIN_PHENOMENON_KINDS"]["value"] - 1
        for value in (1, phenomenon_limit):
            witness = quest_record(107, self.quest_types["AQ_WITNESS_PHENOMENON"], target=value)
            self.assertNotIn("SEM026", {item.code for item in self.quest_findings(witness)})
    ...
    def test_qst_dialogue_domains_and_alternative_topology(self) -> None:
        ...
        self.assertTrue(any("unjoinable" in item.message for item in self.quest_findings(poisoned)))
    ...
    def test_qst_chain_self_links_reciprocity_and_cycles(self) -> None:
        ...
        finding = next(item for item in self.quest_findings(previous_self) if item.code == "SEM028")
        ...
        cycle = next(
            item for item in self.validate([], [], quests=previous_cycle) if item.code == "SEM029"
        )
        ...
        cycle = next(
            item for item in self.validate([], [], quests=next_cycle) if item.code == "SEM029"
        )
    ...
    def test_hlq_entry_shape_and_command_legality(self) -> None:
        ...
        self.assertFalse({item.code for item in self.hlquest_findings(ask)} & {"SEM030", "SEM031"})
        ...
        item_input = replace(give, commands=[hlq_command(item_type, 300, 0, "input")])
        self.assertNotIn("SEM031", {item.code for item in self.hlquest_findings(item_input)})
        ignored_input = replace(give, commands=[hlq_command(load_type, 300, 100, "input")])
    ...
    def test_hlq_runtime_safe_command_boundaries_and_parameters(self) -> None:
        ...
        for value in (0, 100000, 2140000000):
            self.assertNotIn(
                "SEM032", {item.code for item in findings(hlq_command(coin, value, 0))}
            )
        ...
        for value in (-1, 13):
            self.assertIn("SEM032", {item.code for item in findings(hlq_command(church, value, 0))})
