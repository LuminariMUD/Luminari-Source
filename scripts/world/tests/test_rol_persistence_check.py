...
class _World:
    ...
    def records(self, record_type: str) -> list[SimpleNamespace]:
        return [SimpleNamespace(vnum=vnum) for vnum in self.definitions.get(record_type, [])]
...
class RolPersistenceCheckTests(unittest.TestCase):
    ...
    def test_audit_requires_each_persisted_vnum_to_resolve_once(self) -> None:
        ...
        with (
            patch("wtool_lib.rol_persistence_check.PERSISTENT_BINDINGS", bindings),
            patch(
    ...
    def test_audit_redacts_explicit_development_lib_root(self) -> None:
        ...
        self.assertEqual("explicit-development-lib-root", result["configuration_scope"])
        self.assertEqual("external-development-lib/mysql_config", result["configuration"])
