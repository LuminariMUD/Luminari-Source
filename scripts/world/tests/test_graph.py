...
class FullGraphTests(unittest.TestCase):
    ...
    def test_tracked_complete_fixture_covers_normal_mini_and_legacy_read_only(self) -> None:
        ...
        mini = validate_indexed_world(root, self.repo_root, self.manifest, self.config, mini=True)
    ...
    def test_tracked_real_bundles_have_stable_phase2_results_and_are_read_only(self) -> None:
        ...
        for name, expected in expected_errors.items():
            with self.subTest(bundle=name):
                ...
                self.assertEqual(
                    expected, {item.code for item in result.findings if item.severity == "error"}
                )
                self.assertEqual(before, tree_hash(root))
    ...
    def test_trigger_attach_type_and_key_item_type_are_checked(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            obj = root / "obj/1.obj"
            obj.write_text(
                obj.read_text(encoding="ascii").replace("18 0 0 0 0", "5 0 0 0 0"), encoding="ascii"
            )
    ...
    def test_p_reset_requires_a_container_type(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            zone = root / "zon/1.zon"
            zone.write_text(
                zone.read_text(encoding="ascii").replace("S\n$\n", "P 0 100 1 100 100\nS\n$\n"),
                encoding="ascii",
            )
    ...
    def test_reference_roles_report_wrong_type_targets(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            messages = [
                item.message for item in self.validate(root).findings if item.code == "REF023"
            ]
            for role in (
                "M reset prototype",
                "O reset prototype",
    ...
    def test_qst_reference_model_covers_every_edge_role_and_target_type(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            world = load_indexed_world_data(root, self.repo_root, self.manifest, self.config)
    ...
    def test_hlq_reference_model_and_missing_targets_cover_nested_context(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(9, len(missing))
            self.assertTrue(
                all("entry 1" in item.message or "host" in item.message for item in missing)
            )
