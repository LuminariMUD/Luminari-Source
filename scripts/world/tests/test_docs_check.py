...
class DocumentationCheckTests(unittest.TestCase):
    ...
    def test_unregistered_command_in_reference_is_reported(self) -> None:
        ...
        texts = {
            item.path: (self.repo_root / item.path).read_text(encoding="ascii")
            for item in COMMAND_SECTIONS
        }
    ...
    def test_rol_implemented_gap_spells_have_flat_help_coverage(self) -> None:
        spell_names = [
            _SOURCE_SPELL_MAP[source_id][0] for source_id in ROL_IMPLEMENTED_GAP_SOURCE_IDS
        ]

        help_lines = (
            (self.repo_root / "lib/text/help/help.hlp").read_text(encoding="utf-8").splitlines()
        )
        ...
        expected_keywords = {
            re.sub(r"[^A-Z0-9]+", "-", name.upper()).strip("-") for name in spell_names
        }
