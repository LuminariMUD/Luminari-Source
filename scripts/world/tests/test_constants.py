...
class ConstantsTests(unittest.TestCase):
    ...
    def test_room_table_matches_source_contract(self) -> None:
        ...
        self.assertEqual("ROOM_ROL_ASTRAL", entries[47]["macro"])
        self.assertEqual("RoL-Astral", entries[47]["name"])
    ...
    def test_bounded_blocks_keep_aliases_without_prefix_collisions(self) -> None:
        ...
        mob_macros = {entry["macro"] for entry in self.manifest["tables"]["mob"]["entries"]}
    ...
    def test_reverse_directions_match_runtime_table(self) -> None:
        ...
        self.assertEqual(
            [2, 3, 0, 1, 5, 4, 8, 9, 6, 7], [entry["reverse_index"] for entry in directions]
        )
...
class SpecRegistryTests(unittest.TestCase):
    def test_current_registry_exposes_canonical_and_alias_names(self) -> None:
        ...
        self.assertIn("rol totem restorer", names)
        self.assertIn("rol mage guild room", names)
        self.assertIn("rol weapon proc", names)
        self.assertIn("rol avernus object", names)
        self.assertIn("rol avernus garden", names)
        ...
        self.assertIn("rol lavatubes mobile", names)
        self.assertIn("rol lavatubes object", names)
        self.assertIn("rol lavatubes room", names)
        self.assertIn("rol bard guild room", names)
        self.assertIn("rol waterdeep guild room", names)
        self.assertIn("rol waterdeep peacekeeper", names)
        self.assertIn("rol bloodstone critter", names)
        self.assertIn("rol source periodic", names)
        self.assertIn("rol stateful periodic", names)
        self.assertIn("rol designated follower", names)
        self.assertIn("rol fixed bodyguard", names)
        self.assertIn("rol portal door", names)
        self.assertIn("rol item blocker", names)
        ...
        self.assertIn("rol sister knight", names)
