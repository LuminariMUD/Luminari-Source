...
class RolSpecialReconciliationTests(unittest.TestCase):
  ...
  def test_production_inputs_generate_complete_progress_ledgers(self) -> None:
    with tempfile.TemporaryDirectory() as temporary:
      ...
      self.assertEqual(552, summary["direct_bindings_by_status"]["resolved"])
      self.assertEqual(595, summary["direct_bindings_by_status"]["pending"])
      self.assertEqual(88, summary["source_handlers_by_status"]["resolved"])
      self.assertEqual(474, summary["source_handlers_by_status"]["pending"])
      self.assertEqual(181, summary["direct_bindings_by_strategy"]["NATIVE_ADAPTED"])
      self.assertEqual(848, summary["act_spec_records"])
      self.assertEqual(554, summary["act_spec_by_status"]["resolved"])
      self.assertEqual(294, summary["act_spec_by_status"]["pending"])
      self.assertEqual(
