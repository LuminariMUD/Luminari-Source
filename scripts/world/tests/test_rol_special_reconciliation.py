...
class RolSpecialReconciliationTests(unittest.TestCase):
  ...
  def test_production_inputs_generate_complete_progress_ledgers(self) -> None:
    with tempfile.TemporaryDirectory() as temporary:
      ...
      self.assertEqual(567, summary["direct_bindings_by_status"]["resolved"])
      self.assertEqual(580, summary["direct_bindings_by_status"]["pending"])
      self.assertEqual(93, summary["source_handlers_by_status"]["resolved"])
      self.assertEqual(469, summary["source_handlers_by_status"]["pending"])
