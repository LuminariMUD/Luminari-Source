...
class RolSpecialReconciliationTests(unittest.TestCase):
    ...
    def test_reviewed_handler_dispositions_do_not_rely_on_matching_names(self) -> None:
        ...
        bard_guilds = [handler_disposition(name) for name in ("guild_bard", "guild_battlechanter")]
        ...
        composed_alerts = [handler_disposition(name) for name in ("imix_shout", "yancbin_shout")]
        death_profiles = [
            handler_disposition(name)
            for name in (
                "tentacle_die",
        ...
        source_periodic = [
            ...
            if name not in COMPOSED_PROFILE_TARGETS
        ]
        composed_source_periodic = [handler_disposition(name) for name in COMPOSED_PROFILE_TARGETS]
        ...
        self.assertTrue(
            all(
                row["target"] == "RoL Bard Guild Room" and row["strategy"] == "NATIVE_ADAPTED"
                for row in bard_guilds
            )
        )
        self.assertTrue(
            all(
                row["target"] == "RoL Guild Guard" and row["strategy"] == "NATIVE_ADAPTED"
        ...
        self.assertTrue(
            all(
                row["target"] == "RoL Alert Caller" and row["strategy"] == "NATIVE_ADAPTED"
        ...
        self.assertTrue(
            all(
                row["target"] == "RoL Waterdeep Ambient" and row["strategy"] == "NATIVE_ADAPTED"
                for row in waterdeep_ambient
            )
        )
        self.assertTrue(
            all(
                row["target"] == "RoL Source Periodic" and row["strategy"] == "NATIVE_ADAPTED"
        ...
        self.assertTrue(
            all(
                row["target"] == "RoL Stateful Periodic" and row["strategy"] == "NATIVE_ADAPTED"
        ...
        self.assertTrue(
            all(
                row["target"] == "RoL Waterdeep Guild Room" and row["strategy"] == "NATIVE_ADAPTED"
        ...
        self.assertTrue(
            all(
                row["target"] == "RoL Monster Combat" and row["strategy"] == "NATIVE_ADAPTED"
        ...
        self.assertTrue(
            all(
                row["target"] == "RoL Undead Drain" and row["strategy"] == "NATIVE_ADAPTED"
        ...
        self.assertTrue(all(row["strategy"] == "NATIVE_RECONCILED" for row in artifact_handlers))
        self.assertTrue(
    ...
    def test_planar_base_handlers_use_independent_composable_hooks(self) -> None:
        ...
        self.assertEqual("MOB_ROL_DEMON composition-safe runtime hook", standard_demon["target"])

    def test_undermountain_social_and_disabled_stench_handlers_are_resolved(self) -> None:
        for handler in (
            "um2_deriahSocials",
            "um2_devilSocials",
            "um2_impSocials",
        ...
        stench = handler_disposition("um2_troglodyteStench")
        self.assertEqual("resolved", stench["status"])
    ...
    def test_planar_static_initializers_have_exact_prototype_or_inert_dispositions(self) -> None:
        ...
        self.assertEqual("mobile action flag 112 plus mobile affect flag 20", bar_lgura["target"])
        self.assertEqual("NATIVE_ADAPTED_COMPOSABLE", cambion["strategy"])
        self.assertEqual("mobile action flag 112 plus mobile affect flag 19", cambion["target"])
    ...
    def test_source_death_effect_batch_shares_composable_profile_runtime(self) -> None:
        handlers = (
            "dk_aleanrahel",
            "um_helmedHorror",
            "um2_butcherKnife",
    ...
    def test_deaths_head_lifecycle_has_one_owner_aware_disposition(self) -> None:
        for handler in ("um2_deathsHead", "um2_deathsHeadSeed", "um2_deathsHeadTree"):
            disposition = handler_disposition(handler)
            self.assertEqual("resolved", disposition["status"])
            self.assertEqual("NATIVE_ADAPTED", disposition["strategy"])
    ...
    def test_undermountain_lifecycle_handlers_use_typed_runtimes(self) -> None:
        for handler in (
            "um_flyingDagger",
            "um_ochreJelly",
            "um2_vortexGuardian",
        ...
        disposition = handler_disposition("um2_orchidDecay")
        self.assertEqual("resolved", disposition["status"])
        self.assertEqual("NATIVE_ADAPTED", disposition["strategy"])
    ...
    def test_production_inputs_generate_complete_progress_ledgers(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(42, summary["direct_bindings_by_strategy"]["SOURCE_INERT_EXCLUDED"])
            ...
            self.assertEqual(18, summary["direct_bindings_by_strategy"]["SOURCE_UNSAFE_EXCLUDED"])
            ...
            binding_rows = [
                json.loads(line)
                for line in (output_dir / "binding-ledger.jsonl")
                .read_text(encoding="ascii")
                .splitlines()
            ]
            seelie_vnums = {
                handler: {
                    row["source_vnum"] for row in binding_rows if row["source_handler"] == handler
                }
                for handler in ("standard_faerie_ff", "standard_faerie_prism", "faerie_search")
            }
            self.assertEqual(
            ...
            self.assertEqual({"resolved": 247}, summary["implicit_race_bindings_by_status"])
