...
class RolWeaponMappingTests(unittest.TestCase):
    ...
    def test_dart_and_blowgun_delivery_types_stay_distinct(self) -> None:
        ...
        self.assertEqual("WEAPON_TYPE_DART", mapping.infer_ranged_weapon_type(thrown_dart).name)
        self.assertEqual("WEAPON_TYPE_BLOWGUN", mapping.infer_ranged_weapon_type(blowgun).name)
    ...
    def test_native_javelin_profile_is_thrown_but_not_an_ammo_launcher(self) -> None:
        ...
        self.assertEqual(
            (1, 6, 2, 30), (entry.num_dice, entry.dice_size, entry.weight, entry.range)
        )

    def test_weak_modifiers_never_outrank_a_named_weapon(self) -> None:
        decorated = self._weapon(
            "spear hunting razor", "a razor-sharp hunting spear", 0, "0 1 6 11"
        )
    ...
    def test_emitted_object_carries_the_inferred_weapon_type(self) -> None:
        ...
        self.assertEqual(str(mapping.WEAPON_TYPE_IDS["WEAPON_TYPE_LONG_SWORD"]), values[0])
    ...
    def test_ammo_type_table_matches_the_target_header(self) -> None:
        header = (self.root / "src/core/structs.h").read_text(encoding="utf-8", errors="ignore")
        declared = {
            int(value): name for name, value in re.findall(r"#define (AMMO_TYPE_\w+) (\d+)", header)
        }
    ...
    def test_final_review_categories_have_no_unreviewed_members(self) -> None:
        self._require_reference_corpus()
        report = mapping.audit(self.corpus.records)
        fallbacks = {row["vnum"] for row in report["records"] if row["tier"] == "fallback"}
    ...
    def test_ranged_override_catalog_accepts_native_thrown_weapon_types(self) -> None:
        ...
        self.assertEqual("WEAPON_TYPE_JAVELIN", catalog["ranged_overrides"][1]["name"])
    ...
    def test_emitted_ammunition_zeroes_the_imbued_spell_and_inverts_durability(self) -> None:
        ...
        for record in self.ammunition:
            with self.subTest(vnum=record.vnum):
                ...
                self.assertEqual(mapping.missile_break_probability(source_values[2]), values[2])
    ...
    def test_source_quivers_share_the_ammo_pouch_contract(self) -> None:
        ...
        self.assertEqual({mapping.TARGET_ITEM_AMMO_POUCH: 44}, kinds)
    ...
    def test_hitroll_and_damroll_become_the_enhancement_bonus(self) -> None:
        ...
        emitted, item_type, values, _economy, _blocks = self._emit(record)
        self.assertEqual(mapping.TARGET_ITEM_WEAPON, item_type)
        self.assertEqual(mapping.WEAPON_TYPE_IDS["WEAPON_TYPE_LONG_BOW"], values[0])
        ...
        for record in self.corpus.records:
            ...
            for directive in record.directives:
                ...
                if token == "M":
                    ...
                    if (
                        item_type == mapping.TARGET_ITEM_WEAPON
                        and table[values[0]].weapon_flags & _WEAPON_FLAG_RANGED
                    ):
                        launcher = values[0]
                    if (
    ...
    def test_rol_archer_loadouts_cover_launcher_and_throwable_modes(self) -> None:
        self._require_reference_corpus()
        objects = {record.vnum: record for record in self.corpus.records if record.kind == "obj"}
        table = weapon_table()
