...
class LookupTests(unittest.TestCase):
    def test_quest_show_supports_json_human_and_qst_alias(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            status, stdout, stderr = run_cli(["--world-root", str(root), "show", "quest", "100"])
            self.assertEqual(0, status)
            self.assertEqual("", stderr)
    ...
    def test_hlquest_show_exposes_physical_and_runtime_order(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            status, stdout, stderr = run_cli(["--world-root", str(root), "show", "hlquest", "100"])
            self.assertEqual(0, status)
            self.assertEqual("", stderr)
    ...
    def test_show_type_disambiguates_overlapping_vnums(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "world"
            make_world(root)
            status, stdout, _ = run_cli(["--world-root", str(root), "--json", "show", "obj", "100"])
            payload = json.loads(stdout)
            self.assertEqual(0, status)
    ...
    def test_refs_reports_typed_incoming_edges_and_is_read_only(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(
                any(
                    edge["source_type"] == "shop" and edge["role"] == "shop product"
    ...
    def test_quest_system_refs_are_bidirectional_in_json_and_human_output(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(any(edge["target_type"] == "mobile" for edge in outgoing))
            self.assertTrue(any(edge["target_type"] == "object" for edge in outgoing))
            ...
            self.assertTrue(
                any(
                    edge["target_type"] == "mobile" and edge["role"] == "attached host mobile"
            ...
            status, stdout, stderr = run_cli(["--world-root", str(root), "refs", "mobile", "100"])
            self.assertEqual(0, status)
            self.assertEqual("", stderr)
    ...
    def test_existing_six_record_payloads_match_compatibility_golden(self) -> None:
        golden_path = (
            Path(__file__).resolve().parent / "fixtures/phase2/contracts/lookup/existing-six.json"
        )
    ...
    def test_refs_reports_room_exit_outgoing_edge(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(
                any(
                    edge["target_type"] == "room" and edge["target_vnum"] == 100
    ...
    def test_missing_lookup_is_stable_status_one(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "world"
            make_world(root)
            first = run_cli(["--world-root", str(root), "--json", "show", "trigger", "999"])
            second = run_cli(["--world-root", str(root), "--json", "show", "trigger", "999"])
