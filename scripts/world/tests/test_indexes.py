...
class IndexTests(unittest.TestCase):
    ...
    def test_qst_indexes_are_required_but_may_describe_an_empty_dataset(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            repo = Path(directory)
            root = repo / "world"
            make_world(root)
            (root / "qst/index").unlink()
            missing = validate_indexes(root, repo)
            self.assertIn("IDX009", {finding.code for finding in missing.findings})
            self.assertFalse(missing.complete)

            (root / "qst/index").write_text("$\n", encoding="ascii")
            (root / "qst/1.qst").unlink()
            empty = validate_indexes(root, repo)
            qst_findings = [
                finding for finding in empty.findings if finding.span.path.startswith("qst/")
            ]
            self.assertEqual([], qst_findings)
            self.assertTrue(empty.complete)

    def test_missing_qst_mini_index_uses_the_standard_error(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            repo = Path(directory)
            root = repo / "world"
            make_world(root, mini=True)
            (root / "qst/index.mini").unlink()
            result = validate_indexes(root, repo, mini=True)
            findings = [
                finding for finding in result.findings if finding.span.path == "qst/index.mini"
            ]
    ...
    def test_missing_hlq_mini_index_uses_the_standard_error(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = validate_indexes(root, repo, mini=True)
            findings = [
                finding for finding in result.findings if finding.span.path == "hlq/index.mini"
            ]
            self.assertEqual(["IDX009"], [finding.code for finding in findings])
            self.assertFalse(result.complete)
