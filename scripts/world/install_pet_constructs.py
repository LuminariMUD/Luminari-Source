...
def main():
    ...
    changes = {}
    for kind, number, collection in (
        (kind, number, collection)
        for number, collection in (
            (195, "pet-lycanthropes"),
            (196, "pet-constructs"),
        ...
        for existing in directory.glob(f"*.{kind}"):
            ...
            if any(
                int(n) == number if kind == "zon" else number * 100 <= int(n) <= number * 100 + 99
                for n in numbers
            ):
        ...
        if target.name not in lines:
            position = next(
                (
                    ...
                    if line.split(".")[0].isdigit() and int(line.split(".")[0]) > number
                ),
                len(lines) - 1,
            )
    ...
    keepers = [
        i for i, line in enumerate(lines) if line.split()[:5] == ["M", "0", "14116", "1", "14125"]
    ]
    if len(keepers) != 1:
    ...
    start = keepers[0] + 1
        ...
        (
            ...
            if lines[i].split() and lines[i].split()[0] not in ("G", "E", "*")
        ),
        len(lines),
    )
    ...
    print(
        "Installed lycanthrope, construct, planar ally, undead, and illusion records; existing area records preserved."
    )
