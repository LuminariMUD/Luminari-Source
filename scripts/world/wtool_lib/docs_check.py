...
def _table_findings(
    ...
    heading_line = next(
        index for index, line in enumerate(text.splitlines(), start=1) if line == spec.start_heading
    )
...
def _documented_commands(
    text: str, spec: CommandSectionSpec
) -> tuple[list[tuple[str, int]], list[Finding]]:
    section, findings = _section(text, spec.path, spec.start_heading, spec.end_heading)
