...
def _development_database_config(
    ...
    if missing:
        raise RolPersistenceCheckError(f"lib/mysql_config is missing {', '.join(missing)}")
...
def _query_expression(binding: dict[str, Any]) -> tuple[str, str] | None:
    ...
    if encoding == "object_header_blob":
        expression = (
            f"CAST(SUBSTRING(SUBSTRING_INDEX(CAST(`{column}` AS CHAR),CHAR(10),1),2) AS UNSIGNED)"
        )
        header_scope = (
            f"({scope}) AND LEFT(CAST(`{column}` AS CHAR),1)='#' AND LOCATE(CHAR(10),`{column}`)>0"
        )
        return expression, header_scope
    return None


def _binding_values(config: dict[str, str], binding: dict[str, Any]) -> Counter[int]:
    ...
    if str(binding["encoding"]) not in {"csv_integer", "vessel_connections"}:
        raise RolPersistenceCheckError(f"unsupported persistence encoding {binding['encoding']}")
    scope = str(binding["predicate"]) if binding.get("predicate") else "TRUE"
...
def audit_development_persistence(
    ...
    config_path, config = _development_database_config(repo_root, development_lib_root)
            ...
            references.append(
                {
                    ...
                    else ("missing" if definition_count == 0 else "duplicate"),
                }
            )
    ...
    result = {
        ...
        "tracked_bindings": sum(
            str(binding["record_type"]) in _ROL_RANGES for binding in PERSISTENT_BINDINGS
        ),
        ...
        "summary": {
            ...
            "unique_candidate_definitions": sum(row["status"] == "unique" for row in references),
