...
def _index_tokens(path: Path) -> list[tuple[str, int]]:
    ...
    for line_number, line in enumerate(path.read_bytes().splitlines(), start=1):
        for token in re.findall(rb"\S+", line):
...
def _manifest_order(source_inventory: dict[str, Any], kind: str) -> list[str]:
    ...
    manifest = next(item for item in source_inventory["manifests"] if item["name"] == manifest_name)
        ...
        for path in sorted(candidate for candidate in root.rglob("*") if candidate.is_file()):
            ...
            for line_number, line in enumerate(path.read_bytes().splitlines(), start=1):
                ...
                if b"vnum" not in lowered and not re.search(
                    rb"(?:assign(?:room|mob|obj)|real_(?:room|mobile|object))\s*\(",
                    lowered,
                ):
                    continue
