...
def _created_at(value: str | None) -> str:
    ...
    try:
        ...
        raise RolSpecialReconciliationError("--created-at must be an ISO-8601 timestamp") from error
...
def _verify_capability_audit(audit_dir: Path) -> dict[str, Any]:
    ...
    for artifact in manifest.get("artifacts", []):
        ...
        if not path.is_file():
            raise RolSpecialReconciliationError(
                f"capability-audit artifact is missing: {path.name}"
            )
...
def source_handler_definitions(source_root: Path, handlers: set[str]) -> dict[str, dict[str, Any]]:
    ...
    for path in sorted((source_root / "src").glob("*.c")):
        ...
        matches = [
            match for match in _FUNCTION_HEADER.finditer(masked) if match.group("name") in handlers
        ]
        if not matches:
            continue
...
def write_special_reconciliation_bundle(
    ...
    for ordinal, binding in enumerate(bindings, start=1):
        ...
        row = {
            "binding_id": (
                f"{record_type}:{source_vnum}:{binding['source_handler']}:{binding['source_line']}"
            ),
    ...
    automatic_by_record = {str(row["source_record_id"]): row for row in automatic_race_rows}
    ...
    for diagnostic in act_spec_input:
        ...
        if record is None:
            raise RolSpecialReconciliationError(
                f"ACT_SPEC record is absent from discovery: {record_id}"
            )
        source_vnum = int(record["vnum"])
        ...
        if linked:
            ...
            if automatic is not None:
                ...
                strategy = (
                    "DIRECT_ASSIGNMENTS_RESOLVED" if resolved else "DIRECT_ASSIGNMENTS_PENDING"
                )
            ...
            reason = (
                f"source boot attaches {automatic['source_handler']} before ACT_SPEC validation"
            )
        else:
            status = "resolved"
    ...
    summary = {
        ...
        "dynamic_bindings_by_handler": {
            str(row["source_handler"]): int(row["active_binding_instances"]) for row in dynamic_rows
        },
        ...
        "implicit_race_bindings_by_composition": dict(sorted(automatic_composition.items())),
    ...
    artifacts.append(_artifact(dynamic_path, output_dir, _write_jsonl(dynamic_path, dynamic_rows)))
    ...
    artifacts.append(
        _artifact(act_spec_path, output_dir, _write_jsonl(act_spec_path, act_spec_rows))
    )
    ...
    manifest = {
        ...
        "acceptance": {
            ...
            == sum(
                ...
                if record["kind"] == "mob" and _mobile_race_code(record) in _AUTO_RACE_HANDLERS
            ),
...
def render_rol_special_reconciliation_human(summary: dict[str, Any]) -> str:
    ...
    return "\n".join(
        (
            ...
            f"Source-preprocessor exclusions: {summary['source_preprocessor_excluded_bindings']}",
