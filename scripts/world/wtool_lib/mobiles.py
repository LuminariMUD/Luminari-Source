    ...
    if line is None:
        result.findings.append(
            finding(
                "MOB012",
                "error",
                f"mobile ends before its {label}",
                record.span,
                "mobile",
                record.vnum,
            )
    ...
    if dice_line is None:
        result.findings.append(
            finding(
                "MOB012",
                "error",
                "mobile ends before its dice line",
                record.span,
                "mobile",
                record.vnum,
            )
...
def _parse_path(
    ...
    record.references.extend(
        VnumReference("room", room, "mobile path", line.span) for room in rooms
    )
        ...
        if error is not None or len(values) != 2:
            result.findings.append(
                finding(
                    "MOB021",
                    "error",
                    f"{keyword} requires two integers",
                    line.span,
                    "mobile",
                    record.vnum,
                )
...
def parse_mobile_file(
    ...
    try:
        ...
        result.findings.append(
            finding(
                "MOB002", "error", f"cannot read mobile file: {error}", SourceSpan(display_path, 1)
            )
        )
        result.complete = False
        return result
    ...
    while True:
        ...
        if match is None:
            result.findings.append(
                finding("MOB003", "error", "expected a #vnum mobile header", header.span)
            )
        ...
        if parsed_vnum.error is not None or parsed_vnum.value is None:
            result.findings.append(
                finding("MOB003", "error", f"invalid mobile vnum: {parsed_vnum.error}", header.span)
            )
            _recover_record(cursor)
            continue
        ...
        if record.vnum < 0:
            result.findings.append(
                finding(
                    "MOB004",
                    "error",
                    "mobile vnum must be non-negative",
                    header.span,
                    "mobile",
                    record.vnum,
                )
        ...
        for _ in range(4):
            ...
            if not complete:
                record.complete = False
                return result
        record.aliases, record.short_description, record.long_description, record.description = (
            strings
        )
        ...
        if flags_line is None:
            result.findings.append(
                finding(
                    "MOB005",
                    "error",
                    "mobile ends before its flag header",
                    header.span,
                    "mobile",
                    record.vnum,
                )
            ...
            if alignment.error is not None or alignment.value is None:
                result.findings.append(
                    finding(
                        "MOB006",
                        "error",
                        "legacy alignment is not an integer",
                        flags_line.span,
                        "mobile",
                        record.vnum,
                    )
    ...
    if not found_end:
        result.findings.append(
            finding(
                "MOB028",
                "error",
                "mobile file is missing its '$' terminator",
