...
def parse_trigger_file(
    ...
    try:
        ...
        result.findings.append(
            finding(
                "TRG002", "error", f"cannot read trigger file: {error}", SourceSpan(display_path, 1)
            )
        )
        result.complete = False
        return result
    ...
    while True:
        ...
        if match is None:
            result.findings.append(
                finding("TRG003", "error", "expected a #vnum trigger header", header.span)
            )
        ...
        if parsed_vnum.error is not None or parsed_vnum.value is None:
            result.findings.append(
                finding(
                    "TRG003", "error", f"invalid trigger vnum: {parsed_vnum.error}", header.span
                )
            )
            _recover_record(cursor)
            continue
        ...
        if record.vnum < 0:
            result.findings.append(
                finding(
                    "TRG004",
                    "error",
                    "trigger vnum must be non-negative",
                    header.span,
                    "trigger",
                    record.vnum,
                )
        ...
        if numeric is None:
            result.findings.append(
                finding(
                    "TRG005",
                    "error",
                    "trigger ends before its numeric header",
                    header.span,
                    "trigger",
                    record.vnum,
                )
        ...
        if attach.error is not None or attach.value is None:
            result.findings.append(
                finding(
                    "TRG005",
                    "error",
                    "trigger attach type is not an integer",
                    numeric.span,
                    "trigger",
                    record.vnum,
                )
    ...
    if not found_end:
        result.findings.append(
            finding(
                "TRG010",
                "error",
                "trigger file is missing its '$' terminator",
