...
def _validate_zone_order(
    ...
    for zone in zones:
        ...
        if (
            ...
            and zone.vnum <= previous.vnum
            and _selected(zone, selected_packages)
        ):
            _append_related_finding(
                findings,
                "ZON041",
        ...
        for prior in prior_ranges:
            ...
            if (
                zone.bottom <= prior.top
                and prior.bottom <= zone.top
                and (_selected(zone, selected_packages) or _selected(prior, selected_packages))
            ):
...
def _validate_room_graph(
    ...
    for room in rooms:
        ...
        if (
            previous is not None
            and room.vnum <= previous.vnum
            and _selected(room, selected_packages)
        ):
            _append_related_finding(
                findings,
                "WLD041",
    ...
    for room_index, room in sorted(enumerate(rooms), key=lambda item: item[1].vnum):
        active_zones = [
            zone for zone in active_zones if zone.top is not None and zone.top >= room.vnum
        ]
    ...
    for room_index, room in enumerate(rooms):
        ...
        for exit_record in room.exits:
            if (
                exit_record.destination_vnum >= 0
        ...
        for command in zone.commands:
            ...
            if command.command == "D" and len(command.arguments) >= 2:
                ...
                if target is not None and not any(
                    exit_record.direction == direction for exit_record in target.exits
                ):
                    findings.append(
                        finding(
                            "REF007",
        ...
        for command in zone.commands:
            ...
            if command.command == "E" and len(command.arguments) >= 3:
                ...
                if obj is not None and 0 <= position < len(wear_slots):
                    wear_bits = decode_tokens(
                        obj.wear_flags, len(manifest["tables"]["obj-wear"]["entries"])
                    )
...
def validate_indexed_world(
    ...
    if selected_packages is not None:
        ...
        loaded_zone_packages = {int(path.stem) for path in zone_paths if path.stem.isdigit()}
