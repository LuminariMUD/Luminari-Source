...
def _normalization(
    ...
    matches = [
        ...
        or (item.get("source_header_vnum") == vnum and item.get("source_basename") == basename)
    ]
    if len(matches) > 1:
    ...
    if missing:
        raise RolIdentityError(f"normalization for {basename} is missing: {', '.join(missing)}")
    ...
    if target != logical + 20000:
        raise RolIdentityError(f"normalization for {basename} violates the canonical zone formula")
    logical_zones = item["logical_source_zones"]
    if not isinstance(logical_zones, list) or logical not in logical_zones:
        raise RolIdentityError(f"normalization for {basename} must include its logical source zone")
...
def canonical_destination(
    ...
    if kind == "zon":
        ...
        destination = normalized["target_zone_vnum"] if normalized is not None else vnum + 20000
