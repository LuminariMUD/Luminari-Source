...
def _identity_resolver(plan_dir: Path):
    ...
    identities = {
        (str(row["source_kind"]), int(row["source_vnum"])): row["destination_vnum"] for row in rows
    }
...
def _decode_flag_chunks(tokens: list[str]) -> set[int]:
    ...
    if decoded.issues:
        raise RolPilotBuildError(f"cannot decode existing mobile flag chunks {tokens!r}")
...
def _patch_mobile_block(
    ...
    if special_proc is not None:
        existing = [
            line.partition(":")[2].strip() for line in result if line.startswith("SpecProc:")
        ]
...
def _walkthrough_roots(rooms: dict[int, Any], preferred_root: int | None) -> list[dict[str, Any]]:
    covered: set[int] = set()
...
def _pilot_runtime_contract(world: WorldData, zone_vnums: Iterable[int]) -> dict[str, Any]:
    ...
    for zone_vnum in sorted(zone_vnums):
        ...
        rooms = {vnum: room for vnum, room in rooms_by_vnum.items() if room.file_zone == zone_vnum}
    ...
    return {
        "schema_version": 1,
        "zones": evidence,
        "all_reset_observations_pass": all(item["reset_observation_pass"] for item in evidence),
...
def write_pilot_build_bundle(
    ...
    if source_root != (repo_root / _SOURCE_ROOT_PREFIX).resolve():
        raise RolPilotBuildError(
            "Phase 4 build requires the inventoried repository RoL source root"
        )
    ...
    for action in actions:
        ...
        owner_attachments = tuple(
            sorted(attachments.get((_TARGET_KIND.get(kind, kind), destination), []))
        )
    ...
    patched_mobile_targets = {
        target_vnum for (kind, target_vnum) in attachments if kind == "mob"
    ...
    staged_findings = Counter(
        json.dumps(item, ensure_ascii=True, sort_keys=True) for item in validation_data["findings"]
    )
    ...
    new_active_errors = sum(
        item["severity"] == "error" and not item.get("suppressed", False) for item in new_findings
    )
    validation_delta = {
        "new_findings": sorted(
            new_findings,
    ...
    patched_targets = {target for patches in mobile_patches_by_file.values() for target in patches}
    dispositions: list[dict[str, Any]] = []
    special_disposition = {
        (row["source_record_type"], row["source_vnum"]): row for row in specials.dispositions
    }
