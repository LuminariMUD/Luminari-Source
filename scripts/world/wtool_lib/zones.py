...
def _validate_zone_header(
    ...
    if record.min_level is not None and not -1 <= record.min_level <= 100:
        _range_finding(
            result, record, "ZON011", f"minimum level {record.min_level} is outside -1..100"
        )
    if record.max_level is not None and not -1 <= record.max_level <= 100:
        _range_finding(
            result, record, "ZON012", f"maximum level {record.max_level} is outside -1..100"
        )
    if (
        ...
        _range_finding(
            result, record, "ZON013", "minimum level is greater than maximum level", "error"
        )
    ...
    if (
        region_count is not None
        and record.region is not None
        and not 0 <= record.region < region_count
    ):
        _range_finding(
            result,
            record,
            "ZON015",
            f"region {record.region} is outside 0..{region_count - 1}",
            "error",
        )
    ...
    if len(variable.encode("utf-8", errors="surrogateescape")) > 79:
        result.findings.append(
            finding(
                "ZON024",
                "error",
                "V variable name exceeds the 79-byte loader limit",
                line.span,
                "zone",
                zone_vnum,
            )
    ...
    if len(value.encode("utf-8", errors="surrogateescape")) > 79 or any(
        ...
        result.findings.append(
            finding(
                "ZON025",
                "error",
                "V value exceeds or violates the 79-byte loader field",
                line.span,
                "zone",
                zone_vnum,
            )
...
def _parse_command(
        ...
        if not 0 <= state <= 16:
            result.findings.append(
                finding(
                    "ZON030",
                    "error",
                    f"door state {state} is outside 0..16",
                    line.span,
                    "zone",
                    zone_vnum,
                )
        ...
        if attach_type not in {0, 1, 2}:
            result.findings.append(
                finding(
                    "ZON031",
                    "error",
                    f"trigger host type {attach_type} is outside 0..2",
                    line.span,
                    "zone",
                    zone_vnum,
                )
    ...
    if parsed.command == "J" and parsed.arguments and parsed.arguments[0] < 0:
        result.findings.append(
            finding(
                "ZON032", "error", "jump count must be non-negative", line.span, "zone", zone_vnum
            )
        )
    return parsed
...
def parse_zone_file(
    ...
    try:
        ...
        result.findings.append(
            finding(
                "ZON002", "error", f"cannot read zone file: {error}", SourceSpan(display_path, 1)
            )
        )
        result.complete = False
        return result
    ...
    if header is None:
        result.findings.append(
            finding("ZON003", "error", "zone file is empty", SourceSpan(display_path, 1))
        )
        result.complete = False
        return result
    ...
    if match is None:
        result.findings.append(
            finding("ZON003", "error", "expected a #vnum zone header", header.span)
        )
        result.complete = False
        return result
    ...
    if parsed_vnum.error is not None or parsed_vnum.value is None:
        result.findings.append(
            finding("ZON003", "error", f"invalid zone vnum: {parsed_vnum.error}", header.span)
        )
        result.complete = False
        return result
    ...
    if builders_line is None or name_line is None or numeric_line is None:
        result.findings.append(
            finding(
                "ZON002",
                "error",
                "zone header ends before all four fields",
                header.span,
                "zone",
                vnum,
            )
    ...
    for issue in decoded.issues:
        result.findings.append(
            finding(issue.code, "error", issue.message, numeric_line.span, "zone", vnum)
        )
        ...
        if command not in _COMMANDS:
            result.findings.append(
                finding(
                    "ZON019",
                    "error",
                    f"unknown or lowercase reset command {command!r}",
                    line.span,
                    "zone",
                    vnum,
                )
    ...
    if not found_sentinel:
        result.findings.append(
            finding(
                "ZON038",
                "error",
                "zone is missing its S command sentinel",
                header.span,
                "zone",
                vnum,
            )
        ...
        if terminator is None or not terminator.text.startswith("$"):
            ...
            result.findings.append(
                finding(
                    "ZON039",
                    "error",
                    "zone file is missing its conventional '$' terminator",
                    span,
                    "zone",
                    vnum,
                )
