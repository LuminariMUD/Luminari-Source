...
class SocCompilation:
    ...
    def autonomous_path_mobiles(self) -> set[int]:
        ...
        return {
            trigger.host_mobile_vnum for trigger in self.triggers if trigger.trigger_kind == "PATH"
        }
...
def build_soc_prototype_comparison(
    ...
    reduction = (
        100.0 * (len(selected) - len(compilation.triggers)) / len(selected) if selected else 0.0
    )
    return {
...
def _parse_path(record: RolRecord, diagnostics: list[str]) -> SocPath | None:
    ...
    for directive in record.directives:
        values[str(directive["token"])] = [int(value) for value in directive.get("arguments", [])]
    required = ("ID", "TYPE", "DELAY", "ROOMS", "DIRS")
    missing = [name for name in required if not values.get(name)]
    if missing:
        diagnostics.append(
            f"SOC path {record.basename}:{record.vnum}:{record.line} lacks " + ", ".join(missing)
        )
        return None
    rooms = values["ROOMS"]
...
def _command_record_lines(
    ...
    if not groups:
        return None
    trigger_rows = [directive for directive in record.directives if directive["token"] == "TRIGGER"]
    if not trigger_rows:
        diagnostics.append(
...
def _action_trigger(
    ...
    random_records = [record for record in records if record.format_version in {"LIST", "PERIODIC"}]
    ...
    if command_records:
        ...
        for record in command_records:
            compiled = _command_record_lines(record, paths, resolve, source_commands, diagnostics)
            if compiled is None:
                continue
    ...
    for record in random_records:
        compiled = _record_random_lines(record, paths, resolve, source_commands, diagnostics)
        if not compiled:
            continue
        body.extend(compiled)
    modes = tuple(
        sorted({record.format_version for record in records if record.format_version != "PATH"})
    )
    return SocTrigger(
        vnum=vnum,
...
def _time_trigger(
    ...
    return SocTrigger(
        ...
        trigger_kind="TIME",
        source_modes=("TIMED",),
        text=_trigger_text(vnum, f"RoL SOC {source_mobile} time {hour}", {19}, hour, "", body),
    )
...
_SOC_HEADER = re.compile(
    rb"MOB\s*:\s*([+-]?\d+)\s+(PERIODIC|TRIGGER|TIMED|LIST|PATH)\b",
    re.IGNORECASE,
)


_SOC_LOOSE_HEADER = re.compile(
    rb"MOB\s+([+-]?\d+)\s+(PERIODIC|TRIGGER|TIMED|LIST|PATH)\b",
    re.IGNORECASE,
)
            ...
            if key == "ACTION":
                ...
                if not ok:
                    ...
                    _diagnostic(
                        ...
                        "ROLSOC002",
                        "error",
                        "SOC action message is unterminated",
                        line,
                        "soc",
                        vnum,
                    )
