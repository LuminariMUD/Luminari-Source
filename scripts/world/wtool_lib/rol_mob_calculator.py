...
PROFILE_VERSION = 1
CONFIG_VERSION = 1
_VERSION = re.compile(r"^rol_mob_calculator protocol=(\d+) profile=(\d+) config=(\d+)$")
_STAT_FIELDS = (
    "hit_points",
    "hitroll",
    "armor_class",
...
class MobileCalculatorClient:
    ...
    def __init__(
        ...
        try:
            ...
            if executable is None:
                ...
                executable = next(
                    (
                        self.repo_root / str(name)
                        for name in names
                        if (self.repo_root / str(name)).is_file()
                    ),
            ...
            raise MobileCalculatorError(
                f"cannot resolve mobile calculator policy: {error}"
            ) from error
        if executable is None:
    ...
    def calculate(
        ...
        self.start()
        self._write(f"MOB {identifier} {level} {race} {ch_class} {tier} {custom_profile}\n")
        row = self._readline()
        tokens = row.split()
        ...
        if profile_version != PROFILE_VERSION:
            self._fail(f"calculator returned profile {profile_version}, expected {PROFILE_VERSION}")
        ...
        if returned_inputs != submitted_inputs:
            self._fail(f"calculator returned inputs {returned_inputs}, expected {submitted_inputs}")
        ...
        return MobileCalculation(
            ...
            expected_post_load=MobileStats(**dict(zip(_STAT_FIELDS, expected_values, strict=True))),
        )
