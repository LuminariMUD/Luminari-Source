    ...
    if numeric is None:
        result.findings.append(
            finding(
                "WLD012",
                "error",
                "exit block ends before its three integers",
                line.span,
                "room",
                room.vnum,
            )
    ...
    if error is not None or len(values) != 3:
        result.findings.append(
            finding(
                "WLD012",
                "error",
                "exit block requires three integers",
                numeric.span,
                "room",
                room.vnum,
            )
    ...
    if key_vnum in {-1, 65535}:
        ...
        result.findings.append(
            finding(
                "WLD014",
                "error",
                f"invalid negative key vnum {key_vnum}",
                numeric.span,
                "room",
                room.vnum,
            )
...
def _parse_moving_room(
    ...
    if error is not None or len(values) != 5:
        result.findings.append(
            finding(
                "WLD020",
                "error",
                "moving-room header requires five integers",
                line.span,
                "room",
                room.vnum,
            )
        ...
        if message is None:
            result.findings.append(
                finding(
                    "WLD022",
                    "error",
                    f"moving room is missing its {label} message",
                    line.span,
                    "room",
                    room.vnum,
                )
    ...
    while True:
        ...
        if conn_error is not None or len(conn_values) != 3:
            result.findings.append(
                finding(
                    "WLD024",
                    "error",
                    "moving-room connection requires three integers",
                    connection.span,
                    "room",
                    room.vnum,
                )
        ...
        connections.append(
            MovingConnectionRecord(target, direction, effective_repeat, connection.span)
        )
    if not found_end:
        result.findings.append(
            finding(
                "WLD027",
                "error",
                "moving-room connection list is missing its '~' sentinel",
                line.span,
                "room",
                room.vnum,
            )
        ...
        if len(tokens) < 2:
            result.findings.append(
                finding(
                    "WLD030",
                    "error",
                    "inline trigger attachment requires a trigger vnum",
                    line.span,
                    "room",
                    room.vnum,
                )
        ...
        if parsed.error is not None or parsed.value is None:
            result.findings.append(
                finding(
                    "WLD030",
                    "error",
                    f"invalid inline trigger vnum: {parsed.error}",
                    line.span,
                    "room",
                    room.vnum,
                )
...
def parse_room_file(
    ...
    try:
        ...
        result.findings.append(
            finding(
                "WLD004", "error", f"cannot read room file: {error}", SourceSpan(display_path, 1)
            )
        )
        result.complete = False
        return result
    ...
    while True:
        ...
        if match is None:
            result.findings.append(
                finding("WLD005", "error", "expected a #vnum room header", header.span)
            )
        ...
        if parsed_vnum.error is not None or parsed_vnum.value is None:
            result.findings.append(
                finding("WLD005", "error", f"invalid room vnum: {parsed_vnum.error}", header.span)
            )
            _recover_record(cursor)
            continue
        ...
        if vnum < 0:
            result.findings.append(
                finding(
                    "WLD006", "error", "room vnum must be non-negative", header.span, "room", vnum
                )
            )
        ...
        if flags_line is None:
            result.findings.append(
                finding(
                    "WLD007",
                    "error",
                    "room ends before its flags and sector line",
                    header.span,
                    "room",
                    vnum,
                )
        ...
        if (
            file_zone.error is not None
            or sector.error is not None
            or file_zone.value is None
            or sector.value is None
        ):
            result.findings.append(
                finding(
                    "WLD008",
                    "error",
                    "invalid integer on room flags line",
                    flags_line.span,
                    "room",
                    vnum,
                )
        ...
        for issue in decoded.issues:
            result.findings.append(
                finding(issue.code, "error", issue.message, flags_line.span, "room", vnum)
            )
                ...
                if coordinates is None:
                    result.findings.append(
                        finding(
                            "WLD016",
                            "error",
                            "coordinate record is missing its numeric line",
                            line.span,
                            "room",
                            vnum,
                        )
                ...
                if error is not None or len(values) != 2:
                    result.findings.append(
                        finding(
                            "WLD016",
                            "error",
                            "coordinate record requires two integers",
                            coordinates.span,
                            "room",
                            vnum,
                        )
                ...
                if spec_line is None:
                    result.findings.append(
                        finding(
                            "WLD029",
                            "error",
                            "Z record is missing its spec-proc name",
                            line.span,
                            "room",
                            vnum,
                        )
        ...
        if not found_room_end:
            result.findings.append(
                finding(
                    "WLD032",
                    "error",
                    "room is missing its S record terminator",
                    header.span,
                    "room",
                    vnum,
                )
    ...
    if not found_file_end:
        result.findings.append(
            finding(
                "WLD033",
                "error",
                "room file is missing its conventional '$' terminator",
