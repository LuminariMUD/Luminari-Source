...
def _leading_integers(line: SourceLine) -> list[int]:
    ...
    for token in line.raw.strip().split():
        if re.fullmatch(rb"[+-]?\d+", token) is None:
            break
    ...
    for start, end, vnum in _segments(source):
        ...
        if not first_ok or not second_ok or not third_ok:
            ...
            _diagnostic(
                ...
                "ROLZON001",
                "error",
                "zone string header is incomplete",
                source.lines[start],
                "zon",
                vnum,
            )
        ...
        if len(header_values) not in {4, 7}:
            ...
            _diagnostic(
                ...
                "ROLZON002",
                "error",
                "zone header must have four or seven integers",
                header or source.lines[start],
                "zon",
                vnum,
            )
        ...
        if header is not None:
            record.directives.append(
                {"token": "HEADER", "line": header.number, "field_count": len(header_values)}
            )
        ...
        for _ in range(6):
            ...
            record.directives.append(
                {"token": "CLIMATE", "line": climate.number, "field_count": len(values)}
            )
        ...
        while position < end:
            ...
            if command in _RESET_COMMANDS:
                ...
                if command == "G" and stripped.startswith((b"GROUPING", b"GATE QUEST STUFF")):
                    ...
                    _diagnostic(
                        ...
                        "ROLZON003",
                        "warning",
                        "source heading is misread as a malformed G reset",
                        line,
                        "zon",
                        vnum,
                    )
            ...
            _diagnostic(
                ...
                "ROLZON004",
                "error",
                f"unknown reset-stream token {command!r}",
                line,
                "zon",
                vnum,
            )
        if not terminated:
            ...
            _diagnostic(
                ...
                "ROLZON005",
                "error",
                "zone record lacks S terminator",
                source.lines[start],
                "zon",
                vnum,
            )
    ...
    try:
        if command in {"M", "O", "P", "E"}:
            ...
            if command in {"M", "O"}:
                ...
                elif source_prototype in source_tail_ring_objects and source_position in {24, 25}:
                    destination = EQUIPMENT_POSITION_MAP[25]
