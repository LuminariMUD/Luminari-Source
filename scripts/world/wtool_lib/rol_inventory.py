...
def _build_packages(
    ...
    for basename in sorted(basenames):
        ...
        included_kinds = [
            kind for kind in present_kinds if by_kind[kind][basename]["status"] == "active"
        ]
        disabled_kinds = [
            kind for kind in present_kinds if by_kind[kind][basename]["status"] == "disabled"
        ]
        unlisted_kinds = [
            kind for kind in present_kinds if by_kind[kind][basename]["status"] == "unlisted"
        ]
...
def render_rol_inventory_human(inventory: dict[str, Any]) -> str:
    ...
    multi_zone_names = ", ".join(item["basename"] for item in classified["active_multi_zone_files"])
    lines = [
