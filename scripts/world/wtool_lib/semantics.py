...
def _validate_reserved_flags(
    ...
    for obj in objects:
        ...
        _check_flag_tokens(findings, obj, "object", "object wear", obj.wear_flags, wear_table, {})
        _check_flag_tokens(
            findings,
            obj,
            "object",
...
def _validate_exit_topology(
    ...
    exits_by_room = {room.vnum: _effective_exits(room, direction_count) for room in rooms}
    directions = manifest["tables"]["directions"]["entries"]
    reverse = {
        int(entry["index"]): int(entry["reverse_index"]) for entry in directions[:direction_count]
    }
...
def _validate_room_level_ranges(
    ...
    for room in rooms:
        ...
        reversed_range = minimum_level > 0 and maximum_level > 0 and minimum_level > maximum_level
...
def _validate_reachability(
    ...
    for zone in zones:
        ...
        entrances = {vnum for vnum in incoming_roots.get(zone.vnum, set()) if vnum in zone_vnums}
        entrances |= portal_exact & zone_vnums
        entrances |= script_targets & zone_vnums
        entrances |= {
            vnum for vnum in zone_vnums if any(low <= vnum <= high for low, high in portal_ranges)
        }
    ...
    for reset_zone in zones:
        ...
        for command in reset_zone.commands:
            ...
            if (
                mobile is None
                or room is None
                or room.owner_zone_vnum is None
                or mobile.level is None
            ):
                continue
        ...
        if quest.quest_type == quest_types["AQ_COMPLETE_MISSION"]:
            ...
            if quest.wilderness_x is None or quest.wilderness_y is None:
                _quest_finding(
                    ...
                    field_name=("wilderness_x" if quest.wilderness_x is None else "wilderness_y"),
                )
            ...
            _quest_finding(
                ...
                quest,
                "SEM026",
                "Luminari QST files cannot persist the mobile list required by AQ_MOB_MULTI_KILL",
                field_name="quest_type",
            )
...
def _quest_cycles(quests_by_vnum: dict[int, QuestRecord], field_name: str) -> list[list[int]]:
    ...
    for quest in hlquests:
        ...
        for entry in quest.entries:
            ...
            if entry.entry_type == entry_types["QUEST_ASK"]:
                ...
                if entry.entry_type == entry_types["QUEST_ROOM"] and entry.room_vnum is None:
                    _hlquest_finding(
                        findings,
                        quest,
                        "SEM030",
...
def validate_semantics(
    ...
    findings: list[Finding] = []
    _validate_reserved_flags(rooms, mobiles, objects, findings, selected_packages, manifest)
    ...
    _validate_reset_levels(zones, rooms, mobiles, findings, selected_packages)
    _validate_dangerous_rooms(zones, rooms, findings, selected_packages, manifest)
