...
def confirmed_lineage_packages(
    ...
    for record in records:
        ...
        if any(_strong_formula(item) for item in row["candidates"]):
            strong[basename] += 1
        if record["kind"] == "zon" and any(item["confirmed_seed"] for item in row["candidates"]):
...
def _formula_candidate(row: dict[str, Any]) -> dict[str, Any] | None:
    candidates = [
        candidate for candidate in row["candidates"] if "canonical_formula" in candidate["evidence"]
    ]
...
def _new_destination(
    ...
    return canonical_destination(record["kind"], record["vnum"], record["basename"], policy)
...
def build_record_actions(
    ...
    for record in records:
        ...
        item = {
            ...
            "dependency_resolutions": dict(sorted(reference_counts[record["record_id"]].items())),
            "rationale": rationale,
...
def _validate_actions(actions: list[dict[str, Any]], policy: dict[str, Any]) -> None:
    ...
    for item in actions:
        ...
        if any(
            ...
            raise RolPlanError(f"ADD destination already exists for {item['source_record_id']}")
    ...
    manifest = {
        ...
        "acceptance": {
            ...
            "ambiguous_candidates_preserved": all(
                item["action"] != "PATCH"
                for item in actions
                if item["candidate_state"] == "candidates" and item["selected_target"] is None
            ),
            "live_target_writes": 0,
