    ...
    if line is None:
        result.findings.append(
            finding(
                "OBJ019",
                "error",
                "A extension is missing its payload",
                record.span,
                "object",
                record.vnum,
            )
    ...
    if line is None:
        result.findings.append(
            finding(
                "OBJ019",
                "error",
                "C extension is missing its payload",
                record.span,
                "object",
                record.vnum,
            )
...
def _add_typed_references(record: ObjectRecord, manifest: dict[str, Any]) -> None:
    ...
    if record.item_type in {item_types["ITEM_CONTAINER"], item_types["ITEM_AMMO_POUCH"]}:
        if values[2] > 0:
            record.references.append(
                VnumReference("object", values[2], "container key", record.span)
            )
        ...
        if portal_type in exact_types and values[1] > 0:
            record.references.append(
                VnumReference("room", values[1], "portal destination", record.span)
            )
        elif portal_type == manifest["limits"]["PORTAL_RANDOM"]["value"]:
            if values[1] > 0:
                record.references.append(
                    VnumReference("room", values[1], "portal range start", record.span)
                )
            if values[2] > 0:
                record.references.append(
                    VnumReference("room", values[2], "portal range end", record.span)
                )
    ...
    while True:
        ...
        if kind == "A":
                ...
                if not 1 <= values[0] <= manifest["limits"]["MAX_SPELLS"]["value"]:
                    result.findings.append(
                        finding(
                            "OBJ024",
                            "error",
                            f"invalid spellbook spell {values[0]}",
                            span,
                            "object",
                            record.vnum,
                        )
            ...
            if kind == "I":
                ...
                if value > 0:
                    record.references.append(
                        VnumReference("mobile", value, "object recipient", span)
                    )
        elif kind == "R":
            ...
            if payload_line is None:
                result.findings.append(
                    finding(
                        "OBJ019",
                        "error",
                        "R extension is missing its identifier",
                        line.span,
                        "object",
                        record.vnum,
                    )
                ...
                if not 1 <= spell <= manifest["limits"]["NUM_SPELLS"]["value"]:
                    result.findings.append(
                        finding(
                            "OBJ024",
                            "error",
                            f"invalid activated spell {spell}",
                            payload[1],
                            "object",
                            record.vnum,
                        )
            ...
            if parsed is None or parsed.error is not None or parsed.value is None:
                ...
                record.attachments.append(
                    AttachmentRecord(parsed.value, "object", record.vnum, line.span)
                )
        elif kind == "Z":
            ...
            if spec_line is None:
                result.findings.append(
                    finding(
                        "OBJ030",
                        "error",
                        "Z extension is missing its spec-proc name",
                        line.span,
                        "object",
                        record.vnum,
                    )
...
def parse_object_file(
    ...
    try:
        ...
        result.findings.append(
            finding(
                "OBJ002", "error", f"cannot read object file: {error}", SourceSpan(display_path, 1)
            )
        )
        result.complete = False
        return result
    ...
    while True:
        ...
        if match is None:
            result.findings.append(
                finding("OBJ003", "error", "expected a #vnum object header", header.span)
            )
        ...
        if parsed_vnum.error is not None or parsed_vnum.value is None:
            result.findings.append(
                finding("OBJ003", "error", f"invalid object vnum: {parsed_vnum.error}", header.span)
            )
            _recover_record(cursor)
            continue
        ...
        if record.vnum < 0:
            result.findings.append(
                finding(
                    "OBJ004",
                    "error",
                    "object vnum must be non-negative",
                    header.span,
                    "object",
                    record.vnum,
                )
        ...
        for _ in range(4):
            ...
            if not complete:
                record.complete = False
                return result
        record.aliases, record.short_description, record.description, record.action_description = (
            strings
        )
        ...
        if first_line is None:
            result.findings.append(
                finding(
                    "OBJ005",
                    "error",
                    "object ends before its first numeric line",
                    header.span,
                    "object",
                    record.vnum,
                )
        ...
        if value_line is None:
            result.findings.append(
                finding(
                    "OBJ011",
                    "error",
                    "object ends before its value vector",
                    header.span,
                    "object",
                    record.vnum,
                )
        ...
        if cost_line is None:
            result.findings.append(
                finding(
                    "OBJ012",
                    "error",
                    "object ends before weight/cost fields",
                    header.span,
                    "object",
                    record.vnum,
                )
        ...
        if record.weight < 0:
            result.findings.append(
                finding(
                    "OBJ013",
                    "error",
                    "object weight must not be negative",
                    cost_line.span,
                    "object",
                    record.vnum,
                )
        ...
        if record.rent < 0:
            result.findings.append(
                finding(
                    "OBJ014",
                    "error",
                    "object rent must not be negative",
                    cost_line.span,
                    "object",
                    record.vnum,
                )
    ...
    if not found_end:
        result.findings.append(
            finding(
                "OBJ032",
                "error",
                "object file is missing its '$' terminator",
