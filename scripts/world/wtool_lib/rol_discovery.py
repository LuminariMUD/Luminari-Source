...
def lineage_candidates(
    ...
    if record.kind in _DEFINITION_TYPE or record.kind == "zon":
        canonical_vnum = canonical_destination(record.kind, record.vnum, record.basename, policy)
    else:
...
def _region_code(region: dict[str, Any], active_lines: dict[int, list[str]] | None) -> str:
...
def extract_spec_bindings(
    ...
    if follow_registration_wrappers:
        ...
        if preprocess:
            ...
            for row in bindings:
                row["vnum_resolution"] = "literal" if row["vnum"] is not None else "unresolved"
        return sorted(
            bindings,
            key=lambda item: (
                item["record_type"],
    ...
    patterns = (
        re.compile(
            r"AddProc(?P<kind>Mob|Obj|Room)\s*\(\s*(?P<vnum>[A-Za-z_]\w*|\d+)\s*,\s*(?P<handler>[A-Za-z_]\w*)"
        ),
        re.compile(
            r"ASSIGN(?P<kind>MOB|OBJ|ROOM)\s*\(\s*(?P<vnum>[A-Za-z_]\w*|\d+)\s*,\s*(?P<handler>[A-Za-z_]\w*)"
        ),
    )
    ...
    for path in sorted(root.glob(relative_glob)):
        ...
        if preprocess:
            ...
            for preprocessed_line in completed.stdout.splitlines():
                ...
                current_line += 1
        for line_number, line in _strip_code_comments(
...
def merge_source_spec_binding_candidates(
    ...
    for raw in raw_bindings:
        ...
        if active is not None and active["vnum"] is not None:
            ...
            row["vnum_resolution"] = "literal" if raw["vnum"] is not None else "preprocessor"
        result.append(row)
    return sorted(
        result,
        key=lambda item: (
...
def build_binding_candidates(
    ...
    active = {
        ...
        for record in corpus.records
        if record.kind in _DEFINITION_TYPE and record.values.get("source_disposition") != "EXCLUDE"
    }
...
def iter_reference_resolutions(
    ...
    for record in sorted(
        corpus.records, key=lambda item: (item.kind, item.vnum, item.path, item.line)
    ):
...
def _semantic_signature(record: RolRecord) -> str:
    payload = {
        ...
        "directives": [
            {key: value for key, value in directive.items() if key not in {"line"}}
        ...
        results.append(
            {
                ...
                or (
                    ...
                    and sum(extra.values()) <= len(byte_by_kind[kind]["dropped_unterminated_tails"])
                ),
            }
        )
    return {
        ...
        "all_semantically_reconciled": all(item["semantically_reconciled"] for item in results),
...
def write_discovery_bundle(
    ...
    binding_candidates = build_binding_candidates(source_bindings, target_bindings, corpus)
    capabilities = build_capability_matrix(corpus)
    aggregate_semantics = reconcile_aggregate_semantics(source_root, corpus, byte_reconciliation)
    ...
    def reference_rows() -> Iterator[dict[str, Any]]:
        for row in iter_reference_resolutions(corpus, source_root, inventory, catalog, commands):
    ...
    manifest = {
        ...
        "acceptance": {
            ...
            "active_records": len(corpus.records),
            "tokens_classified": sum(len(record.directives) for record in corpus.records),
    ...
    return {
        ...
        "aggregate_semantics_reconciled": aggregate_semantics["all_semantically_reconciled"],
        "target_parse_complete": world.complete,
...
def render_rol_discovery_human(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"Dependency closure complete: {str(summary['dependency_closure_complete']).lower()}",
        f"Aggregate semantics equal: {str(summary['aggregate_semantics_equal']).lower()}",
        f"Aggregate semantics reconciled: {str(summary['aggregate_semantics_reconciled']).lower()}",
