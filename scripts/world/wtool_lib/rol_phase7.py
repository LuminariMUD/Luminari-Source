...
def _augment_actions(
    ...
    metadata = {
        str(row["record_id"]): row for row in _load_jsonl(discovery_dir / "source-records.jsonl")
    }
...
def _package_batches(
    ...
    packages = sorted(
        str(row["basename"]) for row in inventory["packages"] if row.get("included") is True
    )
...
def _target_room_zone(
    ...
    owners = [zone for zone, (bottom, top) in ranges.items() if bottom <= destination_vnum <= top]
    if len(owners) == 1:
        return owners[0]
    return fallback
    ...
    for key, bindings in sorted(grouped.items()):
        ...
        if len(persisted) > 1:
            ...
            if expected is None or tuple(persisted) != expected:
                raise RolPhase7Error(f"unmeasured composite special profile {key}: {persisted!r}")
        ...
        result[key] = replace(
            ...
            value_reference_slots=tuple(
                sorted({slot for binding in bindings for slot in binding.value_reference_slots})
            ),
        )
...
def _clear_rol_namespace(world_root: Path) -> set[str]:
    ...
    for kind in ("zon", "wld", "mob", "obj", "shp", "trg", "qst", "hlq"):
        ...
        for path in sorted(directory.glob(f"*.{kind}")):
            headers = {int(value) for value in re.findall(rb"(?m)^#(\d+)~?\s*$", path.read_bytes())}
...
def _compile_soc_corpus(
    ...
    for basename in sorted(by_basename):
        ...
        if basename in fixed:
            ...
            candidates = [
                value for value in range(zone * 100, zone * 100 + 100) if value not in reserved
            ]
        try:
        ...
        if used & reserved:
            raise RolPhase7Error(
                f"SOC trigger collision for {basename}: {sorted(used & reserved)[0]}"
            )
...
def _patch_preserved_records(
    ...
    touched: set[str] = set()
    for (kind, path), file_patches in sorted(
        by_file.items(), key=lambda item: item[0][1].as_posix()
    ):
        prefix, blocks = _split_world_file(path, kind)
...
def _validation_delta(baseline: dict[str, Any], staged: dict[str, Any]) -> dict[str, Any]:
...
def _selected_reference_exceptions(
    ...
    for row in references:
        ...
        if (
            ...
            and (str(row.get("target_type")), int(row.get("target_vnum", -1))) in repaired_targets
        ):
            continue
        disposition = (
...
def write_phase7_bundle(
    ...
    capability_manifest = _verify_bundle(capability_audit_dir, 5, "full-corpus-capability-audit")
    ...
    actions, repaired_identities, source_repairs = _apply_source_record_repairs(actions, records)
    ...
    selected_packages = {package for batch in batches[:through_batch] for package in batch}
    selected_actions = [
    ...
    if (
        ...
        raise RolPhase7Error(
            "full special compilation drifted from the sealed Phase 6 measurements"
        )
    native = _aggregate_native_bindings(special_compilation)

    command_evidence = extract_source_commands(source_root)
    source_commands = {
        int(row["action_code"]): str(row["command"]) for row in command_evidence["commands"]
    }
    ...
    for trigger in [*soc.triggers, *special_compilation.triggers]:
        ...
        generated[relative].append((trigger.vnum, trigger_text, f"trigger:{trigger.vnum}"))
        transform_diagnostics.extend(
            {
    ...
    batch_rows = [
        {
            ...
            "record_count": sum(1 for action in actions if str(action["basename"]) in set(batch)),
        }
        for index, batch in enumerate(batches, start=1)
    ]
    package_rows = [
        {
            "basename": package,
            "batch": next(
                index for index, batch in enumerate(batches, start=1) if package in batch
            ),
    ...
    for prior_dir in prior_milestone_dirs:
        ...
        prior_runs.append({"run_id": prior["run_id"], "through_batch": prior["through_batch"]})
    ...
    source_parse_complete = not any(row.get("severity") == "error" for row in source_diagnostics)
    manifest = {
        ...
        "acceptance": {
            ...
            == sum(record.kind == "soc" for record in selected_records)
            - sum(record.record_id in _SOC_OWNER_EXCLUSIONS for record in selected_records),
