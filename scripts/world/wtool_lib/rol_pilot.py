...
def package_metrics(
    ...
    modes = Counter(
        record["format_version"] for record in selected_records if record["kind"] == "soc"
    )
    ...
    capabilities = set().union(*(_directive_ids(record) for record in selected_records))
    reset_tokens = {
        directive["token"]
        for record in selected_records
        if record["kind"] == "zon"
...
def write_pilot_selection_bundle(
    ...
    selected_records = [record for record in records if record["basename"] in PILOT_BASENAMES]
    selected_ids = {record["record_id"] for record in selected_records}
    selected_actions = [action for action in actions if action["source_record_id"] in selected_ids]
    selected_identities = [
        identity for identity in identities if identity["source_record_id"] in selected_ids
    ]
    selected_references = [
        reference for reference in references if reference["source_record_id"] in selected_ids
    ]
    packages = [
