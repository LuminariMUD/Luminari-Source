...
_CODE_EVIDENCE_PATHS = (
    ...
    "scripts/world/wtool_lib/rol_weapon_mapping.py",
    "scripts/world/wtool_lib/rol_weapon_table.py",
...
_DOCUMENTATION_PATHS = (
    ...
    "docs/previous_changelogs/old_CHANGELOG_2026_09_11.md",
...
def _repeat_evidence(primary_dir: Path, repeat_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "output_tree_byte_identical": primary_output["tree_sha256"] == repeat_output["tree_sha256"],
...
def _mechanics_isolation_audit(repo_root: Path, baseline_world, candidate_world) -> dict[str, Any]:
    ...
    low_baseline_markers = [
        ...
        if int(row["vnum"]) < (20_000 if row["owner_type"] == "zone" else 2_000_000)
    ]
    ...
    return {
        ...
        "mechanics_source_file_sha256": {
            path.relative_to(repo_root).as_posix(): _sha256_path(path) for path in mechanics_paths
        },
...
def _action_audit(
    ...
    if baseline_world is not None:
        ...
        existing_target_merge_destinations = [
            ...
            if row.get("destination_vnum") in baseline_by_kind.get(str(row["source_kind"]), set())
        ]
    return {
...
def _behavior_evidence(world, selected_zones: set[int]) -> dict[str, Any]:
    reset_counts = Counter(command.command for zone in world.zones for command in zone.commands)
    ...
    samples = {
        ...
        "hlquests": sorted(record.vnum for record in world.hlquests if record.vnum >= 2_000_000)[
            :20
        ],
        ...
        "triggered_rooms": sorted(
            record.vnum for record in world.rooms if record.vnum >= 2_000_000 and record.attachments
        )[:20],
    }
    counts = {
...
def _code_gates(repo_root: Path, logs: dict[str, Path]) -> dict[str, Any]:
    if set(logs) != set(_LOG_NAMES):
        raise RolPhase8Error(
            "Phase 8 requires world-tools, cutest, install, syntax, and runtime logs"
        )
...
def write_phase8_bundle(
    ...
    with tempfile.TemporaryDirectory(prefix="rol-phase8-") as temporary:
        ...
        action_audit = _action_audit(action_rows, candidate_validation, baseline_world)
        selected_zones = {
        ...
        mechanics_isolation = _mechanics_isolation_audit(repo_root, baseline_world, world)
        persistence = audit_development_persistence(world, repo_root, development_lib_root)
    ...
    namespace = {
        ...
        "missing_persisted_targets": persistence_summary["missing_candidate_definitions"],
        "duplicate_persisted_targets": persistence_summary["duplicate_candidate_definitions"],
    }
    ...
    evidence = {
        "freeze.json": {
            ...
            "repeat_phase7_manifest_sha256": _sha256_path(repeat_phase7_dir / "run-manifest.json"),
    ...
    manifest = {
        ...
        "acceptance": {
            ...
            "cross_world_typed_references": graph_summary.get("cross_world_typed_references"),
            "missing_namespace_targets": graph_summary.get("missing_namespace_targets"),
...
def _documentation_audit(repo_root: Path) -> dict[str, Any]:
    ...
    changelog_path = repo_root / "docs/previous_changelogs/old_CHANGELOG_2026_09_11.md"
