...
def _quest_arguments(
    ...
    while position < end:
        subtype_match = re.match(rb"\s*([A-Za-z])", payload)
        values = (
            [int(value) for value in _INTEGER.findall(payload[subtype_match.end() :])]
            if subtype_match
            else []
        )
    ...
    for start, end, vnum in _segments(source):
        ...
        while position < end:
            ...
            if token in {"Q", "D"}:
                ...
                record.directives.append({"token": token, "line": line.number, "message": message})
                if not ok:
                    record.complete = False
                continue
            if token in {"G", "R"}:
                ...
                if token == "G" and subtype == "I":
                    ...
                    if destination > number and destination - number < 100:
                        _reference(
                            record, "object", destination, "quest_random_reward_end", argument_line
                        )
            ...
            _diagnostic(
                ...
                "ROLQST002",
                "error",
                f"unknown quest directive {token!r}",
                line,
                "qst",
                vnum,
            )
    ...
    for _, entry_type, payload in sorted(entries, key=lambda item: item[0]):
        if entry_type == "M":
            ...
            if not message_value.strip():
                message_value = "."
                diagnostics.append(f"replaced an empty ASK reply at source line {payload['line']}")
