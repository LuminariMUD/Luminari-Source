...
def _parser() -> argparse.ArgumentParser:
    ...
    rol_pilot_build.add_argument("--source-root", type=Path, default=_default_rol_source_root())
    ...
    rol_special_reconciliation.add_argument("--capability-audit-dir", type=Path, required=True)
    ...
    rol_phase7.add_argument("--source-root", type=Path, default=_default_rol_source_root())
    ...
    rol_phase8_apply.add_argument("--lib-root", type=Path, default=default_repo_root() / "lib")
    ...
    rol_phase8_completion.add_argument("--lib-root", type=Path, default=default_repo_root() / "lib")
    ...
    if args.flag_command == "decode":
        ...
        if args.json_output:
            ...
            for entry in data["entries"]:
                sys.stdout.write(
                    f"{entry['index']:3d} {entry.get('macro') or '-':<36} {entry['name']}\n"
                )
            for issue in decoded.issues:
...
def _run_rol_persistence_check(args: argparse.Namespace) -> int:
    ...
    result = audit_development_persistence(world, repo_root, args.development_lib_root)
    if args.json_output:
        _print_json(result)
    else:
