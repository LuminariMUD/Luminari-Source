...
_SHOP_HEADER = re.compile(rb"SHOP\s*:\s*([+-]?\d+)", re.IGNORECASE)
            ...
            record.directives.append(
                {
                    ...
                    "line": line.number,
                    "arguments": values,
                    "text": raw_value.strip().decode("utf-8", errors="surrogateescape"),
                }
            )
            if key == "ROOM":
                for value in values:
...
def emit_shop(
    ...
    if not intervals:
        intervals = [(0, 28)]
        diagnostics.append(
            "source shop has no effective open-hour interval; used always-open target hours"
        )
    if len(intervals) > 2:
        diagnostics.append(
