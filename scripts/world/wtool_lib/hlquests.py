...
def _record_error(
    ...
    result.findings.append(finding(code, "error", message, span, "hlquest", record.vnum))
...
def _read_string(
    ...
    for issue in value.issues:
        result.findings.append(source_issue_finding(issue, "HLQ", "hlquest", record.vnum))
        if issue.fatal:
            _mark_incomplete(result, record, entry)
...
def _command_type_map(manifest: dict[str, Any]) -> dict[str, int]:
    return {
        entry["code"]: entry["index"] for entry in manifest["tables"]["hlquest-commands"]["entries"]
    }
...
def _finalize_command_order(entry: HlQuestEntryRecord) -> None:
    inputs = [
        command for command in entry.commands if command.effective and command.direction == "input"
    ]
    outputs = [
        command for command in entry.commands if command.effective and command.direction == "output"
    ]
...
def _reference_role(entry: HlQuestEntryRecord, command: HlQuestCommandRecord, role: str) -> str:
    ...
    return f"entry {entry.physical_ordinal} {direction} command {command.physical_ordinal} {role}"
