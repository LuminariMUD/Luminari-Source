...
def audit_connection_graph(
    ...
    destination_by_record = {
        str(action["source_record_id"]): int(action["destination_vnum"]) for action in room_actions
    }
    destinations_by_source: defaultdict[int, set[int]] = defaultdict(set)
    for action in room_actions:
        destinations_by_source[int(action["source_vnum"])].add(int(action["destination_vnum"]))
    ...
    expected_object_source: dict[tuple[int, str, int], tuple[str, int, int, str, int]] = {}
    ...
    for action in action_rows:
        if _action_disposition(action) != "EXCLUDE" and action.get("destination_vnum") is not None:
            mapped_by_kind[str(action.get("source_kind"))].add(int(action["destination_vnum"]))
    ...
    missing_rows = [_edge_payload(*edge, expected_source.get(edge)) for edge in missing]
    extra_rows = [{**_edge_payload(*edge), **actual_locations.get(edge, {})} for edge in extra]
    missing_object_connections = sorted(expected_object_connections - actual_object_connections)
    extra_object_connections = sorted(actual_object_connections - expected_object_connections)
    missing_object_rows = [
        {
    ...
    return {
        ...
        "summary": {
            ...
            "expected_resolvable_object_room_connections": len(expected_object_connections),
            "unresolved_source_object_connections": len(unresolved_source_object_connections),
