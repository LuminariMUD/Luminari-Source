...
def is_safe_path_component(name: str) -> bool:
    ...
    return bool(
        name and name not in {".", ".."} and ".." not in name and _SAFE_COMPONENT.fullmatch(name)
    )
...
def _index_tokens(data: bytes) -> list[tuple[str, int]]:
    ...
    for line_number, line in enumerate(data.splitlines(), start=1):
        for raw_token in re.findall(rb"\S+", line):
...
def _parse_index(
    ...
    if not index_path.is_file():
        findings.append(
            _finding(
                "IDX009",
                "error",
                f"required index file is missing for .{extension} data",
                display_path,
            )
        )
    ...
    for token, line_number in tokens:
        ...
        if not safe:
            findings.append(
                _finding(
                    "IDX001", "error", f"unsafe data filename {token!r}", display_path, line_number
                )
            )
    ...
    if not parsed.terminated:
        findings.append(
            _finding(
                "IDX005", "error", "index is missing the conventional '$' terminator", display_path
            )
        )
        parsed.complete = False
    return parsed
