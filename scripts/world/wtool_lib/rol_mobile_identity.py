    ...
    def ledger(self) -> dict[str, Any]:
        ...
        return {
            ...
            "sex": {
                ...
                if any("sex" in repair for repair in self.repairs)
                else "EXACT",
...
def _load_source_race_registry_snapshot(
    ...
    for row in rows:
        ...
        if race.code in registry or race.source_index in source_indexes or race.macro in macros:
            raise MobileConversionError(
                "source race registry snapshot contains a duplicate identity"
            )
...
def load_source_race_registry(
    repo_root: Path, mobile_policy: dict[str, Any]
) -> dict[str, SourceRace]:
    ...
    race_values = {
        ...
        for macro, value in re.findall(
            r"^#define\s+(RACE_[A-Z0-9_]+)\s+(\d+)\b", header, re.MULTILINE
        )
    }
    ...
    for macro, code in lookup_rows:
        ...
        if index >= len(names) or index >= len(size_rows) or len(size_rows[index]) != 12:
            raise MobileConversionError(
                f"source race {macro} index {index} has no complete metadata"
            )
        if code in registry:
...
def load_mobile_conversion_policy(
    ...
    policy = json.loads(
        (root / "scripts/world/rol_conversion_policy.json").read_text(encoding="ascii")
    )
...
def _validate_profile(profile: dict[str, Any], manifest: dict[str, Any]) -> None:
    ...
    if not isinstance(subraces, list) or len(subraces) > 3 or len(set(subraces)) != len(subraces):
        raise MobileConversionError(
            "mobile rule subraces must be a unique list of at most three symbols"
        )
...
def _validate_policy(
    ...
    if set(assigned) != set(registry):
        ...
        raise MobileConversionError(
            f"base profile registry mismatch; missing={missing}, extra={extra}"
        )
    ...
    for source_class, target_symbol in policy.get("class_map", {}).items():
        if not source_class.isdigit() or target_symbol not in class_symbols:
            raise MobileConversionError(
                f"invalid class mapping {source_class!r}: {target_symbol!r}"
            )
    ...
    for rule in policy.get("exact_records", []):
        ...
        if custom_profile is not None:
            if (
                custom_profile not in custom_profiles
                or not isinstance(custom_hit_points, int)
                or (custom_hit_points < 1)
            ):
                raise MobileConversionError(
...
def _effective_rows(record: RolRecord, exact: dict[str, Any] | None) -> list[list[str]]:
    ...
    if "repair_position_row" in exact:
        ...
        if len(rows) == 3:
            ...
            raise MobileConversionError(
                f"mobile {record.record_id} cannot apply a position-row repair"
            )
    return rows
...
def _phrase_rule(policy: dict[str, Any], record: RolRecord) -> tuple[dict[str, Any], str] | None:
    ...
    evidence_text = " ".join(
        normalize_identity(str(strings.get(key) or "")) for key in ("aliases", "short_description")
    )
...
def _size_from_dimensions(
    height: int, weight: int, size_symbols: dict[str, int]
) -> tuple[int, str]:
    height_thresholds = (
        (6, "SIZE_FINE"),
        (12, "SIZE_DIMINUTIVE"),
...
def resolve_mobile_identity(
    ...
    if source_code not in registry:
        raise MobileConversionError(
            f"mobile {record.record_id} uses unknown source race code {source_code!r}"
        )
    ...
    if "size" in winner:
        ...
        try:
            ...
            raise MobileConversionError(
                f"mobile {record.record_id} has a malformed source race row"
            ) from error
...
def _source_actions(record: RolRecord) -> set[int]:
    ...
    try:
        ...
        raise MobileConversionError(
            f"mobile {record.record_id} has no valid source action mask"
        ) from error
...
def select_mobile_class(
    ...
    try:
        ...
        raise MobileConversionError(
            f"mobile {record.record_id} has invalid source class syntax"
        ) from error
...
def select_mobile_tier(
    ...
    try:
        ...
        raise MobileConversionError(
            f"mobile {record.record_id} has no valid source level"
        ) from error
    if source_level <= 50:
    ...
    try:
        ...
        raise MobileConversionError(
            f"mobile {record.record_id} lacks tier-classification evidence"
        ) from error
...
def select_mobile_conversion(
    ...
    try:
        ...
        raise MobileConversionError(
            f"mobile {record.record_id} has no valid authored level"
        ) from error
    effective_level = (
        int(exact["repair_level"]) if exact and "repair_level" in exact else authored_level
    )
    ...
    ignored_aggression_tokens: tuple[str, ...] = ()
    if len(race_row) > 3:
        authored_aggression = tuple(code for code in str(race_row[3]).upper().split(".") if code)
    ...
    return MobileSelection(
        ...
        custom_profile=str(exact["custom_profile"])
        if exact and exact.get("custom_profile")
        else None,
        custom_hit_points=int(exact["custom_hit_points"])
        if exact and exact.get("custom_hit_points")
        else None,
