...
def write_capability_audit_bundle(
    ...
    random_ranges = next(
        row["observations"] for row in symbolic_rows if row["family"] == "quest_random_item_range"
    )
    ...
    manifest = {
        ...
        "acceptance": {
            ...
            "all_convertible_records_emitted": (
                ...
                == len(actions)
                - disposition_counts["EXCLUDE"]
                - sum(1 for action in actions if action["source_kind"] == "soc")
            ),
            "transform_exceptions": len(exceptions),
