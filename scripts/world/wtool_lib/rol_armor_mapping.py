...
_KEYWORDS = (
    ...
    (
        "FULL_PLATE",
        ("full plate", "plate mail", "platemail", "field plate", "plate", "plates", "plated"),
    ),
    ("BANDED", ("banded",)),
    ("SPLINT", ("splint", "splinted")),
    ("PIECEMEAL", ("piecemeal", "patchwork", "mismatched", "cobbled together")),
    ("LIGHT_CHAIN", ("elven chain", "light chain", "mithril chain", "chain shirt", "fine mesh")),
    ("SCALE", ("scale", "scales", "scalemail", "lamellar", "dragonscale")),
    (
        "CHAINMAIL",
        ("chainmail", "chain mail", "ringmail", "ring mail", "hauberk", "maille", "chain", "mail"),
    ),
    ...
    (
        "PADDED",
        ("padded", "quilted", "gambeson", "aketon", "felt", "knit", "wool", "woolen", "velvet"),
    ),
    (
        "CLOTHING",
        (
...
_MATERIALS = (
    (
        "CHAINMAIL",
        ("iron", "steel", "alloy", "bronze", "brass", "metal", "adamantine", "mithril", "cyanite"),
    ),
...
def _armor_table(base: Path) -> dict[int, dict[str, int | str]]:
    ...
    for body in _call_bodies(source, "setarmor"):
        ...
        for field, term in zip(_TABLE_FIELDS, args[1:]):
            value = (
                _string_literal(term)
                if field in {"name", "description"}
                else _evaluate(term, defines)
            )
            if value is None:
...
def family_entry(family: str, slot: int) -> tuple[int, dict[str, int | str]]:
    ...
    if (
        index in {0, defines["SPEC_ARMOR_TYPE_CHAIN_HEAD"]}
        or entry is None
        or entry["wear"] != slot
    ):
...
class ArmorInference:
    ...
    def diagnostic(self) -> str:
        return (
            f"converted source armor to {self.disposition}: family {self.family or 'none'}, "
...
def infer_armor(record: RolRecord) -> ArmorInference:
    ...
    if mask & ~((1 << slot) | 1) and (override is None or override.get("slot") != slot):
        raise ValueError(
            f"mixed armor wear mask requires a reviewed disposition: {record.record_id}"
        )
    if override is not None:
        ...
        details = _text(
            _safe_string(strings.get("description"))
            + " "
            + " ".join(
                _safe_string(d.get("description"))
        ...
        if " string me " in identity or " string this " in identity:
            ...
            rules = (
                (
                    ...
                    (
                        "LARGE_SHIELD",
                        ("large shield", "heavy shield", "kite shield", "great shield"),
                    ),
