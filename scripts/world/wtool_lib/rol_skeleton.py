...
def write_skeleton_bundle(
    ...
    serialized_once = {
        relative: _serialized_payload(relative, payload) for relative, payload in payloads.items()
    }
    serialized_twice = {
        relative: _serialized_payload(relative, payload) for relative, payload in payloads.items()
    }
    ...
    determinism = {
        ...
        "staged_tree_equals_target": staged_tree["tree_sha256"] == target_before["tree_sha256"],
...
def render_rol_skeleton_human(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"Staged validation equals target: {str(summary['staged_validation_equal']).lower()}",
