...
_FLOAT_PREFIX = re.compile(r"^[ \t\v\f]*([+-]?(?:(?:\d+(?:\.\d*)?)|(?:\.\d+))(?:[eE][+-]?\d+)?)")
...
def _read_integer(
    ...
    if line is None:
        result.findings.append(
            finding(
                "SHP010", "error", f"shop ends before {label}", record.span, "shop", record.vnum
            )
        )
...
def _read_float(
    ...
    if line is None:
        result.findings.append(
            finding(
                "SHP011", "error", f"shop ends before {label}", record.span, "shop", record.vnum
            )
        )
        record.complete = False
        result.complete = False
        ...
        if line is None:
            result.findings.append(
                finding(
                    "SHP014",
                    "error",
                    "shop buy-type list ends unexpectedly",
                    record.span,
                    "shop",
                    record.vnum,
                )
        ...
        if item_type is None:
            result.findings.append(
                finding(
                    "SHP015",
                    "error",
                    f"invalid shop buy type {text!r}",
                    line.span,
                    "shop",
                    record.vnum,
                )
...
def _parse_shop_record(
    ...
    for index in range(7):
        ...
        if not message:
            result.findings.append(
                finding(
                    "SHP017", "error", f"shop message {index} is empty", span, "shop", record.vnum
                )
            )
...
def parse_shop_file(
    ...
    try:
        ...
        result.findings.append(
            finding(
                "SHP002", "error", f"cannot read shop file: {error}", SourceSpan(display_path, 1)
            )
        )
        result.complete = False
        return result
    ...
    while not cursor.eof:
        ...
        if not header.startswith("#"):
            if "v3.0" in header:
                ...
                if not result.records or match is None:
                    ...
                    parsed_restrictions = parse_c_integer_token(
                        match.group(1), signed=False, bits=64
                    )
        ...
        if match is None:
            result.findings.append(
                finding("SHP004", "error", f"invalid shop header {header!r}", span)
            )
            result.complete = False
            continue
        ...
        if parsed_vnum.error is not None or parsed_vnum.value is None:
            result.findings.append(
                finding("SHP004", "error", f"invalid shop vnum: {parsed_vnum.error}", span)
            )
            result.complete = False
            continue
        ...
        if record.vnum < 0:
            result.findings.append(
                finding(
                    "SHP006", "error", "shop vnum must be non-negative", span, "shop", record.vnum
                )
            )
        if not modern:
            result.findings.append(
    ...
    if not found_end:
        result.findings.append(
            finding(
                "SHP020",
                "error",
                "shop file is missing its '$~' terminator",
