    ...
    if _MOBILE_REPAIR_POLICY is None:
        ...
        try:
            ...
            _MOBILE_REPAIR_POLICY = {
                (
                    str(rule["basename"]),
                    int(rule["source_vnum"]),
                    str(rule["source_sha256"]),
                ): frozenset(
                    key
                    for key in (
                        "repair_race_row",
    ...
    for start, end, vnum in _segments(source):
        ...
        if not strings_ok or flags_line is None or len(flag_tokens) < 5:
            ...
            _diagnostic(
                ...
                "ROLMOB001",
                "error",
                "mobile strings or flag row are incomplete",
                source.lines[start],
                "mob",
                vnum,
            )
        ...
        record.directives.append(
            {"token": "FLAGS", "line": flags_line.number, "field_count": len(flag_tokens)}
        )
        if letter != "S":
            record.complete = False
            _diagnostic(
                ...
                "ROLMOB002",
                "error",
                f"unsupported mobile format letter {letter!r}",
                flags_line,
                "mob",
                vnum,
            )
        ...
        for token in ("RACE", "COMBAT", "MONEY", "POSITION"):
            ...
            record.directives.append(
                {"token": token, "line": line.number, "field_count": len(values)}
            )
        ...
        if extra is not None and not extra.raw.strip().startswith(b"$"):
            record.complete = False
            _diagnostic(
                corpus, "ROLMOB004", "error", "unconsumed mobile record content", extra, "mob", vnum
            )
        records.append(record)
    return records
...
MOB_SOURCE_ONLY_AFFECTS = frozenset({10, 47, 48, 53, 54, 55, 56, 57, 58, 59, 62, 63, 67, 97})
...
def emit_mobile(
    ...
    try:
        ...
        custom_profile = int(
            manifest["symbols"]["mob-autoroll-custom-profiles"]["values"][custom_profile_symbol]
        )
    ...
    source_affects = _source_mask_bits(affect_mask1, 1) | _source_mask_bits(affect_mask2, 33)
    rows = record.values.get("base_rows", [])
    race_row = rows[0] if len(rows) > 0 else ["N", "0", "0"]
    race_code = race_row[0].upper() if race_row else "N"
    ...
    missing_actions = sorted(
        source_actions - MOB_ACTION_MAP.keys() - MOB_DEFERRED_ACTIONS - MOB_SOURCE_ONLY_ACTIONS
    )
    ...
    if missing_actions:
        diagnostics.append(
            f"mobile action flags requiring behavior reconciliation: {missing_actions}"
        )
