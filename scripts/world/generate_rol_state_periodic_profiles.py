...
def _parse_switch(
    segment: str, name: str, socials: dict[str, SourceSocial | None]
) -> tuple[StateTable, int]:
    switch = re.search(r"switch\s*\(\s*dice\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)\s*\)\s*\{", segment)
    if switch is None:
    ...
    if any(
        outcome.roll < dice_count or outcome.roll > dice_count * dice_sides for outcome in outcomes
    ):
...
def _parse_profile(
    ...
    if (
        (not composed and not calls.issubset(allowed_calls))
        or "dice" not in calls
        or "act" not in calls
    ):
...
def render(source_root: Path) -> str:
    ...
    output.extend(
        [
            "};",
            "",
            "static const struct rol_state_periodic_profile rol_state_periodic_profiles[] = {",
        ]
    )
    for vnum, profile in sorted((vnum, profile) for profile in profiles for vnum in profile.vnums):
    ...
    output.extend(
        [
            "};",
            "",
            "static const struct rol_state_periodic_outcome rol_state_periodic_outcomes[] = {",
        ]
    )
    ...
    output.extend(
        [
            "};",
            "",
            "static const struct rol_source_periodic_action rol_state_periodic_actions[] = {",
        ]
    )
