...
usage()
{
  printf '%s\n' \
    ...
    "  ./scripts/vessels/run_vessel_scale_benchmark.sh cleanup" >&2
  exit 1
}
...
restore_snapshot()
{
  ...
  "$repo_root/scripts/development/dev_kohdee_login_smoke.sh" --commands "goto 1000389" \
...
run_benchmark()
{
  ...
  "$repo_root/scripts/development/dev_kohdee_login_smoke.sh" --commands "shiplist summary" \
  ...
  timeout 3600 "$repo_root/scripts/development/dev_kohdee_login_smoke.sh" \
...
SQL
  ...
  timeout 900 "$repo_root/scripts/development/dev_kohdee_login_smoke.sh" \
  ...
  timeout 90 "$repo_root/scripts/development/dev_kohdee_login_smoke.sh" \
  ...
  printf 'epoch\tpid\trss_kib\tvsz_kib\tthreads\tfile_descriptors\n' \
    >"$run_dir/process-samples.tsv"
  "$repo_root/scripts/process-memory/sample_process_memory_details.sh" --header \
