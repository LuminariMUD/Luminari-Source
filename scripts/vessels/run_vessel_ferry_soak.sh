...
run_monitor()
{
  ...
  resume_after_failure()
  {
    ...
    timeout 90 "$repo_root/scripts/development/dev_kohdee_login_smoke.sh" --commands \
      "shipgoto $ferry_slot" \
  ...
  if ! ensure_local_mud_available "$server_unit" "$server_log" \
    "$repo_root/scripts/development/dev_kohdee_login_smoke.sh" "$run_dir/server-bootstrap.log"; then
