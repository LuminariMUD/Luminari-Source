...
def container_job():
    ...
    subprocess.run(['git', '-c', 'user.name=Local CI', '-c', 'user.email=ci@example.invalid',
                    '-c', 'commit.gpgsign=false', 'commit', '-qm',
                    f"Local CI snapshot of {job['revision']}"], check=True)
    env = dict(os.environ, **job['env'])
    env.update(GITHUB_WORKSPACE='/workspace', CCACHE_DIR='/ccache', CCACHE_BASEDIR='/workspace',
               RUNNER_TEMP='/tmp', LUMINARI_LOCAL_CI='1')
    database = None
    try:
        ...
        if service:
            ...
            database = subprocess.Popen(['mariadbd', '--no-defaults', '--datadir=/tmp/mysql',
                                         '--socket=/tmp/mysql.sock', '--pid-file=/tmp/mysql.pid',
                                         '--bind-address=127.0.0.1', '--port=3306',
                                         '--log-error=/tmp/mysql.log'])
            for _ in range(100):
                ready = subprocess.run(['mariadb-admin', '--no-defaults', '--socket=/tmp/mysql.sock',
                                        '-u', 'root', 'ping'], stdout=subprocess.DEVNULL,
                                       stderr=subprocess.DEVNULL)
                if ready.returncode == 0:
                    break
                time.sleep(0.1)
            sql = (f"CREATE DATABASE {service['MARIADB_DATABASE']};"
            ...
            subprocess.run(['mariadb', '--no-defaults', '--socket=/tmp/mysql.sock', '-u', 'root'],
        ...
        for step in job['steps']:
            ...
            command = '\n'.join(line for line in step['run'].splitlines()
                                if not line.strip().startswith(('sudo apt-get ',
...
def main():
    ...
    for workflow_name in WORKFLOWS:
        content = subprocess.check_output(['git', 'show',
                                          f'{revision}:.github/workflows/{workflow_name}.yml'], cwd=root)
