...
test_watchdog_pid_verification()
{
  ...
  cp "$project_root/autorun-watchdog.sh" "$mismatch_dir/autorun-watchdog.sh"
...
test_systemd_unit_installation()
{
  ...
  mkdir -p "$deploy_dir/scripts" "$fake_bin" "$deploy_dir/installed"
  cp "$project_root/scripts/deploy.sh" "$deploy_dir/scripts/deploy.sh"
  cp "$project_root/autorun.sh" "$deploy_dir/autorun.sh"
  cp "$project_root/luminari.service" "$deploy_dir/luminari.service"
  chmod +x "$deploy_dir/autorun.sh"
...
EOF
  ...
  PATH="$fake_bin:$PATH" \
    ...
    "$deploy_dir/scripts/deploy.sh" --install-systemd --restart-service \
  ...
  grep -Fxq "ExecStart=$deploy_dir/autorun.sh" "$installed_unit" ||
    fail "installed systemd unit does not start autorun"
  grep -Fxq "ExecStop=$deploy_dir/autorun.sh stop" "$installed_unit" ||
