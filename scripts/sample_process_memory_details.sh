...
usage()
{
  printf '%s\n' \
    "Usage:" \
    "  ./scripts/sample_process_memory_details.sh --header" \
    "  ./scripts/sample_process_memory_details.sh --sample <pid> <label>" \
    "  ./scripts/sample_process_memory_details.sh --validate <samples.tsv>" >&2
  exit 1
}
