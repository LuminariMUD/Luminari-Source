...
ACMD(do_oasis_list) {
  ...
  switch (subcmd) {
    ...
    case SCMD_OASIS_OLIST:
      ...
      break;
    case SCMD_OASIS_RLIST: list_rooms(ch, rzone, vmin, vmax);
      break;
    case SCMD_OASIS_TLIST: list_triggers(ch, rzone, vmin, vmax);
      break;
    case SCMD_OASIS_SLIST: list_shops(ch, rzone, vmin, vmax);
      break;
    case SCMD_OASIS_QLIST: list_quests(ch, rzone, vmin, vmax);
      break;
...
static void list_regions(struct char_data *ch) {
  ...
  for (i = 0; i <= top_of_region_table; i++) {
    ...
    len += snprintf(buf + len, sizeof (buf) - len,
              "%s%3d%s|%s%-7d%s|%s%-37s%s|%s%12s%s|%s%5d%s\r\n",
              QGRN, counter, QNRM,
              ...
              QYEL, (region_table[i].region_type == 4 ? wild_map_info[region_table[i].region_props].disp : "[N/A]"), QNRM
              );
  ...
  for (i = 0; i <= top_of_mobt; i++) {
    if (mob_index[i].vnum >= bottom && mob_index[i].vnum <= top) {
      ...
      len += snprintf(buf + len, sizeof (buf) - len,
              ...
              IS_SET_AR(mob_proto[i].char_specials.saved.act, MOB_NOCLASS) ? "---" : class_abbrevs[mob_proto[i].player.chclass],
