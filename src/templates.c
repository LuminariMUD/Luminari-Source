...
void gain_template_level(struct char_data *ch, int t_type, int level)
{
  ...
  snprintf(query, sizeof(query),
           "SELECT level_id, class_number FROM player_levelups WHERE character_name='%s' AND "
           "level_number='%d'",
  ...
  snprintf(query, sizeof(query),
           "SELECT feat_num, subfeat FROM player_levelup_feats WHERE level_id='%ld'", level_id);
  if (mysql_query(conn, query))
    log("%s", query);
  res = mysql_store_result(conn);
  if (res != NULL)
  ...
  send_to_char(ch,
               ...
               class_names[LEVELUP(ch)->class], template_types[GET_TEMPLATE(ch)]);
...
SPECIAL(select_templates)
{
  ...
  if (CMD_IS("north"))
  {
    ...
    {
      send_to_char(ch, "You cannot proceed without choosing a template. If you wish to do a custom "
                       "build, type recall.\r\n");
      return true;
    }
  ...
  {
    send_to_char(ch, "That is not an available template. Please type @Ylisttemplates@n to see "
                     "which templates are available, and select again.\r\n");
    return true;
  }
...
void show_level_history(struct char_data *ch, int level)
{
  ...
  snprintf(query, sizeof(query),
           "SELECT * FROM player_levels WHERE char_name=\"%s\" AND char_level='%d'", GET_NAME(ch),
           level);
  if (mysql_query(conn, query))
    log("%s", query);
  res = mysql_store_result(conn);
...
ACMD(do_templates)
{
  ...
  if (*argument)
  {
    ...
    {
      send_to_char(ch,
                   "That is not a known template type.  Please type @Y'templates'@n for a list.\r\n"
  ...
  send_to_char(
      ch, "The following templates are currently available:\r\n"
          "\r\n"
...
long get_level_id_by_level_num(int level_num, char *chname)
{
  ...
  snprintf(query, sizeof(query),
           "SELECT level_id from player_levelups WHERE character_name='%s' AND level_number='%d'",
           escaped_chname, level_num);
  free(escaped_chname);
...
void show_levelinfo_for_specific_level(struct char_data *ch, long level_id, char *chname)
{
  ...
  snprintf(query, sizeof(query),
           "SELECT level_number, class_number FROM player_levelups WHERE level_id='%ld' AND "
           "character_name='%s'",
           level_id, chname);
  ...
  {
    send_to_char(ch,
                 "We're sorry, but there is no level information for your character %s with "
                 "level_id %d.\r\nQ: %s\r\n",
  ...
  send_to_char(ch,
               "Level Information for '%s', for level '%d', having taken class '%s' and with "
               "level_id '%ld'\r\n",
               display_name, level_num, class_names[class_number], level_id);
  send_to_char(
      ch, "--------------------------------------------------------------------------------\r\n");
  ...
  snprintf(query, sizeof(query), "SELECT skill_num FROM player_levelup_skills WHERE level_id='%ld'",
           level_id);
  mysql_query(conn, query);
  res = mysql_store_result(conn);
  if (res != NULL)
...
ACMD(do_levelinfo)
{
  ...
  if (!*arg1)
  {
    ...
    send_to_char(
        ch, "--------------------------------------------------------------------------------\r\n");
    send_to_char(ch, "%-20s %-14s %s\r\n", "Level ID", "Level Number", "Class Taken");
    send_to_char(
        ch, "--------------------------------------------------------------------------------\r\n");

    snprintf(query, sizeof(query),
             "SELECT level_id, level_number, class_number FROM player_levelups WHERE "
             "character_name='%s' ORDER BY level_number DESC",
             GET_NAME(ch));
    mysql_query(conn, query);
    res = mysql_store_result(conn);
    if (res != NULL)
  ...
  else
  {
    if (is_abbrev(arg1, "search"))
    {
      ...
      {
        send_to_char(
            ch, "You need to specify whether you are searching for a feat, skill (or language), or "
                "an ability score,\r\n"
      ...
      if (!*arg3)
      {
        send_to_char(ch, "You need to specify whether you are searching for a feat, skill, "
                         "language, or an ability score.\r\n");
        send_to_char(ch, "\r\n%s\r\n", LEVELINFO_SYNTAX);
        // mysql_close(conn);  // REMOVED - don't close global connection
        return;
      }
      if (!*arg4)
      {
        send_to_char(ch, "You need to specify which feat/skill/language/ability score you are "
                         "searching for.\r\n");
        send_to_char(ch, "\r\n%s\r\n", LEVELINFO_SYNTAX);
        // mysql_close(conn);  // REMOVED - don't close global connection
        return;
      ...
      {
        send_to_char(ch, "You need to specify whether you are searching for a feat, skill (or "
                         "language), or an ability score.\r\n");
        send_to_char(ch, "\r\n%s\r\n", LEVELINFO_SYNTAX);
        // mysql_close(conn);  // REMOVED - don't close global connection
        return;
...
void display_levelinfo_feats(struct char_data *ch, int feat_num, int subfeat)
{
  ...
  case FEAT_WEAPON_FLURRY:
  case FEAT_WEAPON_SUPREMACY:
    send_to_char(ch, "%-30s (%s)", feat_list[feat_num].name, weapon_damage_types[subfeat]);
    break;
  case FEAT_SKILL_FOCUS:
  case FEAT_EPIC_SKILL_FOCUS:
    send_to_char(ch, "%-30s (%s)", feat_list[feat_num].name, spell_info[subfeat].name);
    break;
  default:
...
void levelinfo_search(struct char_data *ch, int type, char *searchString)
{
  ...
  { // feats
    ...
    if (i >= NUM_FEATS)
    {
      send_to_char(ch, "We are sorry, but there is no feat by that name. Please try again.\r\n");
      // mysql_close(conn);  // REMOVED - don't close global connection
      return;
    }

    send_to_char(ch, "@5@oLEVELUPS WHEREIN THE FOLLOWING FEAT WAS SELECTED: %s@n\r\n",
                 feat_list[i].name);

    snprintf(query, sizeof(query),
             "SELECT a.feat_num, a.subfeat, b.level_id, b.level_number FROM player_levelup_feats a "
             "LEFT JOIN player_levelups b ON a.level_id=b.level_id WHERE b.character_name='%s' AND "
             "a.feat_num='%d' ORDER BY b.level_number DESC",
             GET_NAME(ch), i);
    // log("%s", query);
    mysql_query(conn, query);
    res = mysql_store_result(conn);
  ...
  { // skills and languages
    ...
    {
      send_to_char(ch, "We are sorry, but there is no skill by that name. Please try again.\r\n");
      // mysql_close(conn);  // REMOVED - don't close global connection
      return;
    }

    send_to_char(ch, "@5@oLEVELUPS WHEREIN THE FOLLOWING SKILL WAS SELECTED: %s@n\r\n",
                 spell_info[i].name);

    snprintf(query, sizeof(query),
             "SELECT a.skill_num, a.skill_ranks, b.level_id, b.level_number FROM "
             "player_levelup_skills a LEFT JOIN player_levelups b ON a.level_id=b.level_id WHERE "
             "b.character_name='%s' AND a.skill_num='%d' ORDER BY b.level_number DESC",
             GET_NAME(ch), i);
    // log("%s", query);
    mysql_query(conn, query);
    res = mysql_store_result(conn);
  ...
  { // languages
    ...
    {
      send_to_char(ch, "Were sorry, but there is no language by that name. Please try again.\r\n");
      // mysql_close(conn);  // REMOVED - don't close global connection
      return;
    }

    send_to_char(ch, "@5@oLEVELUPS WHEREIN THE FOLLOWING LANGUAGE WAS SELECTED: %s@n\r\n",
                 spell_info[i].name);
  ...
  { // ability scores
    ...
    if (i < 0 || i > 6)
    {
      send_to_char(ch, "We are sorry, but there is no ability score by that name.\r\n");
      // mysql_close(conn);  // REMOVED - don't close global connection
      return;
    }

    send_to_char(ch, "@5@oLEVELUPS WHEREIN THE FOLLOWING ABILITY SCORE WAS RAISED: %s@n\r\n",
                 levelup_ability_scores[i]);

    snprintf(query, sizeof(query),
             "SELECT a.ability_score, b.level_id, b.level_number FROM "
             "player_levelup_ability_scores a LEFT JOIN player_levelups b ON a.level_id=b.level_id "
             "WHERE b.character_name='%s' AND a.ability_score='%d' ORDER BY b.level_number DESC",
             GET_NAME(ch), i);
    // log("%s", query);
    mysql_query(conn, query);
    res = mysql_store_result(conn);
...
void erase_levelup_info(struct char_data *ch)
{
  ...
  while (found)
  {
    ...
    if (level_id > 0)
    {
      snprintf(query, sizeof(query), "DELETE FROM player_levelups WHERE level_id='%lld'", level_id);
      //      log("%s", query);
      mysql_query(conn, query);
      snprintf(query, sizeof(query), "DELETE FROM player_levelup_skills WHERE level_id='%lld'",
               level_id);
      //      log("%s", query);
      mysql_query(conn, query);
      snprintf(query, sizeof(query), "DELETE FROM player_levelup_feats WHERE level_id='%lld'",
               level_id);
      //      log("%s", query);
      mysql_query(conn, query);
      snprintf(query, sizeof(query),
               "DELETE FROM player_levelup_ability_scores WHERE level_id='%lld'", level_id);
      //      log("%s", query);
      mysql_query(conn, query);
    }
  }
...
const char *const levelup_ability_scores[6] = {"strength",     "dexterity", "constitution",
