...
static struct help_entry_list *search_help_table(const char *argument, int level)
{
  ...
  for (i = 0; i < top_of_helpt; i++)
  {
    ...
    if (strn_cmp(argument, help_table[i].keywords, strlen(argument)) == 0)
    {
      ...
      new_entry->entry =
          strdup(help_table[i].entry ? help_table[i].entry : "No help available.\r\n");
...
struct help_entry_list *search_help(const char *argument, int level)
{
  ...
  {
    if (HELP_DEBUG)
      log("DEBUG: search_help: Found '%s' in cache", argument);
    return cached_result; /* Return deep copy from cache */
  }

  if (HELP_DEBUG)
    log("DEBUG: search_help: Searching database for '%s' (level %d)", argument, level);
  ...
  if (!conn)
  {
    ...
    if (HELP_DEBUG)
      log("DEBUG: search_help: ERROR - No database connection!");
  ...
  {
    log("SYSERR: %s: Database connection lost while searching for '%s', using file-based help",
        __func__, argument);
    if (HELP_DEBUG)
      log("DEBUG: search_help: MySQL ping failed, connection lost");
    return search_help_table(argument, level);
  }
  if (HELP_DEBUG)
    log("DEBUG: search_help: Database connection OK");

  /* Create prepared statement for secure query execution */
  pstmt = mysql_stmt_create(conn);
  if (!pstmt)
  {
    log("SYSERR: Failed to create prepared statement for help search '%s': %s", argument,
        mysql_error(conn));
    if (HELP_DEBUG)
      log("DEBUG: search_help: Failed to create prepared statement, falling back to file-based");
    return search_help_table(argument, level);
  }
  if (HELP_DEBUG)
                                ...
                                "ORDER BY LENGTH(hk.keyword) ASC"))
  {
    ...
    if (HELP_DEBUG)
      log("DEBUG: search_help: Failed to prepare query, falling back to file-based");
    mysql_stmt_cleanup(pstmt);
    return search_help_table(argument, level);
  }
  if (HELP_DEBUG)
    log("DEBUG: search_help: Query prepared successfully");
  ...
  if (HELP_DEBUG)
    log("DEBUG: search_help: Search pattern is '%s'", search_pattern);
      ...
      !mysql_stmt_bind_param_int(pstmt, 1, level))
  {
    ...
    if (HELP_DEBUG)
      log("DEBUG: search_help: Failed to bind parameters, falling back to file-based");
    mysql_stmt_cleanup(pstmt);
    return search_help_table(argument, level);
  }
  if (HELP_DEBUG)
    log("DEBUG: search_help: Parameters bound (pattern='%s', level=%d)", search_pattern, level);

  /* Execute the prepared statement */
  if (!mysql_stmt_execute_prepared(pstmt))
  {
    log("SYSERR: Failed to execute help search query for '%s': %s", argument, mysql_error(conn));
    if (HELP_DEBUG)
      log("DEBUG: search_help: Query execution failed, falling back to file-based");
    mysql_stmt_cleanup(pstmt);
    return search_help_table(argument, level);
  }
  if (HELP_DEBUG)
    log("DEBUG: search_help: Query executed successfully");

  /* Fetch results row by row */
  while (mysql_stmt_fetch_row(pstmt))
  {
    row_count++;
    if (HELP_DEBUG && row_count == 1)
      log("DEBUG: search_help: Found database results for '%s'", argument);
    ...
    new_help_entry->tag =
        strdup(mysql_stmt_get_string(pstmt, 0) ? mysql_stmt_get_string(pstmt, 0) : "");
    new_help_entry->entry =
        strdup(mysql_stmt_get_string(pstmt, 1) ? mysql_stmt_get_string(pstmt, 1) : "");
    new_help_entry->min_level = mysql_stmt_get_int(pstmt, 2);
    new_help_entry->last_updated =
        strdup(mysql_stmt_get_string(pstmt, 3) ? mysql_stmt_get_string(pstmt, 3) : "");
    new_help_entry->keywords =
        strdup(mysql_stmt_get_string(pstmt, 4) ? mysql_stmt_get_string(pstmt, 4) : "");

    /* Check for allocation failures - use centralized cleanup */
    if (!new_help_entry->tag || !new_help_entry->entry || !new_help_entry->last_updated ||
        !new_help_entry->keywords)
    {
  ...
  if (HELP_DEBUG)
    log("DEBUG: search_help: Query returned %d rows for '%s'", row_count, argument);

  /* If database returned no results, try file-based help as fallback */
  if (help_entries == NULL)
  {
    if (HELP_DEBUG)
      log("DEBUG: search_help: No database results for '%s', trying file-based", argument);
    help_entries = search_help_table(argument, level);
    if (help_entries)
    {
      if (HELP_DEBUG)
        log("DEBUG: search_help: Found file-based help for '%s'", argument);
    }
    else
    {
      if (HELP_DEBUG)
        log("DEBUG: search_help: No file-based help for '%s' either", argument);
    }
  ...
  {
    if (HELP_DEBUG)
      log("DEBUG: search_help: Found database help for '%s'", argument);
    ...
    if (conn && mysql_available)
    {
      ...
      for (tmp = help_entries; tmp; tmp = tmp->next)
        count++;

      char query[MAX_STRING_LENGTH];
      snprintf(query, sizeof(query),
               "INSERT INTO help_search_history (search_term, searcher_level, results_count, "
               "search_type) "
...
struct help_entry_list *search_help_fulltext(const char *search_term, int level)
{
  ...
  if (HELP_DEBUG)
    log("DEBUG: search_help_fulltext: Searching for '%s' in help content (level %d)", search_term,
        level);

  /* Check the connection */
  if (!conn)
  {
    log("SYSERR: search_help_fulltext: No database connection available for fulltext search '%s'",
        search_term);
  ...
  if (!mysql_stmt_prepare_query(pstmt, "SELECT DISTINCT he.tag, he.entry, he.min_level, "
                                       "he.last_updated, "
                                       "GROUP_CONCAT(DISTINCT CONCAT(UCASE(LEFT(hk.keyword, 1)), "
                                       "LCASE(SUBSTRING(hk.keyword, 2))) SEPARATOR ', ') "
                                       "FROM help_entries he "
                                       "LEFT JOIN help_keywords hk ON he.tag = hk.help_tag "
                                       "WHERE (LOWER(he.entry) LIKE LOWER(?) OR LOWER(hk.keyword) "
                                       "LIKE LOWER(?)) "
                                       "AND he.min_level <= ? "
                                       "GROUP BY he.tag, he.entry, he.min_level, he.last_updated "
                                       "ORDER BY "
                                       "  CASE WHEN LOWER(hk.keyword) LIKE LOWER(?) THEN 0 ELSE 1 "
                                       "END, "                   /* Keyword matches first */
                                       "  LENGTH(he.entry) ASC " /* Shorter entries first */
                                       "LIMIT 20"))
  { /* Limit results to prevent overwhelming output */
    log("SYSERR: Failed to prepare fulltext help search query for '%s': %s", search_term,
        mysql_error(conn));
    mysql_stmt_cleanup(pstmt);
    return NULL;
  }
  ...
  {
    log("SYSERR: Failed to bind parameters for fulltext help search '%s': %s", search_term,
        mysql_error(conn));
    mysql_stmt_cleanup(pstmt);
    return NULL;
  }
  ...
  if (!mysql_stmt_execute_prepared(pstmt))
  {
    log("SYSERR: Failed to execute fulltext help search for '%s': %s", search_term,
        mysql_error(conn));
    mysql_stmt_cleanup(pstmt);
    return NULL;
  }
  ...
  if (HELP_DEBUG)
    log("DEBUG: search_help_fulltext: Found %d results for '%s'", row_count, search_term);
...
struct help_keyword_list *get_help_keywords(const char *tag)
{
  ...
  if (!mysql_stmt_prepare_query(
          pstmt, "SELECT help_tag, CONCAT(UCASE(LEFT(keyword, 1)), LCASE(SUBSTRING(keyword, 2))) "
  ...
  while (mysql_stmt_fetch_row(pstmt))
  {
    ...
    if (!new_keyword->tag || !new_keyword->keyword)
    {
      ...
      if (new_keyword->tag)
      {
        ...
        new_keyword->tag = NULL;
      }
      if (new_keyword->keyword)
      {
        free(new_keyword->keyword);
        new_keyword->keyword = NULL;
      }
      free(new_keyword);
      continue;
    }
...
struct help_keyword_list *soundex_search_help_keywords(const char *argument, int level)
{
  ...
  if (HELP_DEBUG)
    log("DEBUG: soundex_search_help_keywords: Searching for suggestions for '%s' (level %d)",
        argument, level);
  ...
  {
    if (HELP_DEBUG)
      log("DEBUG: soundex_search_help_keywords: ERROR - No database connection!");
    return NULL;
  }
  ...
  {
    if (HELP_DEBUG)
      log("DEBUG: %s: MySQL ping failed, connection lost", __func__);
    return NULL;
  }
  if (HELP_DEBUG)
    log("DEBUG: soundex_search_help_keywords: Database connection OK");
  ...
  if (!pstmt)
  {
    ...
    if (HELP_DEBUG)
      log("DEBUG: soundex_search_help_keywords: Failed to create prepared statement");
    return NULL;
  }
  if (HELP_DEBUG)
    log("DEBUG: soundex_search_help_keywords: Created prepared statement");
  ...
  { /* Limit to 10 suggestions */
    ...
    if (HELP_DEBUG)
      log("DEBUG: soundex_search_help_keywords: Failed to prepare query");
    mysql_stmt_cleanup(pstmt);
    return NULL;
  }
  if (HELP_DEBUG)
    log("DEBUG: soundex_search_help_keywords: Query prepared successfully");
  ...
  if (!mysql_stmt_bind_param_string(pstmt, 0, argument) || /* For exact match case */
      ...
      !mysql_stmt_bind_param_string(pstmt, 8, argument))
  { /* For contains in WHERE */
    log("SYSERR: Failed to bind parameters for soundex search");
    if (HELP_DEBUG)
      log("DEBUG: soundex_search_help_keywords: Failed to bind parameters (argument='%s', "
          "level=%d)",
          argument, level);
    mysql_stmt_cleanup(pstmt);
    return NULL;
  ...
  if (HELP_DEBUG)
    log("DEBUG: soundex_search_help_keywords: All parameters bound (argument='%s', level=%d)",
        argument, level);
  ...
  if (!mysql_stmt_execute_prepared(pstmt))
  {
    ...
    if (HELP_DEBUG)
      log("DEBUG: soundex_search_help_keywords: Failed to execute query");
    mysql_stmt_cleanup(pstmt);
    return NULL;
  }
  if (HELP_DEBUG)
    log("DEBUG: soundex_search_help_keywords: Query executed successfully");
  ...
  while (mysql_stmt_fetch_row(pstmt))
  {
    row_count++;
    if (HELP_DEBUG && row_count == 1)
      log("DEBUG: soundex_search_help_keywords: Found soundex matches for '%s'", argument);
  ...
  if (HELP_DEBUG)
    log("DEBUG: soundex_search_help_keywords: Found %d soundex matches for '%s'", row_count,
        argument);
...
void perform_help(struct descriptor_data *d, const char *argument)
{
  ...
  if ((STATE(d) == CON_QRACE) || (STATE(d) == CON_QCLASS))
                         ...
                         struct help_context *ctx)
{
  ...
  if ((entries = search_help(argument, GET_LEVEL(ch))) == NULL)
  {
    ...
    if (HELP_DEBUG)
      log("DEBUG: handle_database_help found no entry for '%s'", argument);
    return 0; /* Not found, let next handler try */
  }

  /* Check if any of the keywords is an exact match (case-insensitive) */
  if (HELP_DEBUG)
    log("DEBUG: handle_database_help: Checking for exact match among keywords");
  for (tmp_keyword = entries->keyword_list; tmp_keyword; tmp_keyword = tmp_keyword->next)
  {
        ...
        strcasecmp(tmp_keyword->keyword, raw_argument) == 0)
    {
      exact_match_found = 1;
      if (HELP_DEBUG)
        log("DEBUG: handle_database_help: EXACT MATCH found for '%s' -> '%s'", argument,
            tmp_keyword->keyword);
      break;
    }
  }
  ...
  if (!exact_match_found && entries->keyword_list == NULL)
  {
        ...
        (entries->tag && strcasecmp(entries->tag, raw_argument) == 0))
    {
      exact_match_found = 1;
      if (HELP_DEBUG)
        log("DEBUG: handle_database_help: EXACT MATCH via tag '%s'", entries->tag);
    }
    else if (entries->keywords && *entries->keywords)
    {
      ...
      for (tok = strtok_r(kw_buf, ",", &saveptr); tok; tok = strtok_r(NULL, ",", &saveptr))
      {
        ...
        if (strcasecmp(tok, argument) == 0 || strcasecmp(tok, raw_argument) == 0)
        {
          exact_match_found = 1;
          if (HELP_DEBUG)
            log("DEBUG: handle_database_help: EXACT MATCH via keywords string '%s'", tok);
          break;
        }
      }
    }
  ...
  if (!exact_match_found)
  {
    ...
    if (strlen(argument) <= MIN_SEARCH_LENGTH_FOR_STRICT)
    {
      ...
      {
        if (HELP_DEBUG)
          log("DEBUG: Weak partial match for short search '%s', continuing to soundex", argument);
        ...
        while (entries != NULL)
        {
          ...
          if (tmp->keywords)
            free(tmp->keywords);
          if (tmp->last_updated)
            free(tmp->last_updated);
          while (tmp->keyword_list != NULL)
          {
            tmp_keyword = tmp->keyword_list->next;
            if (tmp->keyword_list->tag)
              free(tmp->keyword_list->tag);
            if (tmp->keyword_list->keyword)
              free(tmp->keyword_list->keyword);
    ...
    if (HELP_DEBUG)
      log("DEBUG: Only partial match found for '%s', will show help but continue to soundex",
          argument);
  }
  ...
  snprintf(immo_data_buffer, sizeof(immo_data_buffer), "\tcHelp Tag      : \tn%s\r\n",
           entries->tag);
  snprintf(help_entry_buffer, sizeof(help_entry_buffer),
           "\tC%s\tn"
           "%s"
           "\tcHelp Keywords : \tn%s\r\n"
           "\tcLast Updated  : \tn%s\r\n"
           ...
           "\tn%s\r\n"
           "\tC%s",
           line_string(GET_SCREEN_WIDTH(ch), '-', '-'), (IS_IMMORTAL(ch) ? immo_data_buffer : ""),
           entries->keywords, entries->last_updated, line_string(GET_SCREEN_WIDTH(ch), '-', '-'),
           entries->entry, line_string(GET_SCREEN_WIDTH(ch), '-', '-'));
  ...
  {
    if (HELP_DEBUG)
      log("DEBUG: Exact match handled, stopping chain for '%s'", argument);
 ...
 */
int handle_grand_discovery_help(struct char_data *ch, const char *argument,
 ...
 */
int handle_discovery_types_help(struct char_data *ch, const char *argument,
                                const char *raw_argument, struct help_context *ctx)
{
  /* Alchemy functions require non-const char*, so we make a copy */
  char *arg_copy = strdup(raw_argument);
...
ACMDU(do_help)
{
  ...
  for (handler = help_handler_chain; handler; handler = handler->next)
  {
    ...
    if (HELP_DEBUG)
      log("DEBUG: do_help: Trying handler '%s' for '%s' (raw: '%s')", handler->name, argument_copy,
          raw_argument);
    if (handler->handler(ch, argument_copy, raw_argument, &ctx))
    {
      /* Handler processed the request successfully */
      if (HELP_DEBUG)
        log("DEBUG: do_help: Handler '%s' HANDLED request for '%s' - stopping chain", handler->name,
            argument_copy);
      free(raw_argument);
      return;
    }
    if (HELP_DEBUG)
      log("DEBUG: do_help: Handler '%s' DID NOT HANDLE '%s' - continuing chain", handler->name,
          argument_copy);
  }
