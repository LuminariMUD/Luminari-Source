...
void mysql_pool_init(void)
{
  ...
  for (i = 0; i < MYSQL_POOL_MIN_SIZE; i++)
  {
    ...
    /* Connect to database */
    if (!mysql_real_connect(pc->conn, mysql_pool->host, mysql_pool->username, mysql_pool->password,
                            mysql_pool->database, 0, NULL, 0))
    {
      log("ERROR: Failed to connect MySQL connection %d: %s", i, mysql_error(pc->conn));
      mysql_close(pc->conn);
      free(pc);
      continue;
    }
  ...
  if (mysql_pool->current_size > 0)
  {
    ...
    log("Success: MySQL connection pool initialized with %d connections", mysql_pool->current_size);
  }
  else
  {
  ...
  while (1)
  {
    ...
    for (pc = mysql_pool->connections; pc; pc = pc->next)
    {
      if (pc->state == CONN_STATE_FREE)
      {
        ...
        if (MYSQL_DEBUG)
        {
          log("DEBUG: Acquired connection %d from pool (active: %d/%d)", pc->id,
              mysql_pool->active_count, mysql_pool->current_size);
        }
...
void mysql_pool_release(MYSQL_POOL_CONN *pc)
{
  ...
  if (MYSQL_DEBUG)
  {
    log("DEBUG: Released connection %d to pool (active: %d/%d)", pc->id, mysql_pool->active_count,
        mysql_pool->current_size);
  }
...
void mysql_pool_expand(void)
{
  ...
  if (!mysql_real_connect(pc->conn, mysql_pool->host, mysql_pool->username, mysql_pool->password,
                          mysql_pool->database, 0, NULL, 0))
  {
    log("ERROR: Failed to connect new connection %d: %s", new_id, mysql_error(pc->conn));
    mysql_close(pc->conn);
    free(pc);
    pthread_mutex_unlock(&mysql_pool->pool_mutex);
    return;
...
void mysql_pool_stats(char *buf, size_t size)
{
  ...
  snprintf(buf, size,
           ...
           "  Error Count: %lu\n"
           "  Uptime: %ld seconds",
           mysql_pool->current_size, mysql_pool->active_count, mysql_pool->total_requests,
           mysql_pool->wait_count, mysql_pool->error_count,
...
void connect_to_mysql()
{
  ...
  if (mysql_pool && mysql_pool->initialized)
  {
    ...
    log("Success: Connected to MySQL database '%s' on host '%s' as user '%s'", mysql_pool->database,
        mysql_pool->host, mysql_pool->username);
...
bool mysql_stmt_execute_prepared(PREPARED_STMT *pstmt)
{
  ...
  if (pstmt->param_count > 0 && pstmt->params)
  {
    ...
    {
      log("SYSERR: mysql_stmt_bind_param failed: %s (Error: %u)", mysql_stmt_error(pstmt->stmt),
          mysql_stmt_errno(pstmt->stmt));
      MYSQL_UNLOCK(*mutex);
      return FALSE;
    }
  ...
  if (pstmt->metadata && pstmt->result_count > 0)
  {
    ...
    {
      log("SYSERR: mysql_stmt_bind_result failed: %s (Error: %u)", mysql_stmt_error(pstmt->stmt),
          mysql_stmt_errno(pstmt->stmt));
...
bool mysql_stmt_fetch_row(PREPARED_STMT *pstmt)
{
  ...
  if (MYSQL_DEBUG)
  {
    ...
    {
      int i;
      log("DEBUG: mysql_stmt_fetch returned MYSQL_DATA_TRUNCATED - checking which columns "
          "truncated");
      ...
      for (i = 0; i < pstmt->result_count; i++)
      {
        ...
        {
          log("DEBUG:   Column %d was truncated (buffer_length=%lu, actual_length=%lu)", i,
              pstmt->results[i].buffer_length,
    ...
    {
      log("DEBUG: mysql_stmt_fetch returned error %d: %s", result, mysql_stmt_error(pstmt->stmt));
    }
  }
  MYSQL_UNLOCK(*mutex);
...
struct region_proximity_list *get_nearby_regions(zone_rnum zone, int x, int y, int r)
{
  ...
  snprintf(
      ...
      "         WHEN ST_Distance(ri.region_polygon, ST_GeomFromText('POINT(%d %d)')) = 0 THEN "
      "'EDGE' "
      "         ELSE 'NEARBY' "
      "       END AS position, "
      "       ST_Distance(ri.region_polygon, ST_GeomFromText('POINT(%d %d)')) AS distance, "
      "       CASE WHEN ST_Intersects(ri.region_polygon, ST_GeomFromText('POLYGON((%d %d, %d %d, "
      "%d %d, %d %d, %d %d))')) THEN 1 ELSE 0 END AS n, "
      "       CASE WHEN ST_Intersects(ri.region_polygon, ST_GeomFromText('POLYGON((%d %d, %d %d, "
      "%d %d, %d %d, %d %d))')) THEN 1 ELSE 0 END AS ne, "
      "       CASE WHEN ST_Intersects(ri.region_polygon, ST_GeomFromText('POLYGON((%d %d, %d %d, "
      "%d %d, %d %d, %d %d))')) THEN 1 ELSE 0 END AS e, "
      "       CASE WHEN ST_Intersects(ri.region_polygon, ST_GeomFromText('POLYGON((%d %d, %d %d, "
      "%d %d, %d %d, %d %d))')) THEN 1 ELSE 0 END AS se, "
      "       CASE WHEN ST_Intersects(ri.region_polygon, ST_GeomFromText('POLYGON((%d %d, %d %d, "
      "%d %d, %d %d, %d %d))')) THEN 1 ELSE 0 END AS s, "
      "       CASE WHEN ST_Intersects(ri.region_polygon, ST_GeomFromText('POLYGON((%d %d, %d %d, "
      "%d %d, %d %d, %d %d))')) THEN 1 ELSE 0 END AS sw, "
      "       CASE WHEN ST_Intersects(ri.region_polygon, ST_GeomFromText('POLYGON((%d %d, %d %d, "
      "%d %d, %d %d, %d %d))')) THEN 1 ELSE 0 END AS w, "
      "       CASE WHEN ST_Intersects(ri.region_polygon, ST_GeomFromText('POLYGON((%d %d, %d %d, "
      "%d %d, %d %d, %d %d))')) THEN 1 ELSE 0 END AS nw "
...
void who_to_mysql()
{
  ...
  for (d = descriptor_list; d; d = d->next)
  {
    ...
    {
      snprintf(buf, sizeof(buf),
               "INSERT INTO who (player, title, killer, thief) VALUES ('%s', '%s', %d, %d)",
               escaped_name, buf2, PLR_FLAGGED(tch, PLR_KILLER) ? 1 : 0,
               PLR_FLAGGED(tch, PLR_THIEF) ? 1 : 0);
    }
    else
    {
      snprintf(
          buf, sizeof(buf),
          "INSERT INTO who (player, level, title, killer, thief) VALUES ('%s', %d, '%s', %d, %d)",
          escaped_name, GET_LEVEL(tch), buf2, PLR_FLAGGED(tch, PLR_KILLER) ? 1 : 0,
          PLR_FLAGGED(tch, PLR_THIEF) ? 1 : 0);
    }
    free(escaped_name);
...
void read_factions_from_mysql()
{
  ...
  if (mysql_query(conn,
                  "SELECT f.id, f.name, f.flags, f.gold, f.tax, COUNT(r.rank) FROM factions AS f "
                  "LEFT JOIN faction_ranks AS r ON f.id = r.faction GROUP BY f.id ORDER BY f.name"))
  {
  ...
  if (mysql_query(conn,
                  "SELECT faction_id, skillgroup_id FROM faction_skillgroups ORDER BY faction_id"))
  {
