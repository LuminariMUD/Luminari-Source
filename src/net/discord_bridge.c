...
char *build_discord_json(const char *channel, const char *name, const char *message, int emoted)
{
  ...
  snprintf(json_buf, DISCORD_JSON_BUFFER_SIZE,
           "{\"channel\":\"%s\",\"name\":\"%s\",\"message\":\"%s\",\"emoted\":%d}", escaped_channel,
           escaped_name, escaped_message, emoted);
...
void init_discord_bridge(void)
{
  ...
  if (discord_bridge)
  {
    log("WARNING: Discord bridge already initialized (ptr=%p), reinitializing for copyover",
        discord_bridge);
...
void shutdown_discord_bridge(void)
{
  ...
  log("Info: Shutting down Discord bridge (ptr=%p, server_socket=%d)", discord_bridge,
      discord_bridge->server_socket);
...
int start_discord_server(int port)
{
  ...
  if (discord_bridge->server_socket == INVALID_SOCKET)
  {
    log("SYSERR: Discord bridge failed to create socket: %s (errno=%d)", strerror(errno), errno);
    return 0;
  }
  ...
  if (setsockopt(discord_bridge->server_socket, SOL_SOCKET, SO_REUSEADDR, (char *)&opt,
                 sizeof(opt)) < 0)
  {
    log("WARNING: Discord bridge setsockopt SO_REUSEADDR failed: %s (errno=%d)", strerror(errno),
        errno);
  }
  else
  {
...
#ifdef SO_REUSEPORT
  if (setsockopt(discord_bridge->server_socket, SOL_SOCKET, SO_REUSEPORT, (char *)&opt,
                 sizeof(opt)) < 0)
  {
    log("WARNING: Discord bridge setsockopt SO_REUSEPORT failed: %s (errno=%d)", strerror(errno),
        errno);
  }
  else
  {
  ...
  {
    log("WARNING: Discord bridge failed to set non-blocking: %s (errno=%d)", strerror(errno),
        errno);
  }
  else
  {
...
void accept_discord_connection(void)
{
  ...
  log("Discord bridge connected from %s:%d", inet_ntoa(peer.sin_addr), ntohs(peer.sin_port));
}
...
void process_discord_input(void)
{
  ...
  bytes_read =
      recv(discord_bridge->client_socket, discord_bridge->inbuf + discord_bridge->inbuf_len,
...
void process_discord_output(void)
{
  ...
  /* Send data */
  bytes_sent =
      send(discord_bridge->client_socket, discord_bridge->outbuf, discord_bridge->outbuf_len, 0);
...
void route_discord_to_mud(const char *channel, const char *name, const char *message)
{
  ...
  for (d = descriptor_list; d; d = d->next)
  {
    ...
    snprintf(buf, sizeof(buf), "%s%s: %s%s\r\n", channel_color, formatted_name, message,
...
void load_discord_config(void)
{
  ...
  if (discord_bridge->num_channels > 0)
  {
    ...
    for (i = 0; i < discord_bridge->num_channels; i++)
    {
      DISCORD_DEBUG("  Channel %d: MUD='%s' Discord='%s' SCMD=%d Enabled=%d", i + 1,
                    discord_bridge->channels[i].mud_channel,
                    discord_bridge->channels[i].discord_name, discord_bridge->channels[i].scmd,
