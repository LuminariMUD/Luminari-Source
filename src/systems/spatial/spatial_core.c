...
int spatial_process_stimulus(struct spatial_context *ctx, struct spatial_system *system)
{
  ...
  spatial_debug("Processing %s stimulus from (%d,%d,%d) to (%d,%d,%d)", system->system_name,
                ctx->source_x, ctx->source_y, ctx->source_z, ctx->observer_x, ctx->observer_y,
                ctx->observer_z);
  ...
  if (system->modifiers->apply_environmental_modifiers(ctx, &range_modifier, &clarity_modifier) !=
      SPATIAL_SUCCESS)
  {
  ...
  if (ctx->final_intensity > SPATIAL_MIN_THRESHOLD)
  {
    ...
    if (system->stimulus->generate_base_message(ctx, ctx->processed_message,
                                                SPATIAL_MAX_MESSAGE_LENGTH) == SPATIAL_SUCCESS)
    {
      spatial_log("SPATIAL: Base message generated successfully");
      /* Allow modifiers to adjust the message based on clarity */
      if (system->modifiers->modify_message(ctx, ctx->processed_message,
                                            SPATIAL_MAX_MESSAGE_LENGTH) == SPATIAL_SUCCESS)
      {
  ...
  spatial_debug("Stimulus below threshold: %.3f < %.3f", ctx->final_intensity,
                SPATIAL_MIN_THRESHOLD);
  return SPATIAL_ERROR_BELOW_THRESHOLD;
}
 ...
 */
int spatial_process_all_systems(struct spatial_context *ctx, struct spatial_system **systems,
                                int system_count)
{
  ...
  for (i = 0; i < system_count; i++)
  {
    if (systems[i] && systems[i]->enabled)
    {
      if (spatial_process_stimulus(ctx, systems[i]) == SPATIAL_SUCCESS)
      {
        ...
        spatial_debug("Message processed by %s system: %s", systems[i]->system_name,
...
int spatial_get_weather_type(struct char_data *observer)
{
  ...
  if (room != NOWHERE && IS_WILDERNESS_VNUM(GET_ROOM_VNUM(room)))
  {
    ...
    if (wilderness_weather >= 225)
      return 4; /* Lightning */
    if (wilderness_weather >= 200)
      return 3; /* Storm */
    if (wilderness_weather >= 178)
      return 2; /* Rainy */
    if (wilderness_weather > 127)
      return 1; /* Cloudy */
    return 0;   /* Clear */
  }
  else
...
int spatial_get_raw_weather(struct char_data *observer)
{
  ...
  else
  {
    ...
    {
    case SKY_CLOUDLESS:
      return 100; /* Clear equivalent */
    case SKY_CLOUDY:
      return 150; /* Cloudy equivalent */
    case SKY_RAINING:
      return 190; /* Rainy equivalent */
    case SKY_LIGHTNING:
      return 230; /* Lightning equivalent */
    default:
      return 100; /* Clear fallback */
    }
  }
}
                          ...
                          struct char_data *observer, const char *description)
{
  ...
  ctx->distance = 0.0;
  ctx->direction =
      spatial_calculate_direction(ctx->observer_x, ctx->observer_y, ctx->observer_z, ctx->source_x,
                                  ctx->source_y, ctx->source_z, &ctx->direction_precision);
                                                ...
                                                float *precision)
{
  ...
  if (angle >= 337.5 || angle < 22.5)
    result = SPATIAL_DIR_EAST;
  else if (angle >= 22.5 && angle < 67.5)
    result = SPATIAL_DIR_NORTHEAST;
  else if (angle >= 67.5 && angle < 112.5)
    result = SPATIAL_DIR_NORTH;
  else if (angle >= 112.5 && angle < 157.5)
    result = SPATIAL_DIR_NORTHWEST;
  else if (angle >= 157.5 && angle < 202.5)
    result = SPATIAL_DIR_WEST;
  else if (angle >= 202.5 && angle < 247.5)
    result = SPATIAL_DIR_SOUTHWEST;
  else if (angle >= 247.5 && angle < 292.5)
    result = SPATIAL_DIR_SOUTH;
  else if (angle >= 292.5 && angle < 337.5)
    result = SPATIAL_DIR_SOUTHEAST;
  else
...
const char *spatial_direction_to_string(spatial_direction_t direction, float distance)
{
  ...
  case SPATIAL_DIR_HERE:
    ...
    return buf;
  case SPATIAL_DIR_NORTH:
    base_dir = "north";
    break;
  case SPATIAL_DIR_NORTHEAST:
    base_dir = "northeast";
    break;
  case SPATIAL_DIR_EAST:
    base_dir = "east";
    break;
  case SPATIAL_DIR_SOUTHEAST:
    base_dir = "southeast";
    break;
  case SPATIAL_DIR_SOUTH:
    base_dir = "south";
    break;
  case SPATIAL_DIR_SOUTHWEST:
    base_dir = "southwest";
    break;
  case SPATIAL_DIR_WEST:
    base_dir = "west";
    break;
  case SPATIAL_DIR_NORTHWEST:
    base_dir = "northwest";
    break;
...
const char *spatial_error_string(int error_code)
{
  switch (error_code)
  {
  case SPATIAL_SUCCESS:
    return "Success";
  case SPATIAL_ERROR_STIMULUS:
    return "Stimulus calculation failed";
  case SPATIAL_ERROR_LOS:
    return "Line of sight calculation failed";
  case SPATIAL_ERROR_MODIFIERS:
    return "Environmental modifier calculation failed";
  case SPATIAL_ERROR_BELOW_THRESHOLD:
    return "Stimulus below detection threshold";
  case SPATIAL_ERROR_INVALID_PARAM:
    return "Invalid parameter";
  case SPATIAL_ERROR_MEMORY:
    return "Memory allocation failed";
  case SPATIAL_ERROR_NOT_IMPLEMENTED:
