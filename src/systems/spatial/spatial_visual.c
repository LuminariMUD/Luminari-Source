...
static int visual_generate_message(struct spatial_context *ctx, char *output, size_t max_len)
{
  ...
  {
    spatial_log(
        "SPATIAL: visual_generate_message validation failed - ctx=%p, output=%p, source_desc=%p",
  ...
  case VISUAL_MSG_CLEAR:
    ...
    {
      snprintf(output, max_len, "You clearly see %s to %s.", ctx->source_description,
               direction_str);
    }
    break;
  ...
  case VISUAL_MSG_DISTANT:
    ...
    {
      snprintf(output, max_len, "In the distance to %s, you see %s.", direction_str,
  ...
  case VISUAL_MSG_OBSCURED:
    ...
    {
      snprintf(output, max_len, "Through the haze above, you make out %s.",
               ctx->source_description);
    }
    else if (ctx->direction == SPATIAL_DIR_BELOW)
    {
      snprintf(output, max_len, "Through the haze below, you make out %s.",
               ctx->source_description);
    }
    else
    {
      snprintf(output, max_len, "Through the haze to %s, you make out %s.", direction_str,
               ctx->source_description);
    }
    break;
  ...
  case VISUAL_MSG_SILHOUETTE:
    ...
    {
      snprintf(output, max_len, "Somewhere to %s, you glimpse the silhouette of %s.", direction_str,
               ctx->source_description);
    }
    break;
  ...
  case VISUAL_MSG_GLIMPSE:
    ...
    {
      snprintf(output, max_len, "You catch a brief glimpse of something moving to %s.",
               direction_str);
    }
    break;
  }
...
static int physical_calculate_obstruction(struct spatial_context *ctx, float *obstruction_factor)
{
  ...
  *obstruction_factor = MIN(total_obstruction, 1.0f);
  ...
  spatial_log("SPATIAL: Physical obstruction: %.6f over %d steps from (%d,%d) to (%d,%d)",
              *obstruction_factor, steps, ctx->source_x, ctx->source_y, ctx->observer_x,
              ctx->observer_y);
                                           ...
                                           float *clarity_mod)
{
  ...
  *clarity_mod = weather_clarity_mod * time_clarity_mod;
  ...
  spatial_debug(
      "Environmental modifiers: range %.3f, clarity %.3f (weather: %.3f/%.3f, time: %.3f/%.3f)",
      *range_mod, *clarity_mod, weather_range_mod, weather_clarity_mod, time_range_mod,
      time_clarity_mod);

  spatial_log(
      "SPATIAL: Weather/terrain modifiers - range: %.6f, clarity: %.6f, weather: %d, time: %d",
                                          ...
                                          size_t max_len)
{
  ...
  else if (ctx->weather_conditions == 2)
  { /* Rainy */
    if (strncmp(temp_msg, "You clearly see", 15) == 0)
    {
  ...
  else if (ctx->time_of_day == SUN_DARK)
  { /* Night */
    if (strncmp(temp_msg, "You clearly see", 15) == 0)
    {
