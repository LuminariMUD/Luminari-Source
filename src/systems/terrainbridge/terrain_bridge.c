...
char *process_terrain_request(const char *json_request)
{
  ...
  if (strcmp(command, "get_terrain") == 0)
  {
        ...
        !json_object_object_get_ex(root, "y", &y_obj))
    {
      json_object_object_add(response, "error",
                             json_object_new_string("Missing x or y coordinates"));
    ...
    else
    {
      ...
      else
      {
        ...
        {
          json_object_object_add(result_obj, "sector_name", json_object_new_string("unknown"));
        }
  ...
  else if (strcmp(command, "get_static_rooms_list") == 0)
  {
    ...
    for (room_rnum_val = 0; room_rnum_val <= top_of_world; room_rnum_val++)
    {
      ...
      json_object_object_add(room_obj, "name",
                             json_object_new_string(room->name ? room->name : ""));
      ...
      json_object_object_add(
          room_obj, "sector_type",
          json_object_new_string((room->sector_type >= 0 && room->sector_type < NUM_ROOM_SECTORS)
    ...
    else
    {
      ...
      else
      {
        ...
        json_object_object_add(room_obj, "description",
                               json_object_new_string(room->description ? room->description : ""));
        json_object_object_add(
            room_obj, "sector_type",
            json_object_new_string((room->sector_type >= 0 && room->sector_type < NUM_ROOM_SECTORS)
                                       ? sector_types[room->sector_type]
        ...
        if (zone_rnum_val >= 0 && zone_rnum_val <= top_of_zone_table)
        {
          ...
          json_object_object_add(zone_obj, "vnum",
                                 json_object_new_int(zone_table[zone_rnum_val].number));
          json_object_object_add(zone_obj, "name", json_object_new_string(clean_zone_name));
          json_object_object_add(zone_obj, "builders",
                                 json_object_new_string(zone_table[zone_rnum_val].builders
                                                            ? zone_table[zone_rnum_val].builders
                                                            : ""));
          json_object_object_add(zone_obj, "min_level",
                                 json_object_new_int(zone_table[zone_rnum_val].min_level));
          json_object_object_add(zone_obj, "max_level",
                                 json_object_new_int(zone_table[zone_rnum_val].max_level));
        ...
        for (dir = 0; dir < NUM_OF_DIRS; dir++)
        {
          if (room->dir_option[dir] && room->dir_option[dir]->to_room != NOWHERE)
          {
            ...
            if (exit_room_vnum != NOWHERE)
            {
              ...
              json_object_object_add(exit_obj, "to_room_vnum",
                                     json_object_new_int64(exit_room_vnum));
              json_object_object_add(
                  exit_obj, "to_room_name",
                  json_object_new_string(exit_room->name ? exit_room->name : ""));
              json_object_object_add(
                  exit_obj, "to_room_sector_type",
                  json_object_new_string(
                      (exit_room->sector_type >= 0 && exit_room->sector_type < NUM_ROOM_SECTORS)
                          ? sector_types[exit_room->sector_type]
                          : "unknown"));
              json_object_object_add(exit_obj, "to_room_sector_type_num",
                                     json_object_new_int(exit_room->sector_type));
              ...
              if (zone_rnum_val >= 0 && zone_rnum_val <= top_of_zone_table)
              {
                json_object_object_add(exit_obj, "to_zone_vnum",
                                       json_object_new_int(zone_table[zone_rnum_val].number));
                json_object_object_add(exit_obj, "to_zone_name",
                                       json_object_new_string(zone_table[zone_rnum_val].name
                                                                  ? zone_table[zone_rnum_val].name
                                                                  : ""));
              }
              else
              {
              ...
              /* Exit flags/info */
              json_object_object_add(exit_obj, "exit_info",
                                     json_object_new_int(room->dir_option[dir]->exit_info));
              json_object_object_add(exit_obj, "keyword",
                                     json_object_new_string(room->dir_option[dir]->keyword
                                                                ? room->dir_option[dir]->keyword
                                                                : ""));
              json_object_object_add(
                  exit_obj, "description",
                  json_object_new_string(room->dir_option[dir]->general_description
  ...
  else if (strcmp(command, "get_wilderness_exits") == 0)
  {
    ...
    for (room = 0; room <= top_of_world; room++)
    {
      ...
      {
        /* Check if this room has any exits to non-wilderness zones */
        json_object *exits_array = json_object_new_array();
        int has_non_wild_exit = 0;
        ...
        for (dir = 0; dir < NUM_OF_DIRS; dir++)
        {
          if (world[room].dir_option[dir] && world[room].dir_option[dir]->to_room != NOWHERE)
          {
            ...
            if (!IS_WILDERNESS_VNUM(world[target_room].number))
            {
              ...
              /* Add exit direction */
              json_object_object_add(exit_obj, "direction", json_object_new_string(dirs[dir]));
              json_object_object_add(exit_obj, "direction_number", json_object_new_int(dir));

              /* Add target zone info */
              json_object_object_add(exit_obj, "leads_to_vnum",
                                     json_object_new_int(world[target_room].number));
              json_object_object_add(exit_obj, "leads_to_zone",
              ...
              if (world[target_room].name)
              {
                ...
                if (clean_target_name)
                {
                  ...
                  json_object_object_add(exit_obj, "leads_to_name",
                  ...
                  zone_table[world[target_room].zone].name)
              {
                ...
                {
                  strip_colors(clean_zone_name);
                  json_object_object_add(exit_obj, "leads_to_zone_name",
              ...
              {
                char *clean_desc = strdup(world[room].dir_option[dir]->general_description);
                strip_colors(clean_desc);
                json_object_object_add(exit_obj, "exit_description",
                                       json_object_new_string(clean_desc));
                free(clean_desc);
              }
              else
              {
                json_object_object_add(exit_obj, "exit_description", json_object_new_string(""));
              }
        ...
        /* If this room has non-wilderness exits, add it to results */
        if (has_non_wild_exit)
        {
          json_object *room_obj = json_object_new_object();

          /* Room basic info */
          json_object_object_add(room_obj, "room_vnum", json_object_new_int(world[room].number));

          /* Coordinates */
          json_object *coords_obj = json_object_new_object();
          json_object_object_add(coords_obj, "x", json_object_new_int(world[room].coords[0]));
          json_object_object_add(coords_obj, "y", json_object_new_int(world[room].coords[1]));
          ...
          if (world[room].name)
          {
            ...
            if (clean_name)
            {
              ...
              json_object_object_add(room_obj, "room_name", json_object_new_string(clean_name));
              free(clean_name);
            }
            else
            {
              json_object_object_add(room_obj, "room_name", json_object_new_string("Wilderness"));
            }
          }
          else
          {
          ...
          json_object_object_add(room_obj, "sector_type",
                                 json_object_new_int(world[room].sector_type));

          /* Add sector name if available */
          if (world[room].sector_type >= 0 && world[room].sector_type < NUM_ROOM_SECTORS)
          {
            json_object_object_add(room_obj, "sector_name",
                                   json_object_new_string(sector_types[world[room].sector_type]));
          }
          else
          {
            json_object_object_add(room_obj, "sector_name", json_object_new_string("unknown"));
          }
        ...
        {
          /* Free unused exits array */
          json_object_put(exits_array);
        }
      }
    }

    json_object_object_add(response, "success", json_object_new_boolean(TRUE));
    json_object_object_add(response, "total_rooms", json_object_new_int(total_rooms));
    json_object_object_add(response, "data", data_array);

    if (total_rooms > 0)
    {
      char msg[256];
      snprintf(msg, sizeof(msg), "Found %d wilderness rooms with exits to static zones",
               total_rooms);
    ...
    else
    {
      json_object_object_add(response, "message",
                             json_object_new_string("No wilderness exit rooms found"));
    }
  }
  else if (strcmp(command, "get_static_room_by_coordinates") == 0)
  {
    ...
    else
    {
      ...
      if (static_room != NOWHERE)
      {
        ...
        json_object *coords_obj = json_object_new_object();
        json_object_object_add(coords_obj, "x", json_object_new_int(world[static_room].coords[0]));
        json_object_object_add(coords_obj, "y", json_object_new_int(world[static_room].coords[1]));
        json_object_object_add(room_obj, "coordinates", coords_obj);
        ...
        if (world[static_room].sector_type >= 0 &&
            world[static_room].sector_type < NUM_ROOM_SECTORS)
        {
          json_object_object_add(
              room_obj, "sector_name",
        ...
        if (world[static_room].description)
        {
          ...
          strip_colors(clean_desc);
          json_object_object_add(room_obj, "description", json_object_new_string(clean_desc));
          free(clean_desc);
        }
        else
        {
          json_object_object_add(room_obj, "description", json_object_new_string(""));
        }
        ...
        {
          json_object_object_add(room_obj, "zone_vnum", json_object_new_int(-1));
          json_object_object_add(room_obj, "zone_name", json_object_new_string("Unknown Zone"));
        }

        /* Room flags */
        json_object_object_add(room_obj, "room_flags_0",
                               json_object_new_int(world[static_room].room_flags[0]));
        json_object_object_add(room_obj, "room_flags_1",
                               json_object_new_int(world[static_room].room_flags[1]));
        json_object_object_add(room_obj, "room_flags_2",
                               json_object_new_int(world[static_room].room_flags[2]));
        json_object_object_add(room_obj, "room_flags_3",
                               json_object_new_int(world[static_room].room_flags[3]));
      ...
      else
      {
        ...
        json_object_object_add(response, "message",
                               json_object_new_string("No static room at these coordinates"));
      }
    }
  }
  ...
  else
  {
    ...
    json_object_object_add(
        response, "error",
        json_object_new_string(
            "Unknown command (supported: get_terrain, get_terrain_batch, get_static_rooms_list, "
            "get_room_details, get_wilderness_exits, get_static_room_by_coordinates, ping)"));
    json_object_object_add(response, "success", json_object_new_boolean(FALSE));
  }
...
void log_terrain_api_stats(void)
{
  ...
  log("Terrain-API Info: Clients: %d/%d, Total connections: %d, Total requests: %d",
      terrain_api->num_clients, terrain_api->max_clients, terrain_api->total_connections,
      terrain_api->total_requests);
}
