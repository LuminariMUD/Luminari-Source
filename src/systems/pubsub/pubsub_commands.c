...
ACMD(do_pubsub)
{
  ...
  if (!*subcommand)
  {
    ...
    if (GET_LEVEL(ch) >= LVL_IMMORT)
    {
      ...
      send_to_char(ch, "  pubsub spatial               - Test spatial systems (visual+audio, "
                       "wilderness only)\r\n");
    }
    return;
  }
  ...
  if (is_abbrev(subcommand, "send"))
  {
    ...
    int result = pubsub_send_message(arg1, GET_NAME(ch), message, PUBSUB_MESSAGE_TYPE_SIMPLE,
                                     PUBSUB_CATEGORY_GENERAL);
  ...
  if (is_abbrev(subcommand, "spatial") && GET_LEVEL(ch) >= LVL_IMMORT)
  {
    ...
    int audio_result = spatial_audio_test_thunder(X_LOC(ch) - 8, Y_LOC(ch) + 3, "rumbling thunder");
...
ACMD(do_pubsubstat)
{
  ...
  send_to_char(ch, "Messages: %lld sent, %lld delivered, %lld failed\r\n",
               pubsub_stats.total_messages_sent, pubsub_stats.total_messages_delivered,
               pubsub_stats.total_messages_failed);
  send_to_char(ch, "Memory: %d topics, %d messages, %d subscriptions allocated\r\n",
               pubsub_stats.topics_allocated, pubsub_stats.messages_allocated,
...
ACMD(do_pubsubtopic)
{
  ...
  if (!strcasecmp(subcommand, "create"))
  {
    ...
    result = pubsub_create_topic(topic_name, *description ? description : NULL,
                                 PUBSUB_CATEGORY_GENERAL, PUBSUB_ACCESS_PUBLIC, GET_NAME(ch));
...
ACMD(do_pubsubqueue)
{
  ...
  if (!*subcommand || !strcasecmp(subcommand, "status"))
  {
    ...
    send_to_char(ch, "Processed: C:%lld U:%lld H:%lld N:%lld L:%lld\r\n",
                 pubsub_stats.queue_critical_processed, pubsub_stats.queue_urgent_processed,
                 pubsub_stats.queue_high_processed, pubsub_stats.queue_normal_processed,
  ...
  else if (!strcasecmp(subcommand, "test"))
  {
    ...
    if (pubsub_publish(1, GET_NAME(ch), "Test message for queue system", PUBSUB_MESSAGE_TYPE_SIMPLE,
                       PUBSUB_PRIORITY_NORMAL) == PUBSUB_SUCCESS)
    {
  ...
  else if (!strcasecmp(subcommand, "spatial"))
  {
    ...
    if (ZONE_FLAGGED(GET_ROOM_ZONE(IN_ROOM(ch)), ZONE_WILDERNESS))
    {
      int result = pubsub_publish_wilderness_audio(
          X_LOC(ch), Y_LOC(ch), get_modified_elevation(X_LOC(ch), Y_LOC(ch)), GET_NAME(ch),
          "A mysterious sound echoes across the wilderness", 25, PUBSUB_PRIORITY_NORMAL);
      if (result == PUBSUB_SUCCESS)
      {
