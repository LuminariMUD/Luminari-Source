...
int pubsub_handler_send_formatted(struct char_data *ch, struct pubsub_message *msg)
{
  ...
  case PUBSUB_PRIORITY_CRITICAL:
    snprintf(formatted_msg, sizeof(formatted_msg), "%s[CRITICAL]%s %s%s\r\n", KRED, KNRM,
             msg->content, KNRM);
    break;
  case PUBSUB_PRIORITY_URGENT:
    snprintf(formatted_msg, sizeof(formatted_msg), "%s[URGENT]%s %s%s\r\n", KYEL, KNRM,
             msg->content, KNRM);
    break;
  case PUBSUB_PRIORITY_HIGH:
    snprintf(formatted_msg, sizeof(formatted_msg), "%s[HIGH]%s %s%s\r\n", KCYN, KNRM, msg->content,
             KNRM);
    break;
...
int pubsub_handler_spatial_audio(struct char_data *ch, struct pubsub_message *msg)
{
  ...
  if (msg->spatial_data)
  {
    if (sscanf(msg->spatial_data, "%d,%d,%d,%d", &spatial.world_x, &spatial.world_y,
  ...
  /* Calculate distance */
  distance = (int)sqrt(pow(player_x - spatial.world_x, 2) + pow(player_y - spatial.world_y, 2));
  ...
  {
    /* Right at the source */
    snprintf(spatial_msg, sizeof(spatial_msg), "%s%s%s\r\n", KRED, msg->content, KNRM);
  }
  else if (volume_modifier > 0.8f)
  {
    /* Very close */
    snprintf(spatial_msg, sizeof(spatial_msg), "You hear %s%s%s clearly.\r\n", KYEL, msg->content,
             KNRM);
  }
  else if (volume_modifier > 0.5f)
  {
    /* Moderate distance */
    snprintf(spatial_msg, sizeof(spatial_msg), "You hear %s%s%s in the distance.\r\n", KCYN,
             msg->content, KNRM);
  }
  else if (volume_modifier > 0.2f)
  {
    /* Far away */
    snprintf(spatial_msg, sizeof(spatial_msg), "You faintly hear %s%s%s from far away.\r\n", KBLU,
             msg->content, KNRM);
  }
  else
  {
    /* Very faint */
    snprintf(spatial_msg, sizeof(spatial_msg), "You barely make out the sound of %s%s%s.\r\n", KBLK,
             msg->content, KNRM);
  }
...
int pubsub_handler_personal_message(struct char_data *ch, struct pubsub_message *msg)
{
  ...
  {
    snprintf(personal_msg, sizeof(personal_msg), "%s[Personal Message from %s]%s %s%s\r\n", KMAG,
             msg->sender_name, KNRM, msg->content, KNRM);
  }
  else
  {
    snprintf(personal_msg, sizeof(personal_msg), "%s[Personal Message]%s %s%s\r\n", KMAG, KNRM,
             msg->content, KNRM);
  }
