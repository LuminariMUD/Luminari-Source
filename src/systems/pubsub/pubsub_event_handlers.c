...
int pubsub_handler_combat_logger(struct char_data *ch, struct pubsub_message *msg)
{
  ...
  fprintf(logfile, "[%s] Player: %s | Level: %d | Class: %s | Event: %s | Room: %d\n", timestamp,
          GET_NAME(ch), GET_LEVEL(ch), class_names[GET_CLASS(ch)], msg->content,
...
int pubsub_handler_death_processor(struct char_data *ch, struct pubsub_message *msg)
{
  ...
  /* Basic death message */
  snprintf(death_msg, sizeof(death_msg), "%s has died and been resurrected!", GET_NAME(ch));
...
int pubsub_handler_levelup_processor(struct char_data *ch, struct pubsub_message *msg)
{
  ...
  snprintf(level_msg, sizeof(level_msg),
           "Congratulations! You've gained %d hit points and %d movement!", hit_bonus, move_bonus);
  ...
  /* Log the event */
  mudlog(BRF, LVL_IMMORT, TRUE, "LEVELUP: %s reached level %d (+%d hp, +%d mv)", GET_NAME(ch),
         GET_LEVEL(ch), hit_bonus, move_bonus);
...
{
  pubsub_register_handler("combat_logger", "Combat analysis logger", pubsub_handler_combat_logger);
                          ...
                          int priority)
{
  ...
  if (!topic)
  {
    ...
    int topic_id =
        pubsub_create_topic(event_topic, "Auto-created event topic", PUBSUB_CATEGORY_SYSTEM,
                            PUBSUB_ACCESS_PUBLIC, ch ? GET_NAME(ch) : "System");
    if (topic_id <= 0)
    {
      return;
    }
  ...
  /* Publish the event */
  pubsub_publish(topic->topic_id, ch ? GET_NAME(ch) : "System", event_data,
                 PUBSUB_MESSAGE_TYPE_SYSTEM, priority);
}
