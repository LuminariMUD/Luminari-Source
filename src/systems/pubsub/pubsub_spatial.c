...
int pubsub_handler_wilderness_spatial_audio(struct char_data *ch, struct pubsub_message *msg)
{
  ...
  distance_3d = calculate_distance_3d(listener_x, listener_y, listener_z, spatial.world_x,
                                      spatial.world_y, spatial.world_z);
  ...
  terrain_modifier =
      calculate_terrain_modifier(listener_x, listener_y, spatial.world_x, spatial.world_y);

  /* Calculate elevation effects */
  elevation_effect =
      calculate_elevation_effect(listener_x, listener_y, spatial.world_x, spatial.world_y);
  ...
  else
  {
    ...
    if (volume_modifier < 0.0f)
      volume_modifier = 0.0f;
    if (volume_modifier > 1.0f)
    ...
    {
      /* Close */
      snprintf(spatial_msg, sizeof(spatial_msg), "You clearly hear a mysterious sound nearby.\r\n");
    }
  ...
  { /* Only show direction if reasonably far */
    char direction[64] = "somewhere";

    if (abs(dx) > abs(dy))
    {
      if (dx > 0)
        strcpy(direction, "to the east");
      else
        strcpy(direction, "to the west");
    }
    else
    {
      if (dy > 0)
        strcpy(direction, "to the north");
      else
        strcpy(direction, "to the south");
    }
    ...
    {
      if (dx > 0 && dy > 0)
        strcpy(direction, "to the northeast");
      else if (dx > 0 && dy < 0)
        strcpy(direction, "to the southeast");
      else if (dx < 0 && dy > 0)
        strcpy(direction, "to the northwest");
      else
        strcpy(direction, "to the southwest");
    }

    snprintf(direction_info, sizeof(direction_info), " The sound comes from %s.", direction);
  ...
  add_audio_source(spatial.world_x, spatial.world_y, spatial.world_z, msg->content,
                   msg->sender_name ? msg->sender_name : "unknown", msg->priority, volume_modifier,
...
static bool has_line_of_sound(int listener_x, int listener_y, int source_x, int source_y)
{
  ...
  if (steps <= 1)
    return true; /* Adjacent or same location */
...
static char *blend_audio_sources(struct char_data *ch __attribute__((unused)),
                                 struct spatial_audio_source *sources)
{
  char *result;
  struct spatial_audio_source *source;
  int count = 0;
  ...
  if (count == 1)
  {
    ...
    snprintf(result, MAX_STRING_LENGTH, "You hear %s from %s.\r\n", sources->content,
             sources->source_name);
  }
  else
  {
    /* Multiple sources - create a blended message */
    snprintf(result, MAX_STRING_LENGTH, "You hear a mixture of sounds: ");
