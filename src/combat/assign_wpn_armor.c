...
bool reload_weapon(struct char_data *ch, struct obj_data *wielded,
                   bool silent_mode __attribute__((unused)))
{
  int load_amount = 0;
...
void load_weapons(void)
{
  ...
  setweapon(WEAPON_TYPE_RAPIER, "rapier", 1, 6, 2, 2, WEAPON_FLAG_MARTIAL | WEAPON_FLAG_BALANCED,
            20, DAMAGE_TYPE_PIERCING, 2, 0, WEAPON_FAMILY_SMALL_BLADE, SIZE_SMALL, MATERIAL_STEEL,
            HANDLE_TYPE_HILT, HEAD_TYPE_BLADE,
  ...
  for (type = 1; type < NUM_WEAPON_TYPES; type++)
  {
    ...
    len =
        snprintf_append(buf, sizeof(buf), len,
                        ...
                        "Handle: %s, Head: %s.\r\n",
                        weapon_list[type].name, weapon_list[type].numDice,
                        weapon_list[type].diceSize, (20 - weapon_list[type].critRange), crit_multi,
                        buf2, weapon_list[type].cost, buf3, weapon_list[type].weight,
