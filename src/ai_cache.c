...
void ai_cache_response(const char *key, const char *response)
{
  ...
  AI_DEBUG("ai_cache_response() called with key='%s', response_len=%zu", key ? key : "(null)",
  ...
  entry->expires_at = now + AI_CACHE_EXPIRE_TIME;
  AI_DEBUG("  Entry expires at %ld (in %d seconds)", (long)entry->expires_at, AI_CACHE_EXPIRE_TIME);

  entry->next = ai_state.cache_head;
  ai_state.cache_head = entry;
  ai_state.cache_size++;

  AI_DEBUG("Cache entry added, new cache size: %d/%d", ai_state.cache_size, AI_MAX_CACHE_SIZE);
...
char *ai_cache_get(const char *key)
{
  ...
  for (entry = ai_state.cache_head; entry; entry = entry->next, position++)
  {
    if (!strcmp(entry->key, key))
    {
      ...
      {
        AI_DEBUG("  Entry valid (expires in %ld seconds)", (long)(entry->expires_at - now));
        AI_DEBUG("  Returning cached response (length=%zu)", strlen(entry->response));
        return entry->response;
      }
      /* Entry expired, will be cleaned up later */
      AI_DEBUG("  Entry expired %ld seconds ago", (long)(now - entry->expires_at));
      break;
    }
  }
...
void ai_cache_clear(void)
{
  ...
  {
    next = entry->next;
    AI_DEBUG("  Clearing entry: key='%s'", entry->key ? entry->key : "(null)");
    if (entry->key)
      free(entry->key);
    if (entry->response)
      free(entry->response);
