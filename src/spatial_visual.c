...
static int weather_terrain_apply_modifiers(struct spatial_context *ctx, float *range_mod, float *clarity_mod) {
    ...
    /* Weather effects (placeholder - integrate with actual weather system) */
    ...
    /* Time of day effects */
    if (ctx->time_of_day >= 6 && ctx->time_of_day <= 18) {
        /* Daytime - normal visibility */
        time_range_mod = 1.0f;
        time_clarity_mod = 1.0f;
    } else if (ctx->time_of_day >= 19 && ctx->time_of_day <= 21) {
        /* Twilight - reduced visibility */
        time_range_mod = 0.7f;
        time_clarity_mod = 0.8f;
    } else {
        /* Night - greatly reduced visibility */
        time_range_mod = 0.3f;
...
static int weather_terrain_modify_message(struct spatial_context *ctx, char *message, size_t max_len) {
    ...
    } else if (ctx->time_of_day < 6 || ctx->time_of_day > 21) {  /* Night */
