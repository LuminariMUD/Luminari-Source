...
void repair_database_if_needed(void)
{
  ...
  const char *index_queries[] = {
      ...
      "CREATE SPATIAL INDEX IF NOT EXISTS idx_path_geometry ON path_data(path_geometry)", NULL};
...
void log_database_startup_stats(void)
{
  ...
  /* Get table count */
  if (!mysql_query_safe(
          conn, "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE()"))
  {
    MYSQL_RES *result = mysql_store_result_safe(conn);
    if (result)
    {
