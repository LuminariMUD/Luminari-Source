...
int get_crafting_group_for_wilderness_material(int category, int subtype, int quality)
{
  ...
  case RESOURCE_MINERALS:
    ...
    if (quality >= 4)
      return CRAFT_GROUP_HARD_METALS; // Rare/Legendary → hard metals
    else
      return CRAFT_GROUP_SOFT_METALS; // Common metals → soft metals
...
int wilderness_to_crafting_material(int category, int subtype, int quality)
{
  ...
  case CRAFT_GROUP_HARD_METALS:
    return CRAFT_MAT_HARDMETALS;
  case CRAFT_GROUP_SOFT_METALS:
    return CRAFT_MAT_SOFTMETALS;
  case CRAFT_GROUP_ANIMAL_HIDES:
    return CRAFT_MAT_ANIMALHIDES;
  case CRAFT_GROUP_WOOD:
    return CRAFT_MAT_WOOD;
  case CRAFT_GROUP_CLOTH:
    return CRAFT_MAT_CLOTH;
  default:
...
int is_wilderness_material_craftable(int category, int subtype, int quality)
{
  ...
  if (category >= 0 && category < NUM_RESOURCE_TYPES && subtype >= 0 &&
      subtype < NUM_SUBTYPES_PER_CATEGORY && quality >= 1 && quality <= 5)
  {
    return TRUE;
  }
  return FALSE;
...
void show_crafting_materials_display(struct char_data *ch)
{
  ...
  for (i = 1; i < NUM_CRAFT_MATS; i++)
  {
    ...
    {
      send_to_char(ch, "  %s: %d unit%s\r\n", crafting_materials[i], GET_CRAFT_MAT(ch, i),
...
int get_enhanced_crafting_group_for_wilderness_material(int category, int subtype, int quality)
{
  ...
  case RESOURCE_WOOD:
    ...
    if (subtype >= 4)
    { /* Magical woods: thornvine, moonweave, darkwillow, starwood */
  ...
  case RESOURCE_GAME:
    ...
    if (subtype >= 4 || quality >= 4)
    { /* Magical creature hides */
  ...
  case RESOURCE_STONE:
    ...
    if (subtype >= 3)
    { /* Magical stones: moonstone, dragonbone, voidrock, etc. */
  ...
  case RESOURCE_VEGETATION:
    ...
    { /* Spirit grass, shadow fern */
      return ENHANCED_CRAFT_GROUP_SPIRIT_MATERIALS;
    }
    else if (subtype == 4 || subtype == 5 || subtype == 7)
    { /* Flame flower, ice lichen, light bloom */
...
void show_enhanced_materials_display(struct char_data *ch)
{
  ...
  send_to_char(
      ch,
      "Your wilderness materials are automatically available for enhanced crafting recipes.\r\n");
  send_to_char(ch,
               "Material quality affects crafting effectiveness and final item properties.\r\n");
