...
/* assistant function to duplicate all the strings in a quest */
...
/* given a quest, free all the memory used by the strings */
...
/* add (or update) a quest entry into the quest-table */
int add_quest(struct aq_data *nqst)
{
  ...
  else
  {
    ...
    /* Now process entries from the top down to see where the new one goes */
