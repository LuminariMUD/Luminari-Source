...
static void medit_disp_stats_menu(struct descriptor_data *d)
{
  ...
  if (CONFIG_MEDIT_ADVANCED)
  {
    ...
    write_to_output(d, "(%sS%s) Spell Resist: %s[%s%3d%s]%s\r\n", cyn, nrm, cyn, yel,
                    GET_REAL_SPELL_RES(mob), cyn, nrm);
  }
...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_ADD_ECHO:
    smash_tilde(arg);
    if (ECHO_COUNT(OLC_MOB(d)) >= MAX_MOB_ECHOES)
  ...
  case MEDIT_STATS_MENU:
    ...
    case 'S':
      ...
      OLC_MODE(d) = MEDIT_SPELL_RESISTANCE;
      i++;
      break;
    default:
      medit_disp_stats_menu(d);
  ...
  case MEDIT_SPELL_RESISTANCE:
    GET_REAL_SPELL_RES(OLC_MOB(d)) = LIMIT(i, 0, 100);
    GET_SPELL_RES(OLC_MOB(d)) = GET_REAL_SPELL_RES(OLC_MOB(d));
    OLC_VAL(d) = TRUE;
    medit_disp_stats_menu(d);
    return;
