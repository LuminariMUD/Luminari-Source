...
static int write_help_entry_to_file(FILE *fp, const char *keywords, const char *content,
                                    int min_level);
...
static bool validate_help_tag(const char *tag, struct descriptor_data *d)
{
  ...
  for (p = tag; *p; p++)
  {
    ...
    {
      write_to_output(d,
                      "Invalid character '%c' in help tag. Use only letters, numbers, spaces, "
...
static bool validate_help_content(const char *content, struct descriptor_data *d)
{
  ...
  if (strstr(content, "<script") || strstr(content, "<iframe") || strstr(content, "javascript:") ||
      strstr(content, "onclick"))
  {
...
ACMD(do_oasis_hedit)
{
  ...
  {
    send_to_char(ch, "Do you wish to edit the '%s' help file? ", OLC_HELP(d)->tag);
...
static void hedit_save_to_db(struct descriptor_data *d)
{
  ...
  if (!mysql_stmt_prepare_query(pstmt,
                                "SELECT keyword FROM help_keywords WHERE LOWER(help_tag) = ?"))
  {
  ...
  if (!pstmt)
  {
    mudlog(NRM, LVL_STAFF, TRUE,
           "SYSERR: Failed to create prepared statement for keyword insertion");
  ...
  if (!mysql_stmt_prepare_query(
          pstmt, "INSERT IGNORE INTO help_keywords (help_tag, keyword) VALUES (?, ?)"))
  {
  ...
  for (keyword = OLC_HELP(d)->keyword_list; keyword != NULL; keyword = keyword->next)
  {
    ...
    {
      mudlog(NRM, LVL_STAFF, TRUE, "SYSERR: Failed to bind parameters for keyword '%s'",
             keyword->keyword);
...
cleanup:
  ...
  if (transaction_started)
  {
    if (error_occurred)
    {
      ...
      {
        mudlog(NRM, LVL_STAFF, TRUE, "SYSERR: Failed to rollback transaction: %s",
               mysql_error(conn));
      }
...
static void hedit_disp_menu(struct descriptor_data *d)
{
  ...
  write_to_output(d,
                  ...
                  "%sQ%s) Quit\r\n"
                  "Enter choice : ",
                  nrm, grn, nrm, yel, OLC_HELP(d)->tag, grn, nrm, yel,
                  (OLC_HELP(d)->keyword_list == NULL ? "Not set." : "Set."), grn, nrm,
                  OLC_HELP(d)->entry, grn, nrm, yel, OLC_HELP(d)->min_level, grn, nrm, grn, nrm);
...
static void hedit_disp_keywords_menu(struct descriptor_data *d)
{
  ...
  get_char_colors(d->character);
  clear_screen(d);
  write_to_output(d, "Help entry keyword menu\r\n\r\n");
...
void hedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case HEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      if (OLC_HELP(d)->keyword_list == NULL)
      {
        /*  No keywords! */
        write_to_output(
            d, "Can not save a help entry with no keywords!  Add at least one keyword first.\r\n");
        hedit_disp_menu(d);
      }
      else
      {
  ...
  case HEDIT_CONFIRM_EDIT:
    ...
    case 'N':
      if (OLC_HELP(d)->next != NULL)
      {
        ...
        write_to_output(d, "Do you wish to edit the '%s' help file? ", OLC_HELP(d)->tag);
        OLC_MODE(d) = HEDIT_CONFIRM_EDIT;
      }
      else
      {
  ...
  case HEDIT_NEW_KEYWORD:
    ...
    /* Count existing keywords to prevent memory exhaustion */
    {
      ...
      {
        write_to_output(d,
                        "Maximum number of keywords (%d) reached. Delete some keywords first.\r\n",
                        MAX_KEYWORDS_PER_ENTRY);
        hedit_disp_keywords_menu(d);
        return;
      }
...
ACMD(do_hindex)
{
  ...
  pstmt = mysql_stmt_create(conn);
  if (pstmt)
  { /* Don't fail entirely if second query fails */
    if (mysql_stmt_prepare_query(pstmt, "SELECT DISTINCT hk.keyword "
                                        "FROM help_keywords hk "
                                        "JOIN help_entries he ON hk.help_tag = he.tag "
                                        "WHERE LOWER(hk.keyword) LIKE LOWER(?) "
  ...
  if (pstmt)
  {
    if (mysql_stmt_prepare_query(pstmt, "SELECT COUNT(DISTINCT he.tag) "
...
static const char *get_command_category(struct command_info *cmd)
{
  ...
  if (cmd->command_pointer == do_action)
    return "Social";
  if (cmd->command_pointer == do_gen_cast)
    return "Magic";
  if (cmd->command_pointer == do_gen_comm)
    return "Communication";
  if (cmd->command_pointer == do_gen_door)
    return "Interaction";
  if (cmd->command_pointer == do_gen_ps)
    return "Communication";
  if (cmd->command_pointer == do_gen_tog)
    return "Settings";
  if (cmd->command_pointer == do_write)
    return "Communication";
  if (cmd->command_pointer == do_activate)
    return "Magic";
  ...
  if (strstr(cmd->command, "cast") || strstr(cmd->command, "spell") ||
      ...
      strstr(cmd->command, "activate"))
    return "Magic";
  if (strstr(cmd->command, "attack") || strstr(cmd->command, "kill") ||
      strstr(cmd->command, "hit") || strstr(cmd->command, "bash") || strstr(cmd->command, "kick") ||
      strstr(cmd->command, "trip"))
    return "Combat";
  if (strstr(cmd->command, "get") || strstr(cmd->command, "drop") || strstr(cmd->command, "put") ||
      strstr(cmd->command, "wear") || strstr(cmd->command, "remove") ||
      strstr(cmd->command, "wield"))
    return "Items";
  if (strstr(cmd->command, "say") || strstr(cmd->command, "tell") ||
      strstr(cmd->command, "whisper") || strstr(cmd->command, "shout"))
    return "Communication";
  if (strstr(cmd->command, "look") || strstr(cmd->command, "examine") ||
      strstr(cmd->command, "score") || strstr(cmd->command, "who") || strstr(cmd->command, "where"))
    return "Information";
  if (strstr(cmd->command, "save") || strstr(cmd->command, "quit") ||
      strstr(cmd->command, "config") || strstr(cmd->command, "toggle"))
    return "System";

  /* Admin commands */
  if (cmd->minimum_level >= LVL_IMMORT)
...
static int generate_help_entry(struct char_data *ch, int cmd_index, bool force_overwrite)
{
  ...
  snprintf(query, sizeof(query), "SELECT tag FROM help_entries WHERE tag = '%s'", escaped_tag);
  ...
  snprintf(
      ...
      cmd->command, category, usage_str, description, get_level_name(cmd->minimum_level),
      get_position_name(cmd->minimum_position), action_desc[0] ? "  Action Type: " : "",
      action_desc, action_desc[0] ? "\r\n" : "", examples,
  ...
  if (exists && force_overwrite)
  {
    ...
    snprintf(query, sizeof(query), "DELETE FROM help_keywords WHERE help_tag = '%s'", escaped_tag);
...
ACMD(do_helpgen)
{
  ...
  {
    send_to_char(
        ch, "Usage: helpgen <missing|all|list|clean|repair|import|export|command> [options]\r\n"
  ...
  else if (!str_cmp(arg1, "import"))
  {
    ...
    {
      send_to_char(
          ch, "WARNING: Import requires an explicit mode to prevent accidental data changes.\r\n"
  ...
  else if (!str_cmp(arg1, "export"))
  {
    ...
    {
      send_to_char(
          ch, "WARNING: Export requires an explicit mode to prevent accidental file overwrites.\r\n"
...
static int export_help_to_hlp(struct char_data *ch, const char *options)
{
  ...
  snprintf(query, sizeof(query),
           ...
           max_level, exclude_auto ? "AND auto_generated = FALSE" : "");
                                        ...
                                        const char *mode, char *msg_buf, size_t msg_size)
{
  ...
  snprintf(query, query_size, "SELECT tag FROM help_entries WHERE tag = '%s'", escaped_tag);
  ...
  if ((!str_cmp(mode, "skip") || !str_cmp(mode, "preview")) && !tag_exists)
  {
    ...
    if (escaped_len > 0)
    {
      query_size = strlen("SELECT DISTINCT help_tag FROM help_keywords WHERE keyword IN ()") +
                   escaped_len + 1;
      CREATE(query, char, query_size);
      snprintf(query, query_size,
               "SELECT DISTINCT help_tag FROM help_keywords WHERE keyword IN (%s)",
               escaped_keywords);
  ...
  if (!str_cmp(mode, "preview"))
  {
    if (tag_exists)
    {
      snprintf(msg_buf, msg_size,
               "  [TAG EXISTS] %s - would skip/merge/force depending on mode\r\n", entry->tag);
    ...
    else if (keyword_exists)
    {
      snprintf(
          msg_buf, msg_size,
          "  [KEYWORD EXISTS] %s - keywords already mapped to %s (would skip in skip mode)\r\n",
  ...
  if (tag_exists)
  {
    if (!str_cmp(mode, "force"))
    {
      ...
      query_size =
          strlen("DELETE FROM help_keywords WHERE help_tag = ''") + strlen(escaped_tag) + 1;
      CREATE(query, char, query_size);
      snprintf(query, query_size, "DELETE FROM help_keywords WHERE help_tag = '%s'", escaped_tag);
      mysql_query(conn, query);
      free(query);

      query_size = strlen("DELETE FROM help_entries WHERE tag = ''") + strlen(escaped_tag) + 1;
      CREATE(query, char, query_size);
      snprintf(query, query_size, "DELETE FROM help_entries WHERE tag = '%s'", escaped_tag);
      mysql_query(conn, query);
      free(query);
    ...
    else if (!str_cmp(mode, "merge"))
    {
      ...
      do
      {
        ...
        query_size =
            strlen("SELECT tag FROM help_entries WHERE tag = ''") + strlen(escaped_tag) + 1;
        CREATE(query, char, query_size);
        snprintf(query, query_size, "SELECT tag FROM help_entries WHERE tag = '%s'", escaped_tag);
  ...
  else if (keyword_exists && !str_cmp(mode, "skip"))
  {
    ...
    snprintf(msg_buf, msg_size, "  [SKIPPED] %s - keywords already mapped to %s\r\n", entry->tag,
             existing_help_tag ? existing_help_tag : "another entry");
    if (existing_help_tag)
      free(existing_help_tag);
    return 0;
  }
