...
int OakenDefender(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  {
    ClearObjEvents(obj);
    initializeObjectFeature(ITEM_WEAPON, obj, TRORXEK_DEFENDER, PULSE_WEEK, TrorxekD);
    initializeObjectFeature(ITEM_WEAPON, obj, TRORXEK_CREEPINGDOOM, PULSE_DAY, TrorxekC);
    initializeObjectFeature(ITEM_WEAPON, obj, TRORXEK_RECALL, PULSE_HOUR, TrorxekR);
    initializeObjectFeature(ITEM_WEAPON, obj, TRORXEK_MOONWELL, PULSE_DAY, TrorxekM);
  ...
  {
    sprintf(idString, "Called Effects:  (invoked by 'say'ing them)\n"
    ...
    specialProcedureIdentifyObject(obj, ch, calltype, arg, ID_CLASS_DRUID, 0, 0, idString);
    return TRUE;
  }
  ...
  if (calltype == PROC_EVENT)
  {
    ...
    if (rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, TRORXEK_DEFENDER, TrorxekD) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, TRORXEK_CREEPINGDOOM,
                              TrorxekC) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, TRORXEK_RECALL, TrorxekR) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, TRORXEK_MOONWELL, TrorxekM))
      return FALSE;
    ...
    if (owner)
      ...
      if (checkObjectRestrictions(ITEM_WEAPON, obj, owner, OBJ_RESTRICT_CLASS, OBJ_RESTRICT_DRUID))
      {
  ...
  if (word && ((cmd == CMD_SAY) || (cmd == CMD_SAY2)))
  {
    ...
    if (!strcmp(word, TRORXEK_DEFENDER_CMD))
    {
      ...
      {
        send_to_char("The staff refuses, you feel a sense that the staff doesn't want to oblidge "
                     "you anymore, almost as if it dissapproves of you..",
                     ch);
        return TRUE;
        ...
        act("&+gThe ground trembles and shakes like an earthquake tremor!\n"
            ...
            FALSE, ch, obj, 0, TO_CHAR);
        act("&+yIn your mind your hear a deep voice, 'How may I serve, master..'", FALSE, ch, obj,
            0, TO_CHAR);
        ...
        act("&+gThe ground trembles and shakes like an earthquake tremor!\n"
            "&+yA massive Treant grows out of the ground in seconds, towering over you!",
            FALSE, ch, obj, 0, TO_ROOM);
        act("&+yThe Treant stands next to &n$n&n&+y, as silent as an ancient oak..", FALSE, ch, obj,
            0, TO_ROOM);
    ...
    else if (!strcmp(word, TRORXEK_RECALL_CMD))
    {
      if (!IS_SET(obj->value[0], TRORXEK_RECALL))
      {
        act("&+yYou call $p &n&+yfor the forest path home..\n"
            ...
            FALSE, ch, obj, 0, TO_CHAR);
        act("&+gThen the grass dimmishes, you find yourself back in your home.", FALSE, ch, obj, 0,
            TO_CHAR);
        act("&n$n &+ymutters something to $s $p.\n"
            "&+gTall grass suddenly appears beneath $n, and grows up around $n within seconds!",
            FALSE, ch, obj, 0, TO_ROOM);
        act("&+gThe grass consumes $n, which then vanishes along with $n into thin air..", FALSE,
            ch, obj, 0, TO_ROOM);
...
int Amaukekel(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  {
    ClearObjEvents(obj);
    initializeObjectFeature(ITEM_WEAPON, obj, AMAUKEKEL_DIMENTIONAL_SHIFT, PULSE_WEEK, AmaukekelD);
    initializeObjectFeature(ITEM_WEAPON, obj, AMAUKEKEL_RESSURECTION, PULSE_DAY, AmaukekelR);
    initializeObjectFeature(ITEM_WEAPON, obj, AMAUKEKEL_SUNBLAST, PULSE_HOUR, AmaukekelS);
  ...
  if (calltype == PROC_SPECIAL_ID)
  {
    ...
    specialProcedureIdentifyObject(obj, ch, calltype, arg, ID_CLASS_CLERIC, 0, 0, idString);
    return TRUE;
  }
  ...
  {
    /* Recharge spell effect from event */
    if (rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, AMAUKEKEL_DIMENTIONAL_SHIFT,
                              AmaukekelD) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, AMAUKEKEL_RESSURECTION,
                              AmaukekelR) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, AMAUKEKEL_SUNBLAST,
                              AmaukekelS))
      ;
    return FALSE;
    ...
    if (owner)
      /* Now we check to be sure the guy wielding this falls under the */
      /* restrictions we have set */
      if (checkObjectRestrictions(ITEM_WEAPON, obj, owner, OBJ_RESTRICT_CLASS, OBJ_RESTRICT_CLERIC))
      {
        performRestrictionPenalty(ITEM_WEAPON, obj, owner, OBJ_BURN);
        return TRUE;
      }
    ...
    if (!strcmp(fullword, AMAUKEKEL_DIMENTIONAL_SHIFT_CMD))
    {
      if (!IS_SET(obj->value[0], AMAUKEKEL_DIMENTIONAL_SHIFT))
      {
        act("&+YYou call $p &n&+yfor the Sunlit Path..\n"
            ...
            FALSE, ch, obj, 0, TO_CHAR);
        act("&+rThe power of Amaukekel released warms the rod to almost unbearable levels..", FALSE,
            ch, obj, 0, TO_CHAR);
        act("$n &+Ymutters something to &s $p.\n"
            "&+YMagical energy courses through $s's Rod, flowing into the air in bright waves.",
            FALSE, ch, obj, 0, TO_ROOM);
        act("$n's $p &n&+rglows bright red with heat from the magical discharge..", FALSE, ch, obj,
            0, TO_ROOM);
    ...
    else if (!strcmp(word, AMAUKEKEL_RESSURECTION_CMD))
    {
      if (!IS_SET(obj->value[0], AMAUKEKEL_RESSURECTION))
      {
        ...
        act("&+yYou call $p &n&+yfor the restoration of life..\n"
            ...
            FALSE, ch, obj, 0, TO_CHAR);
        act("&+WThe white magical energy finally settles overtop of $p.", FALSE, ch, corpse, 0,
            TO_CHAR);
        act("&n$n&+y mutters something to $s $p.\n"
...
int Gesen(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  if (!number(0, 30))
  {
    act("&+CThe axe glows and flies from your hand, striking &n$N&+C with a CRASH!\n"
        "&+CTwirling wildly, it arcs back around and returns to your hand..",
        FALSE, ch, obj, vict, TO_CHAR);
    act("&+C$n's axe glows and flies through the air, striking &n$N&+C with a CRASH!\n"
...
int Kelrarin(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  if ((GET_ALIGNMENT(ch) > 990) && (GET_HIT(ch) <= GET_MAX_HIT(ch)))
  {
    if (!number(0, 32))
    {
      act("&+WFOOOOOOOOOOOOOSH! \n$n's hammer flashes with a light so bright that everyone in the "
          "room is nearly blinded!\n"
          "&+WA massive bolt of electrical energy that looks very much like a lightning bolt "
          "erupts",
          FALSE, ch, obj, 0, TO_ROOM);
      ...
      act("The force of the blast sends everyone, including $N, tumbling to the floor.\n"
          ...
          FALSE, ch, obj, vict, TO_ROOM);
      act("&+WFOOOOOOOOOOOOOSH! \nYour hammer flashes with a light so bright that everyone in the "
          "room is nearly blinded!\n"
          "&+WA massive bolt of electrical energy that looks very much like a lightning bolt "
          "erupts",
          FALSE, ch, obj, 0, TO_CHAR);
      act("&+Wfrom the end of the hammer, striking $N dead on!\n&+BAn intense wave of Power flow "
          "into your body, you feel like a living God!\n"
          "&=LWBOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOM",
          FALSE, ch, obj, vict, TO_CHAR);
      act("The force of the blast sends everyone, including $N, tumbling to the floor.\n"
          "$N screams out in pain as the wrath of Kelrarin the Great punnishes $s!",
  ...
  if (!number(0, 28))
  {
    act("&+CThe hammer glows and flies from your hand, striking &n$N&+C with a CRASH!\n"
        "&=LWBooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooom",
        FALSE, ch, obj, vict, TO_CHAR);
    act("&+CThe blast shakes the ground and stings your ears with the roar of thunder!\n"
        "&+CTwirling wildly, it arcs back around and returns faithufll to your hand..",
        FALSE, ch, obj, vict, TO_CHAR);
    act("&+bYou feel &=LBPOWER&n&+b surge into your arms as Kelrarin's blessing flows throuth "
        "you!\n",
        FALSE, ch, obj, vict, TO_CHAR);
    act("&+C$n's hammer glows and flies through the air, striking &n$N&+C with a CRASH!\n"
        "&=LWBooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooom",
        FALSE, ch, obj, vict, TO_ROOM);
    act("&+CThe blast shakes the ground and stings your ears with the roar of thunder!\n"
...
int Fade2(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  {
    ClearObjEvents(obj);
    initializeObjectFeature(ITEM_WEAPON, obj, FADE2_BLIND, PULSE_DAY, Fade2D);
    initializeObjectFeature(ITEM_WEAPON, obj, FADE2_DARKNESS, PULSE_HOUR, Fade2C);
    initializeObjectFeature(ITEM_WEAPON, obj, FADE2_WEAKEN, PULSE_WEEK, Fade2R);
    initializeObjectFeature(ITEM_WEAPON, obj, FADE2_SHADOWPATH, PULSE_DAY, Fade2M);
  ...
  {
    /* Recharge spell effect from event */
    if (rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, FADE2_BLIND, Fade2D) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, FADE2_DARKNESS, Fade2C) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, FADE2_WEAKEN, Fade2R) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, FADE2_SHADOWPATH, Fade2M))
      return FALSE;
  ...
  if (word && ((cmd == CMD_SAY) || (cmd == CMD_SAY2)))
  {
    ...
    else if (!strcmp(word, FADE2_WEAKEN_CMD))
    {
      if (!IS_SET(obj->value[0], FADE2_WEAKEN))
      {
        ...
        {
          act("$n &+Lfoolishly tries to drain the life force of the high undead, &n$N&+L grins "
              "horribly.",
              FALSE, ch, obj, target, TO_ROOM);
          act("$N &+Lturns &n&+rFade&n back upon him, and sucks the life force out of &n$n "
              "&+Leasily!",
              FALSE, ch, obj, target, TO_ROOM);
          act("$n &+Lfalls to the floor, crumpled and grey, dead with the horrible expression of "
              "agony..",
              FALSE, ch, obj, target, TO_ROOM);
          act("&+LYou foolishly try to drain the life force of the high undead, who grins horribly "
              "at you.",
              FALSE, ch, obj, target, TO_CHAR);
          act("&N &+Lsucks the life force out of you through &n&+rFade&+L easily, you fall "
              "downwards..",
              FALSE, ch, obj, target, TO_CHAR);
          act("&+LYou feel your body before your mind follows, screaming into the abyss..", FALSE,
              ch, obj, target, TO_CHAR);

          GET_HIT(target) = GET_MAX_HIT(target);
          SuddenDeath(ch, target,
                      "The idiot tried to drain the life force of the high undead. Doh? o_O");
        ...
        {
          act("$n screams in pain, as the magic of &+rFade&n punnishes him with crackles of "
              "energy!",
              FALSE, ch, obj, target, TO_ROOM);
          act("&+LA deep voice in your mind booms, 'I was made for vengence against Evil ONLY!'",
              FALSE, ch, obj, target, TO_CHAR);
          act("&+WCrackles of energy bite at your arm and you scream in agony as Fade punnishes "
              "you!",
              FALSE, ch, obj, target, TO_CHAR);
        ...
        else
        {
          ...
          act("&+LBlack tendils of power emanate from your &n&+rFade&+L, streaming towards $N!\n"
              "\n&+LSwoooooooooosh\n",
              FALSE, ch, obj, target, TO_CHAR);
          act("&+LThe tendrils envelop $N&+L, who screams in agony as $S's life force is sucked "
              "away\n!"
          ...
          act("&+rRed tendrils of energy flow back out of &n$N&n&+r into &n$n&+r, who looks "
              "vitalized!",
              FALSE, ch, obj, target, TO_ROOM);
  ...
  if (IS_NPC(target))
  {
    if (!number(0, 15))
    {
      act("&+LYou feel a surge of warmth as &n&+rFade&+L feeds you $N's&+L life force!", FALSE, ch,
          obj, target, TO_CHAR);
      act("&+L$N&+L screams as $n's $p&+L draws out some of $S's&+L life force.", FALSE, ch, obj,
          target, TO_ROOM);
...
int HornOfHenekar(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  if (calltype == PROC_SPECIAL_ID)
  {
    ...
    specialProcedureIdentifyObject(obj, ch, calltype, arg, ID_CLASS_BARD, 0, 0, idString);
    return TRUE;
  }
    ...
    else if (!strcmp(word, HENEKAR_PACIFY_CMD))
    {
      if (!IS_SET(obj->value[2], HENEKAR_PACIFY))
      {
        ...
        act("&+bYou call $p &n&+bto bring peace to $N&n&+b..\n"
            "&+BWith a powerful blast, the horn jets forth energy at $N&+B, which rapidly "
            "surrounds $H!",
            FALSE, ch, obj, target, TO_CHAR);
        act("$N&+C screams in agony for a moment, unable to escape the permiating energy!\n"
            ...
            FALSE, ch, obj, target, TO_CHAR);
        act("$N&n&+b becomes completely passive, staring out into space, unmoving..", FALSE, ch,
            obj, target, TO_CHAR);
        ...
        act("$N&+C screams in agony for a moment, unable to escape the permiating energy!\n"
            "&+BThen, slowly, $S begins to calm down, stopping dead and relaxing.",
            FALSE, ch, obj, target, TO_ROOM);
        act("&n&+b becomes completely passive, staring out into space, unmoving..", FALSE, ch, obj,
            target, TO_ROOM);
      ...
      if (!IS_SET(obj->value[2], HENEKAR_MUSICPATH))
      {
        ...
        { // targetting gods gets you sent to outhouse
          act("&+bYou call $p &n&+bfor a sonic path to $N..\n"
              ...
              FALSE, ch, obj, target, TO_CHAR);
          act("&+ROh No!!! Somthing has gone horribly wrong..", FALSE, ch, obj, target, TO_CHAR);
          act("&n$n &+bmuttes something to $s $p.\n"
              "&+BA rippling wave in the air forms. \n$n steps into it, and they both vanish into "
              "silence!",
              FALSE, ch, obj, target, TO_ROOM);
...
int New_Avernus(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  if (!number(0, 10))
  {
    act("&+LYou charge and dash, evade and parry with the skill of a master warrior.", FALSE, ch,
        obj, vict, TO_CHAR);
  ...
  if (vict)
  {
    if ((!number(0, 30)) && (GET_HIT(ch) < (GET_MAX_HIT(ch) + 1000)))
    {
      ...
      act("$n's sword &+Wglows with a bright light as it bites into you.", FALSE, ch, obj, vict,
          TO_VICT);
      act("&+LYou feel your life flowing away and &+W$n&+L looks revitalized.", FALSE, ch, obj,
          vict, TO_VICT);
      GET_HIT(ch) += dam;
      GET_HIT(vict) -= (dam * 3);
      return TRUE;
...
int Kelrom(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  if (GET_RACE(vict) == RACE_ANIMAL)
  {
    ...
    act("&+cThe extreme displeasure of Pahluruk flows through you, bites at your soul,\n"
        ...
        FALSE, ch, obj, 0, TO_CHAR);
    act("&+LYou feel .. darkness. You feel .. shame. You fall into oblivion..", FALSE, ch, obj, 0,
        TO_CHAR);
...
int Doombringer(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  {
    ClearObjEvents(obj);
    initializeObjectFeature(ITEM_WEAPON, obj, DOOMY_SPHERE, PULSE_WEEK, DoombringerS);
    initializeObjectFeature(ITEM_WEAPON, obj, DOOMY_LIGHTNING, PULSE_DAY, DoombringerL);
    initializeObjectFeature(ITEM_WEAPON, obj, DOOMY_MEGADOOM, PULSE_DAY, DoombringerM);
    initializeObjectFeature(ITEM_WEAPON, obj, DOOMY_SPAZ, PULSE_HOUR, DoombringerZ);
  ...
  {
    /* Recharge spell effect from event */
    if (rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, DOOMY_SPHERE, DoombringerS) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, DOOMY_LIGHTNING,
                              DoombringerL) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, DOOMY_MEGADOOM,
                              DoombringerM) ||
        rechargeObjectFeature(ITEM_WEAPON, obj, ch, PROC_EVENT, arg, DOOMY_SPAZ, DoombringerZ))
      return FALSE;
      ...
      if (!IS_SET(obj->value[0], DOOMY_LIGHTNING))
      {
        ...
        if (GET_LEVEL(target) < 60)
        {
          act("&+LYou utter arcane words to Doombringer, which responds with a &=LWbright flash!\n"
              "&+LImmediately following the flash, twin bolts of super-heated Black Lightning "
              "stream\n"
          ...
          act("&+rYou feel drained from the expendature of energy..", FALSE, ch, obj, 0, TO_CHAR);
          GET_MOVE(ch) = 0;
          GET_MANA(ch) = 0;
        }
    ...
    else if (!strcmp(word, DOOMY_MEGADOOM_CMD))
    {
      if (!IS_SET(obj->value[0], DOOMY_MEGADOOM))
      {
        ...
        act("&+LYou utter a series of strange arcane words to mighty Doombringer..\n"
            "&+LThe sword eagerly responds as black tendrils of power flow down the blase into "
            "you!\n"
        ...
        act("$n &+Lutters a series of strange arcane words to his mighty sword, Doombringer..\n"
            ...
            "&+L$n flings about so fast you can't even watch $m, striking over and over with "
            "rage!\n"
