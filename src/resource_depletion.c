...
float get_resource_regeneration_rate(int resource_type)
{
  ...
  case RESOURCE_VEGETATION:
    base_rate = 0.12;
    break; /* Fast growing - 12% per hour */
  case RESOURCE_HERBS:
    base_rate = 0.08;
    break; /* Moderate regeneration - 8% per hour */
  case RESOURCE_WATER:
    base_rate = 0.20;
    break; /* Seasonal/weather dependent - 20% per hour when conditions right */
  case RESOURCE_GAME:
    base_rate = 0.06;
    break; /* Animals move in - 6% per hour */
  case RESOURCE_WOOD:
    base_rate = 0.02;
    break; /* Very slow regeneration - 2% per hour */
  case RESOURCE_CLAY:
    base_rate = 0.10;
    break; /* Weather dependent - 10% per hour */
  case RESOURCE_STONE:
    base_rate = 0.005;
    break; /* Extremely slow - 0.5% per hour */
  case RESOURCE_MINERALS:
    base_rate = 0.001;
    break; /* Nearly non-renewable - 0.1% per hour */
  case RESOURCE_CRYSTAL:
    base_rate = 0.0005;
    break; /* Magical regeneration - 0.05% per hour */
  case RESOURCE_SALT:
    base_rate = 0.01;
    break; /* Slow but steady - 1% per hour */
  default:
    base_rate = 0.05;
    break; /* Default moderate regeneration */
  }
...
float calculate_regeneration_amount(int resource_type, time_t last_harvest_time, int x, int y)
{
  ...
  double hours_passed =
      difftime(current_time, last_harvest_time) / 3600.0; /* Convert seconds to hours */
...
float get_resource_depletion_rate(int resource_type)
{
  ...
  {
  case RESOURCE_VEGETATION:
    return 0.08; /* Fast growing, fast depletion */
  case RESOURCE_HERBS:
    return 0.06; /* Moderate depletion */
  case RESOURCE_WATER:
    return 0.03; /* Seasonal regeneration */
  case RESOURCE_GAME:
    return 0.10; /* Mobile, can be overhunted */
  case RESOURCE_WOOD:
    return 0.04; /* Slow regeneration */
  case RESOURCE_CLAY:
    return 0.02; /* Renewable with weather */
  case RESOURCE_STONE:
    return 0.01; /* Very slow regeneration */
  case RESOURCE_MINERALS:
    return 0.005; /* Nearly non-renewable */
  case RESOURCE_CRYSTAL:
    return 0.002; /* Extremely rare regeneration */
  case RESOURCE_SALT:
    return 0.01; /* Location dependent */
  default:
    return 0.05; /* Default moderate depletion */
  }
}
                                 ...
                                 const char *description)
{
  ...
  else
  {
    ...
    log("CASCADE: %s affecting %s at (%d,%d): %.3f", description, resource_names[target_resource],
        x, y, effect_magnitude);
  }
}
...
int get_ecosystem_state(room_rnum room)
{
  ...
  if (average_health >= 0.80)
    return 0; /* Pristine */
  if (average_health >= 0.60)
    return 1; /* Healthy */
  if (average_health >= 0.40)
    return 2; /* Stressed */
  if (average_health >= 0.20)
    return 3; /* Degraded */
  return 4;   /* Collapsed */
}
...
void show_ecosystem_analysis(struct char_data *ch, room_rnum room)
{
  ...
  const char *health_color = ecosystem_state <= 1 ? "\tG" : ecosystem_state == 2 ? "\tY" : "\tR";
  send_to_char(ch, "Overall Health: %s%s\tn\r\n", health_color,
               ecosystem_state_names[ecosystem_state]);
...
void update_conservation_score(struct char_data *ch, int resource_type __attribute__((unused)),
                               bool sustainable)
{
  char query[MAX_STRING_LENGTH];
  long player_id;
  ...
  snprintf(query, sizeof(query),
           ...
           sustainable ? "sustainable_harvests" : "unsustainable_harvests", player_id,
           0.5 + score_change, score_change,
