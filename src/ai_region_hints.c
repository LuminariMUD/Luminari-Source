...
const char *ai_hint_category_names[NUM_AI_HINT_CATEGORIES] = {
    "atmosphere", "fauna",  "flora",  "geography",        "weather_influence", "resources",
    "landmarks",  "sounds", "scents", "seasonal_changes", "time_of_day",       "mystical"};
...
char *generate_ai_enhanced_description(struct ai_description_context *context)
{
  ...
  snprintf(description, sizeof(description), "This wilderness area shows signs of %s. %s",
           context->available_hints->hint_text ? context->available_hints->hint_text
                                               : "natural beauty",
...
char *generate_ai_enhanced_description(struct ai_description_context *context)
{
  ...
  regions = get_enclosing_regions(GET_ROOM_ZONE(context->room), world[context->room].coords[0],
  ...
  selected_hints =
      select_hints_for_description(filtered_hints, AI_HINTS_MAX_PER_DESCRIPTION, context);
  ...
  if (selected_hints && base_description)
  {
    enhanced_description =
        combine_hints_with_base_description(base_description, selected_hints, context);
                    ...
                    struct ai_description_context *context)
{
  ...
  snprintf(query, sizeof(query),
           "INSERT INTO ai_hint_usage_log (hint_id, room_vnum, player_id, weather_condition, "
           "season, time_of_day) "
           "VALUES (%d, %d, %ld, '%s', '%s', '%s')",
