...
const char *ecosystem_state_names[] = {"Pristine", "Healthy",   "Stressed",
                                       "Degraded", "Collapsed", "\n"};
...
void apply_cascade_effects(room_rnum room, int source_resource, int quantity)
{
  ...
  while ((row = mysql_fetch_row(result)))
  {
    ...
    else
    {
      ...
      log_cascade_effect(room, source_resource, target_resource, effect_magnitude * quantity, NULL);
    }
  }
...
void show_ecosystem_analysis(struct char_data *ch, room_rnum room)
{
  ...
  send_to_char(ch, "Overall Health: %s%s\tn (%.1f%%)\r\n",
               ...
               : ecosystem_state == ECOSYSTEM_STRESSED ? "\tY"
                                                       : "\tR",
  ...
  for (i = 0; i < NUM_RESOURCE_TYPES; i++)
  {
    ...
    send_to_char(ch, "  %s%-12s\tn: %s%s\tn (%.0f%%)\r\n", color, resource_names[i], color, status,
                 level * 100);
  }
  ...
  while ((row = mysql_fetch_row(result)))
  {
    ...
    if (target_resource >= 0 && target_resource < NUM_RESOURCE_TYPES)
    {
      ...
      send_to_char(ch, "%sHarvesting %s %s %s\tn (%.1f%% effect)\r\n", effect_color,
                   resource_names[resource_type], effect_type, resource_names[target_resource],
                        ...
                        float effect_magnitude, struct char_data *ch)
{
  ...
  snprintf(query, sizeof(query),
           ...
           "(zone_vnum, x_coord, y_coord, source_resource, target_resource, effect_magnitude, "
           "player_id) "
