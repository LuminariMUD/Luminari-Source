...
#define GET_MAP_SECTOR(x, y)                                                                       \
  (SAFE_MAP_BOUNDS(x, y) ? get_sector_type(get_elevation(NOISE_MATERIAL_PLANE_ELEV, x, y),         \
                                           get_temperature(WEATHER, x, y),                         \
                                           get_elevation(NOISE_MATERIAL_PLANE_MOISTURE, x, y))     \
                         : SECT_INSIDE)
#define GET_MAP_ELEV(x, y)                                                                         \
  (SAFE_MAP_BOUNDS(x, y) ? get_elevation(NOISE_MATERIAL_PLANE_ELEV, x, y) : 0)
#define GET_MAP_TEMP(x, y) (SAFE_MAP_BOUNDS(x, y) ? get_temperature(WEATHER, x, y) : 20)
#define GET_MAP_MOISTURE(x, y)                                                                     \
  (SAFE_MAP_BOUNDS(x, y) ? get_elevation(NOISE_MATERIAL_PLANE_MOISTURE, x, y) : 50)
...
int get_noise_seed(int layer)
{
  ...
  {
  case ELEVATION:
    return NOISE_MATERIAL_PLANE_ELEV_SEED;
  case MOISTURE:
    return NOISE_MATERIAL_PLANE_MOISTURE_SEED;
  case DISTORTION:
    return NOISE_MATERIAL_PLANE_ELEV_DIST_SEED;
  case WEATHER:
    return NOISE_WEATHER_SEED;
  case VEGETATION:
    return NOISE_VEGETATION_SEED;
  case MINERALS:
    return NOISE_MINERALS_SEED;
  case WATER_RESOURCE:
    return NOISE_WATER_RESOURCE_SEED;
  case HERBS:
    return NOISE_HERBS_SEED;
  case GAME:
    return NOISE_GAME_SEED;
  case WOOD:
    return NOISE_WOOD_SEED;
  case STONE:
    return NOISE_STONE_SEED;
  case CRYSTAL:
    return NOISE_CRYSTAL_SEED;
  default:
    return 12345;
  }
}
...
void report_progress(const char *stage, int percent)
{
  ...
  if (strcmp(last_stage, stage) != 0 || percent >= last_percent + 10)
  {
    ...
    if (percent % 20 == 0)
    {
      ...
      {
        if (STATE(d) == CON_PLAYING && d->character && GET_LEVEL(d->character) >= LVL_IMMORT)
        {
...
int is_land_tile(int x, int y)
{
  ...
  return (GET_MAP_SECTOR(x, y) != SECT_WATER_SWIM && GET_MAP_SECTOR(x, y) != SECT_WATER_NOSWIM &&
...
float get_resource_density(int x, int y, int resource_type)
{
  ...
  case 8: /* clay */
    ...
    value = pnoise(nx * 8, ny * 8, WATER_RESOURCE) * 0.6 + pnoise(nx * 10, ny * 10, MOISTURE) * 0.4;
  ...
  case 9: /* salt */
    ...
    value = -pnoise(nx * 9, ny * 9, MOISTURE) * 0.7 + pnoise(nx * 13, ny * 13, MINERALS) * 0.3;
...
{
  int layer;
  const char *layer_names[] = {"Elevation",  "Moisture", "Distortion",     "Weather",
                               "Vegetation", "Minerals", "Water Resource", "Herbs",
                               "Game",       "Wood",     "Stone",          "Crystal"};
  ...
  {
    fprintf(fp, "| %d | %s | %d | ", layer, layer < 12 ? layer_names[layer] : "Unknown",
...
void analyze_world_grid(FILE *fp, struct terrain_stats *stats)
{
  ...
  for (sector = 0; sector < NUM_ROOM_SECTORS; sector++)
  {
    ...
    {
      fprintf(fp, "| %s | %d | %.2f%% |\n", sector_types[sector], stats->sector_counts[sector],
              stats->sector_percentages[sector]);
    }
  }
...
struct landmass_info *detect_landmasses(FILE *fp)
{
    ...
    for (x = 0; x < MAP_WIDTH; x++)
    {
      if (!visited[x][y] && is_land_tile(x, y))
      {
        ...
        while (stack_top > 0)
        {
          ...
          /* Update bounds */
          if (cx < new_landmass->min_x)
            new_landmass->min_x = cx;
          if (cx > new_landmass->max_x)
            new_landmass->max_x = cx;
          if (cy < new_landmass->min_y)
            new_landmass->min_y = cy;
          if (cy > new_landmass->max_y)
            new_landmass->max_y = cy;
          ...
          for (dir = 0; dir < 4; dir++)
          {
            ...
            if (nx >= 0 && nx < MAP_WIDTH && ny >= 0 && ny < MAP_HEIGHT && !visited[nx][ny] &&
                is_land_tile(nx, ny))
            {
  ...
  for (current = landmasses; current; current = current->next)
  {
    ...
    fprintf(fp, "| %d | %d | (%d,%d) to (%d,%d) | %d m | %s | %s |\n", current->id,
            current->tile_count, current->min_x, current->min_y, current->max_x, current->max_y,
            current->peak_elevation, current->dominant_biome, classification);
  }
...
void trace_mountain_ranges(FILE *fp)
{
  ...
  {
    fprintf(fp, "| %d | %d | %d m | %d m | %s |\n", current_range->id, current_range->num_points,
            current_range->highest_peak, current_range->lowest_pass, current_range->name);
  }

  /* Output ridge points for first few ranges */
  fprintf(fp, "\n### Ridge Point Examples\n");
  int range_count = 0;
  for (current_range = ranges; current_range && range_count < 3;
...
void analyze_resource_distribution(FILE *fp)
{
  ...
  for (resource = 0; resource < num_resources; resource++)
  {
        ...
        if (density > hotspot_density[resource][9])
        {
          ...
          for (idx = 0; idx < 10; idx++)
          {
            if (density > hotspot_density[resource][idx])
            {
              ...
              {
                hotspot_density[resource][shift] = hotspot_density[resource][shift - 1];
                hotspot_x[resource][shift] = hotspot_x[resource][shift - 1];
                hotspot_y[resource][shift] = hotspot_y[resource][shift - 1];
              }
              /* Insert */
    ...
    for (x = 0; x < 10; x++)
    {
      ...
      {
        fprintf(fp, "| %d | (%d, %d) | %.1f%% | %s |\n", x + 1, hotspot_x[resource][x],
                hotspot_y[resource][x], hotspot_density[resource][x] * 100,
                sector_types[GET_MAP_SECTOR(hotspot_x[resource][x], hotspot_y[resource][x])]);
      }
    }
    fprintf(fp, "\n");
  }
  ...
  for (r1 = 0; r1 < num_resources; r1++)
  {
    for (r2 = r1 + 1; r2 < num_resources; r2++)
    {
      ...
      { /* Only show meaningful correlations */
        ...
        if (fabs(corr) > 0.7)
          strength = "Strong";
        else if (fabs(corr) > 0.5)
          strength = "Moderate";
        else
          strength = "Weak";

        fprintf(fp, "| %s | %s | %.3f | %s |\n", resource_names[r1], resource_names[r2], corr,
                strength);
      }
    }
  }
...
void analyze_spatial_relationships(FILE *fp)
{
  ...
  while ((region_row1 = mysql_fetch_row(regions_result)) && i < 20)
  {
    fprintf(fp, "| %s | %s | %s |\n", region_row1[0] ? region_row1[0] : "Unknown",
            region_row1[1] ? region_row1[1] : "0", region_row1[2] ? region_row1[2] : "0");
    i++;
  }
...
void map_transition_zones(FILE *fp)
{
  ...
  transitions =
      (struct transition_zone *)malloc(transition_capacity * sizeof(struct transition_zone));
  if (!transitions)
  {
    ...
    for (x = 0; x < MAP_WIDTH; x += sample_rate)
    {
      ...
      for (dy = -1; dy <= 1; dy++)
      {
        for (dx = -1; dx <= 1; dx++)
        {
          ...
          /* Found a transition */
          if (current_biome != neighbor_biome && current_biome != SECT_INSIDE &&
              neighbor_biome != SECT_INSIDE)
          {
  ...
  for (i = 0; i < transition_count && shown < 20; i++)
  {
    ...
    const char *edge_name = transitions[i].edge_type == 0   ? "Sharp"
                            : transitions[i].edge_type == 1 ? "Gradual"
                                                            : "Ecotone";
  ...
  for (i = 0; i < transition_count; i++)
  {
    if (transitions[i].from_biome == SECT_MOUNTAIN || transitions[i].to_biome == SECT_MOUNTAIN)
      mountain_barriers++;
    if (transitions[i].from_biome == SECT_WATER_SWIM ||
        transitions[i].to_biome == SECT_WATER_SWIM ||
        transitions[i].from_biome == SECT_WATER_NOSWIM ||
        transitions[i].to_biome == SECT_WATER_NOSWIM)
      water_barriers++;
  }
  ...
  for (y = 0; y < MAP_HEIGHT; y += sample_rate)
  {
    for (x = 0; x < MAP_WIDTH; x += sample_rate)
    {
      if (!is_land_tile(x, y))
      {
        ...
        { /* Deep water is well below waterline */
          deep_ocean++;
        }
        else
        {
        ...
        if (has_land_neighbor && elevation > WATERLINE - 10)
...
void analyze_civilization_potential(FILE *fp)
{
  ...
  for (y = 0; y < MAP_HEIGHT; y += sample_rate)
  {
    for (x = 0; x < MAP_WIDTH; x += sample_rate)
    {
      ...
      score->overall_score =
          (score->water_access * 0.25 + score->resource_richness * 0.20 +
           (1.0 - score->terrain_difficulty) * 0.15 + score->defense_rating * 0.10 +
           score->agriculture_potential * 0.20 + score->trade_access * 0.10);
  ...
  for (i = 0; i < MIN(10, score_count); i++)
  {
    ...
    if (best_idx >= 0)
    {
      ...
      fprintf(fp, "| %d | (%d,%d) | %s | %.2f | %.2f | %.2f | %.2f | %.2f |\n", i + 1, s->x, s->y,
              biome_name, s->overall_score, s->water_access, s->resource_richness,
...
void construct_path_network_graph(FILE *fp)
{
  ...
  snprintf(query, sizeof(query), "SELECT path_type, COUNT(*) FROM path_data GROUP BY path_type");
...
void write_query_reference(FILE *fp)
{
  ...
  fprintf(fp, "| (0,0) | (%d,%d) | %.0f | Southeast |\n", MAP_WIDTH - 1, MAP_HEIGHT - 1,
          calculate_distance(0, 0, MAP_WIDTH - 1, MAP_HEIGHT - 1));
  ...
  for (i = 0; i < 7; i++)
  {
    ...
    {
      fprintf(fp, "| %s | (%d,%d) | %s | %s |\n", sector_types[biome], biome_x[biome],
              biome_y[biome], elevation_ranges[i], climate_zones[i]);
    }
  }
  ...
  for (current = landmasses; current; current = current->next)
  {
    if (current->tile_count > 10000 && current->tile_count <= 100000)
    {
      fprintf(fp, "- **Island %d**: %d tiles at (%d,%d), %s terrain\n", current->id,
              current->tile_count, (current->min_x + current->max_x) / 2,
              (current->min_y + current->max_y) / 2, current->dominant_biome);
    }
  }
  fprintf(fp, "\n");
}
...
void write_climate_analysis(FILE *fp)
{
  ...
  for (i = 0; i < 5; i++)
  {
    ...
    {
    case 0:
      fprintf(fp, "Permanent ice, tundra |\n");
      break;
    case 1:
      fprintf(fp, "Taiga, cold forests |\n");
      break;
    case 2:
      fprintf(fp, "Deciduous forests, grasslands |\n");
      break;
    case 3:
      fprintf(fp, "Warm forests, savannas |\n");
      break;
    case 4:
...
void write_resource_analysis(FILE *fp)
{
  ...
  for (i = 0; i < 10; i++)
  {
    ...
    {
    case 0:
      fprintf(fp, "Forests, Plains | Food, Materials |\n");
      break;
    case 1:
      fprintf(fp, "Mountains, Hills | Tools, Trade |\n");
      break;
    case 2:
      fprintf(fp, "Rivers, Lakes | Survival, Agriculture |\n");
      break;
    case 3:
      fprintf(fp, "Forests, Wetlands | Medicine, Alchemy |\n");
      break;
    case 4:
      fprintf(fp, "Forests, Plains | Food, Leather |\n");
      break;
    case 5:
      fprintf(fp, "Forests | Construction, Fuel |\n");
      break;
    case 6:
      fprintf(fp, "Mountains, Hills | Construction, Tools |\n");
      break;
    case 7:
      fprintf(fp, "Deep Mountains | Magic, Trade |\n");
      break;
    case 8:
      fprintf(fp, "Riverbeds, Lowlands | Pottery, Bricks |\n");
      break;
    case 9:
...
void calculate_accessibility_matrix(FILE *fp)
{
  ...
  for (i = 0; i < num_points; i++)
  {
    fprintf(fp, "| P%d (%d,%d) |", i + 1, sample_points[i][0], sample_points[i][1]);
    for (j = 0; j < num_points; j++)
    {
      ...
      {
        distances[i][j] = calculate_distance(sample_points[i][0], sample_points[i][1],
...
void calculate_exploration_coverage(FILE *fp)
{
  ...
  for (y = 0; y < MAP_HEIGHT; y += 16)
  {
    ...
    for (x = 0; x < MAP_WIDTH; x += 16)
    {
      ...
      /* Check accessibility */
      if (sector == SECT_FIELD || sector == SECT_FOREST || sector == SECT_HILLS ||
          sector == SECT_ROAD_NS || sector == SECT_ROAD_EW || sector == SECT_ROAD_INT)
      {
        accessible_tiles++;
      }

      /* Check hazards */
      if (sector == SECT_MOUNTAIN || sector == SECT_WATER_NOSWIM || sector == SECT_UNDERWATER ||
          sector == SECT_LAVA)
      {
        hazardous_tiles++;
      }
...
void calculate_fractal_dimensions(FILE *fp)
{
  ...
  if (mountain_boxes < 50)
    fprintf(fp, "Low (mostly flat)\n");
  else if (mountain_boxes < 200)
    fprintf(fp, "Moderate (rolling terrain)\n");
  else
    fprintf(fp, "High (mountainous)\n");
  fprintf(fp, "\n");
}
...
void model_economic_potential(FILE *fp)
{
    ...
    for (x = 64; x < MAP_WIDTH - 64; x += 128)
    {
      ...
      for (sy = y - 32; sy < y + 32; sy += 8)
      {
        for (sx = x - 32; sx < x + 32; sx += 8)
        {
          ...
          if (sector == SECT_ROAD_NS || sector == SECT_ROAD_EW || sector == SECT_ROAD_INT)
            trade_score += 1.0;
  ...
  for (i = 0; i < 10 && top_zones[i].total_score > 0; i++)
  {
    fprintf(fp, "| %d | (%d,%d) | %.2f | %.2f | %.2f | %.2f |\n", i + 1, top_zones[i].x,
            top_zones[i].y, top_zones[i].agriculture_score, top_zones[i].mining_score,
            top_zones[i].trade_score, top_zones[i].total_score);
  }
  ...
  if (trade_density < 0.1)
    ...
    fprintf(fp, "Moderate - regional trade\n");
  else
    fprintf(fp, "High - extensive trade network\n");
  fprintf(fp, "\n");
}
...
void write_landmasses_json(FILE *fp, struct landmass_info *landmasses)
{
  ...
  for (lm = landmasses; lm; lm = lm->next)
  {
    ...
    else if (lm->tile_count > 1000)
      classification = "island";
    else if (lm->tile_count > 100)
  ...
  for (r1 = 0; r1 < 10; r1++)
  {
    for (r2 = r1 + 1; r2 < 10; r2++)
    {
      if (fabs(correlations[r1][r2]) > 0.3)
      {
        if (!first_corr)
          fprintf(fp, ",\n");
        fprintf(fp, "      \"%s_%s\": %.3f", resource_names[r1], resource_names[r2],
                correlations[r1][r2]);
        first_corr = 0;
      }
    }
...
void calculate_dijkstra_paths(FILE *fp, int source_x, int source_y, int dest_x, int dest_y)
{
  ...
  while (search_iterations < max_iterations)
  {
    ...
    for (current_y = MAX(0, MIN(source_y, dest_y) - max_search_radius);
         current_y <= MIN(MAP_HEIGHT - 1, MAX(source_y, dest_y) + max_search_radius); current_y++)
    {
      for (current_x = MAX(0, MIN(source_x, dest_x) - max_search_radius);
           current_x <= MIN(MAP_WIDTH - 1, MAX(source_x, dest_x) + max_search_radius); current_x++)
      {
  ...
  {
    fprintf(fp,
            "Path search from (%d,%d) to (%d,%d) terminated after %d iterations (limit reached).\n",
  ...
  else if (previous_x[dest_y * MAP_WIDTH + dest_x] != -1)
  {
    ...
    fprintf(fp, "Optimal path found: %d steps, cost: %.1f (searched %d nodes)\n", path_length,
            distance[dest_y * MAP_WIDTH + dest_x], search_iterations);
...
void analyze_noise_spectrum(FILE *fp)
{
  ...
  for (freq = 0; freq < 16; freq++)
  {
    ...
    for (y = 0; y < sample_size; y += 16)
    {
      ...
      {
        amplitude[freq] += fabs(pnoise(x * frequency[freq], y * frequency[freq], ELEVATION));
      }
    }
...
void generate_wilderness_knowledge_base(const char *output_filename)
{
  ...
  mudlog(CMP, LVL_IMMORT, FALSE, "Starting wilderness knowledge base generation to %s",
         output_filename);
  WILD_DEBUG("Output file: %s", output_filename);
  WILD_DEBUG("Map dimensions: %dx%d (%d total tiles)", MAP_WIDTH, MAP_HEIGHT,
             MAP_WIDTH * MAP_HEIGHT);
