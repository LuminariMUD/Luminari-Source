...
static int weather_terrain_audio_apply_modifiers(struct spatial_context *ctx, float *range_mod,
                                                 float *clarity_mod);
...
int spatial_audio_test_thunder(int thunder_x, int thunder_y, const char *thunder_desc)
{
  ...
  for (ch = character_list; ch; ch = ch->next)
  {
    ...
    spatial_log("Testing audio for player %s at (%d, %d, %d)", GET_NAME(ch), ctx->observer_x,
