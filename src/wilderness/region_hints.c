...
char *enhance_wilderness_description_with_hints(struct char_data *ch, room_rnum room)
{
  ...
  if (!regions)
  {
    log("DEBUG: No enclosing regions found for coordinates (%d, %d)", world[room].coords[0],
        world[room].coords[1]);
    return NULL; /* No regions found, fall back to default descriptions */
  }

  log("DEBUG: Found enclosing regions for coordinates (%d, %d)", world[room].coords[0],
      world[room].coords[1]);
                               ...
                               struct description_context *context)
{
  ...
  if (IS_WILDERNESS_VNUM(GET_ROOM_VNUM(room)))
  {
    ...
    if (raw_weather >= 225)
      context->weather = 4; /* Lightning */
    else if (raw_weather >= 200)
      context->weather = 3; /* Stormy */
    else if (raw_weather >= 178)
      context->weather = 2; /* Rainy */
    else if (raw_weather > 127)
      context->weather = 1; /* Cloudy */
    else
      context->weather = 0; /* Clear */
  }
  else
  {
...
char *generate_enhanced_description(struct description_context *context)
{
  ...
  if (!base_desc)
  {
    ...
    base_desc = strdup(world[context->room].description ? world[context->room].description
...
char *combine_base_with_hints(char *base_desc, struct region_hint *hints,
                              struct description_context *context)
{
  char *combined = NULL;
  struct region_hint *current = hints;
...
char *get_hint_category_name(int category)
{
  ...
  case HINT_SCENTS:
    return "scents";
  case HINT_SEASONAL_CHANGES:
    return "seasonal_changes";
  case HINT_TIME_OF_DAY:
    return "time_of_day";
  case HINT_MYSTICAL:
    return "mystical";
  default:
