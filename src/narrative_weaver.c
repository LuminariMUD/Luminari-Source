...
int cache_keys_equal(struct hint_cache_key *key1, struct hint_cache_key *key2)
{
  return (key1->region_vnum == key2->region_vnum &&
          ...
          key1->time_category == key2->time_category && key1->season == key2->season &&
          abs((int)(key1->resource_health * 100) - (int)(key2->resource_health * 100)) <=
              5); /* 5% tolerance */
}
...
struct region_hint *get_cached_hints(struct hint_cache_key *key)
{
  ...
  while (entry)
  {
    if (cache_keys_equal(&entry->key, key))
    {
      ...
      if ((current_time - entry->cache_time) <= HINT_CACHE_TTL)
      {
        narrative_debug_log(2, "Cache HIT for region %d, weather %d, time %d", key->region_vnum,
                            key->weather_condition, key->time_category);
  ...
  narrative_debug_log(2, "Cache MISS for region %d, weather %d, time %d", key->region_vnum,
                      key->weather_condition, key->time_category);
  return NULL;
}
...
void cache_hints(struct hint_cache_key *key, struct region_hint *hints)
{
  ...
  narrative_debug_log(2, "Cached %d hints for region %d (total cache entries: %d)", count,
                      key->region_vnum, hint_cache_entries);
}
...
int get_season_from_time(void)
{
  ...
  if (month >= 2 && month <= 4)
    return SEASON_SPRING; /* Mar-May */
  if (month >= 5 && month <= 7)
    return SEASON_SUMMER; /* Jun-Aug */
  if (month >= 8 && month <= 10)
    return SEASON_AUTUMN; /* Sep-Nov */
...
double get_seasonal_weight_for_hint(const char *seasonal_json, int season)
{
  ...
  {
  case SEASON_SPRING:
    return parse_json_double_value(seasonal_json, "spring");
  case SEASON_SUMMER:
    return parse_json_double_value(seasonal_json, "summer");
  case SEASON_AUTUMN:
    return parse_json_double_value(seasonal_json, "autumn");
  case SEASON_WINTER:
    return parse_json_double_value(seasonal_json, "winter");
  default:
    return 1.0;
  }
}
                                ...
                                const char *key_characteristics)
{
  ...
  if (!key_characteristics || !hint_text)
    return weight;
  ...
  case HINT_FLORA:
    ...
    {
      if (strstr(hint_text, "moss") || strstr(hint_text, "lichen") || strstr(hint_text, "fern") ||
          strstr(hint_text, "green"))
      {
                         ...
                         const char *key_characteristics)
{
  ...
  for (i = 0; i < count && i < 20; i++)
  {
    ...
    double mood_weight = get_mood_weight_for_hint(hints[hint_index].hint_category,
                                                  hints[hint_index].hint_text, key_characteristics);
...
double calculate_weather_relevance_for_hint(struct region_hint *hint, int weather_value)
{
  ...
  if (weather_value < 64)
  {
    ...
    if (strstr(hint_text, "shadow") || strstr(hint_text, "mist") || strstr(hint_text, "fog") ||
        strstr(hint_text, "gloom") || strstr(hint_text, "murk") || strstr(hint_text, "obscur"))
    {
  ...
  else if (weather_value < 192)
  {
    if (strstr(hint_text, "rain") || strstr(hint_text, "wet") || strstr(hint_text, "damp") ||
        ...
        strstr(hint_text, "moisture") || strstr(hint_text, "glistening") ||
        strstr(hint_text, "slick"))
    {
  ...
  else
  {
    if (strstr(hint_text, "wind") || strstr(hint_text, "storm") || strstr(hint_text, "tempest") ||
        strstr(hint_text, "dramatic") || strstr(hint_text, "powerful") ||
        strstr(hint_text, "fierce") || strstr(hint_text, "crash") || strstr(hint_text, "roar") ||
        strstr(hint_text, "thunder") || strstr(hint_text, "lightning") ||
        strstr(hint_text, "torrent") || strstr(hint_text, "lash"))
    {
                                       ...
                                       int region_vnum)
{
  ...
  { /* Within 30% of boundary */
    int i;
    for (i = 0; i < hint_count; i++)
    {
      /* Reduce highly region-specific hints near boundaries */
      if (hints[i].hint_category == HINT_MYSTICAL || hints[i].hint_category == HINT_LANDMARKS ||
...
float calculate_regional_resource_health(int x, int y, int radius)
{
  ...
  { /* Sample every 2 coordinates for performance */
                                         ...
                                         float resource_health)
{
  ...
  else if (resource_health <= 0.3)
  {
    for (i = 0; i < hint_count; i++)
    {
      ...
      {
        hints[i].contextual_weight *=
            (1.0 + 0.5 * (1.0 - resource_health)); /* Up to 50% boost when very depleted */
      }
  ...
  else
  {
    ...
    for (i = 0; i < hint_count; i++)
    {
      ...
      if (strstr(hint_text, "lush") || strstr(hint_text, "abundant") ||
          strstr(hint_text, "thriving"))
      {
        hints[i].contextual_weight *= (1.0 + 0.1 * abundance_factor); /* Up to 10% boost */
      }
      if (strstr(hint_text, "sparse") || strstr(hint_text, "struggling") ||
          strstr(hint_text, "scarce"))
      {
                                                           ...
                                                           int *transition_count)
{
  ...
  *transition_count = 0;
  ...
  for (i = 0; i < num_adjacent && count < capacity; i++)
  {
    ...
    double influence =
        calculate_regional_influence(x, y, region_center_x, region_center_y, max_search_radius);
    ...
    if (influence > 0.1)
    {
      ...
      transitions[count].characteristics =
          load_region_characteristics(transitions[count].region_vnum);
      transitions[count].hints = NULL;
      transitions[count].hint_count = 0;
      count++;

      narrative_debug_log(2, "Added adjacent region %d with influence %.2f", adjacent_regions[i],
                          influence);
    }
  }
                                       ...
                                       int transition_count)
{
  ...
  for (i = 0; i < hint_count; i++)
  {
    ...
    {
      if (!transitions[j].characteristics)
        continue;

      /* Get mood weight for this hint from this region */
      double mood_weight = get_mood_weight_for_hint(hints[i].hint_category, hints[i].hint_text,
                                         ...
                                         const char *regional_characteristics)
{
  ...
  if (hint->time_of_day_weight && strlen(hint->time_of_day_weight) > 0)
  {
    ...
    {
    case SUN_DARK:
      time_category_str = "night";
      break;
    case SUN_RISE:
      time_category_str = "dawn";
      break;
    case SUN_LIGHT:
      time_category_str = "day";
      break;
    case SUN_SET:
      time_category_str = "dusk";
      break;
    default:
      time_category_str = "day";
      break;
  ...
  mood_multiplier =
      get_mood_weight_for_hint(hint->hint_category, hint->hint_text, regional_characteristics);

  /* Combine all factors with diminishing returns to prevent extreme scores */
  double combined_score = base_weight * (0.8 + 0.2 * seasonal_multiplier) *
                          (0.8 + 0.2 * time_multiplier) * (0.8 + 0.2 * weather_multiplier) *
                          mood_multiplier;
                                    ...
                                    const char *regional_characteristics)
{
  ...
  for (i = 0; i < count && i < 20; i++)
  {
    int hint_index = hint_indices[i];
    weights[i] =
        calculate_comprehensive_relevance(&hints[hint_index], context, regional_characteristics);
    total_weight += weights[i];

    narrative_debug_log(2, "Hint %d relevance score: %.3f (base=%.2f, category=%d)", hint_index,
                        weights[i], hints[hint_index].contextual_weight,
                        hints[hint_index].hint_category);
  }
...
static const char *mood_indicators[] = {
    "mysterious", "mystical",    "enigmatic", "shadowy",        "hidden",  "peaceful",
    "tranquil",   "serene",      "calm",      "gentle",         "ominous", "foreboding",
    "dark",       "threatening", "menacing",  "vibrant",        "lively",  "energetic",
    "bustling",   "dynamic",     "ethereal",  "otherworldly",   "sacred",  "timeless",
    "ancient",    "luminescent", "glowing",   "bioluminescent", "emerald", NULL};

/* Action verb patterns for dynamic element detection */
static const char *action_verbs[] = {
    "whisper", "murmur",  "rustle", "sway",    "dance",   "tower",   "loom",   "stretch",
    "reach",   "rise",    "flow",   "cascade", "trickle", "babble",  "gurgle", "call",
    "cry",     "sing",    "chirp",  "echo",    "glow",    "emanate", "pulse",  "shimmer",
    "sparkle", "envelop", "drape",  "carpet",  "muffle",  "filter",  NULL};
...
static const struct vocabulary_mapping poetic_mappings[] = {{"trees", "graceful sentinels"},
                                                            {"forest", "sylvan realm"},
...
static const struct vocabulary_mapping mysterious_mappings[] = {{"trees", "shadowed sentries"},
...
static const struct vocabulary_mapping dramatic_mappings[] = {{"trees", "towering giants"},
...
static const struct vocabulary_mapping pastoral_mappings[] = {{"trees", "gentle guardians"},
...
static const struct vocabulary_mapping practical_mappings[] = {{"trees", "timber stands"},
                                                               {"forest", "wooded area"},
                                                               {"light", "illumination"},
                                                               {"wind", "air current"},
                                                               {"water", "water source"},
...
const struct vocabulary_mapping *get_style_vocabulary(int style)
{
  ...
  case STYLE_MYSTERIOUS:
    return mysterious_mappings;
  case STYLE_DRAMATIC:
    return dramatic_mappings;
  case STYLE_PASTORAL:
    return pastoral_mappings;
  case STYLE_PRACTICAL:
    return practical_mappings;
  default:
    return poetic_mappings;
  }
}
...
static const char *poetic_adjectives[] = {"ethereal", "luminous",   "graceful",  "sublime",
                                          "gossamer", "shimmering", "melodious", "enchanting",
                                          "serene",   "mystical",   NULL};

static const char *poetic_verbs[] = {"whisper", "dance",   "caress", "embrace", "weave", "flutter",
                                     "shimmer", "glisten", "murmur", "sigh",    NULL};

static const char *mysterious_adjectives[] = {"enigmatic", "shadowy",   "veiled",  "hidden",
                                              "ancient",   "forgotten", "elusive", "secretive",
                                              "cryptic",   "arcane",    NULL};

static const char *mysterious_verbs[] = {"lurk",    "conceal", "shroud",  "mask",    "veil", "hint",
                                         "suggest", "cloak",   "obscure", "whisper", NULL};

static const char *dramatic_adjectives[] = {"towering", "imposing",     "magnificent", "commanding",
                                            "majestic", "overwhelming", "thunderous",  "colossal",
                                            "fierce",   "tempestuous",  NULL};

static const char *dramatic_verbs[] = {"loom", "dominate", "tower", "command",   "surge", "thunder",
                                       "roar", "clash",    "blaze", "overwhelm", NULL};

static const char *pastoral_adjectives[] = {"gentle", "peaceful", "tranquil",   "soft",
                                            "tender", "warm",     "comforting", "welcoming",
                                            "mild",   "soothing", NULL};

static const char *pastoral_verbs[] = {"nestle",  "cradle",  "embrace", "shelter",
                                       "comfort", "nourish", "protect", "welcome",
                                       "soothe",  "calm",    NULL};

static const char *practical_adjectives[] = {
    "sturdy",    "solid",     "reliable",        "functional", "useful", "durable",
    "efficient", "practical", "straightforward", "clear",      NULL};

static const char *practical_verbs[] = {"support", "provide",  "offer",   "supply",
                                        "serve",   "function", "operate", "work",
                                        "sustain", "maintain", NULL};
...
static const char *atmospheric_transitions[] = {"Meanwhile, ",
                                                "Nearby, ",
                                                "Around you, ",
                                                "In the distance, ",
                                                "Overhead, ",
...
static const char *sensory_transitions[] = {
    "Here, ",   "Softly, ",  "All around, ", "Quietly, ",         "In this place, ",
    "Gently, ", "Faintly, ", "Subtly, ",     "Through the air, ", "From every direction, ",
    "",         NULL};

static const char *mysterious_transitions[] = {"Mysteriously, ",
                                               "In shadows, ",
                                               "Subtly, ",
                                               "Eerily, ",
                                               "With ancient presence, ",
                                               "Enigmatically, ",
...
static const char *dramatic_transitions[] = {"Majestically, ",
                                             "Boldly, ",
                                             "Powerfully, ",
                                             "Impressively, ",
                                             "With great presence, ",
...
static const char *pastoral_transitions[] = {"Peacefully, ",
                                             "Gently, ",
                                             "Serenely, ",
                                             "Tenderly, ",
                                             "With quiet grace, ",
                                             ...
                                             "Soothingly, ",
                                             "With natural beauty, ",
                                             "In tranquil abundance, ",
                                             "Calmly, ",
                                             "",
                                             NULL};

static const char *poetic_transitions[] = {"Gracefully, ",
                                           "Like a melody, ",
                                           "In ethereal beauty, ",
                                           "With flowing elegance, ",
                                           "As if dancing, ",
                                           "In perfect harmony, ",
                                           "With artistic flair, ",
                                           "Like brushstrokes on canvas, ",
                                           "In sublime arrangement, ",
                                           "Rhythmically, ",
                                           "",
                                           NULL};

/* Context-specific transitions for better flow */
static const char *temporal_transitions[] = {"As time passes, ",
...
static const char *spatial_transitions[] = {
    "To the north, ",    "Southward, ", "Eastward, ",      "Westward, ",       "Above, ", "Below, ",
    "At ground level, ", "Higher up, ", "In the depths, ", "At the surface, ", "",        NULL};

static const char *weather_transitions[] = {"Despite the weather, ",
                                            "Enhanced by conditions, ",
                                            "Under these skies, ",
                                            "In this climate, ",
                                            "Blessed by nature, ",
...
int convert_style_string_to_int(const char *style_str)
{
  ...
  if (strcmp(style_str, "poetic") == 0)
    return STYLE_POETIC;
  if (strcmp(style_str, "practical") == 0)
    return STYLE_PRACTICAL;
  if (strcmp(style_str, "mysterious") == 0)
    return STYLE_MYSTERIOUS;
  if (strcmp(style_str, "dramatic") == 0)
    return STYLE_DRAMATIC;
  if (strcmp(style_str, "pastoral") == 0)
    return STYLE_PASTORAL;
...
const char **get_style_adjectives(int style)
{
  ...
  {
  case STYLE_POETIC:
    return poetic_adjectives;
  case STYLE_MYSTERIOUS:
    return mysterious_adjectives;
  case STYLE_DRAMATIC:
    return dramatic_adjectives;
  case STYLE_PASTORAL:
    return pastoral_adjectives;
  case STYLE_PRACTICAL:
    return practical_adjectives;
  default:
    return poetic_adjectives;
  }
}
...
const char **get_style_verbs(int style)
{
  ...
  case STYLE_POETIC:
    return poetic_verbs;
  case STYLE_MYSTERIOUS:
    return mysterious_verbs;
  case STYLE_DRAMATIC:
    return dramatic_verbs;
  case STYLE_PASTORAL:
    return pastoral_verbs;
  case STYLE_PRACTICAL:
    return practical_verbs;
  default:
    return poetic_verbs;
  }
}
                                                      ...
                                                      const char *time, int region_vnum)
{
  ...
  for (i = 0; hints[i].hint_text; i++)
  {
    ...
    narrative_debug_log(2, "Processing hint %d: '%.50s...'", i,
                        text); // Extract mood indicators with style preference
    ...
    if (!elements->temporal_aspects)
    {
      ...
      // Check for time-of-day keywords
               ...
               (strcmp(time, "night") == 0 &&
                (strstr(text, "moonlight") || strstr(text, "starlight") ||
...
struct description_components *parse_description_components(const char *base_description)
{
  ...
  if (sentence)
  {
    ...
    if (strstr(sentence, "trees") || strstr(sentence, "forest") || strstr(sentence, "hills") ||
        strstr(sentence, "mountains"))
    {
      components->primary_nouns = strdup("trees"); // Simplified for now
    }

    if (strstr(sentence, "tower") || strstr(sentence, "rise") || strstr(sentence, "stretch") ||
        strstr(sentence, "form"))
    {
  ...
  if (strcmp(target_mood, "ethereal") == 0)
  {
    ...
    if (desc->opening_imagery)
    {
      ...
      if (temp)
      {
        ...
        char *step1 =
            replace_string_safe(temp, "oak trees", "ancient trees draped in thick, velvety moss");
        ...
        char *step4 =
            replace_string_safe(step3, "canopy", "emerald canopy of moss-draped branches");
    ...
    if (desc->opening_imagery && strstr(desc->opening_imagery, "tall"))
    {
      ...
      if (new_imagery)
      {
        ...
        if (pos)
        {
          ...
          *pos = '\0';
          snprintf(temp, sizeof(temp), "%sancient, shadow-wreathed%s", new_imagery, pos + 4);
          free(desc->opening_imagery);
    ...
    if (desc->opening_imagery && strstr(desc->opening_imagery, "dense"))
    {
      ...
      if (new_imagery)
      {
        ...
        if (pos)
        {
          char temp[MAX_STRING_LENGTH];
          *pos = '\0';
          snprintf(temp, sizeof(temp), "%stranquil%s", new_imagery, pos + 5);
          free(desc->opening_imagery);
          desc->opening_imagery = strdup(temp);
        }
        free(new_imagery);
...
void inject_dynamic_elements(struct description_components *desc, const char *active_elements)
{
  ...
  else if (strcmp(active_elements, "whisper") == 0 && strstr(desc->descriptive_verbs, "tower"))
  {
    ...
    if (enhanced_verbs)
    {
      snprintf(enhanced_verbs, strlen(desc->descriptive_verbs) + 50, "whisper and %s",
               desc->descriptive_verbs);
      free(desc->descriptive_verbs);
      desc->descriptive_verbs = enhanced_verbs;
    }
  ...
  else if (strcmp(active_elements, "sway") == 0)
  {
    ...
    if (enhanced_verbs)
    {
      snprintf(enhanced_verbs, strlen(desc->descriptive_verbs) + 50, "sway as they %s",
               desc->descriptive_verbs);
      free(desc->descriptive_verbs);
      desc->descriptive_verbs = enhanced_verbs;
    }
  ...
  else if (strcmp(active_elements, "cascade") == 0)
  {
    ...
    if (enhanced_verbs)
    {
      snprintf(enhanced_verbs, strlen(desc->descriptive_verbs) + 50, "cascade as they %s",
               desc->descriptive_verbs);
      free(desc->descriptive_verbs);
      desc->descriptive_verbs = enhanced_verbs;
    }
 ...
 */
void inject_temporal_and_sensory_elements(struct description_components *desc,
                                          const char *temporal_aspects, const char *sensory_details)
{
  if (!desc)
    return;
  ...
  narrative_debug_log(
      2,
      "inject_temporal_and_sensory_elements called - opening_imagery=%p, temporal=%s, sensory=%s",
      desc->opening_imagery, temporal_aspects ? "present" : "none",
      sensory_details ? "present" : "none");
  ...
  if (temporal_aspects && desc->opening_imagery)
  {
    ...
    if (temp_buffer)
    {
      ...
      narrative_debug_log(2, "Created new opening_imagery=%p, freeing temp_buffer=%p",
  ...
  if (components->descriptive_verbs && strstr(components->descriptive_verbs, "whisper"))
  {
    ...
    if (verb_pos)
    {
      ...
      *verb_pos = '\0';
      ...
      snprintf(temp_buffer, sizeof(temp_buffer), "%s%s%s", before, components->descriptive_verbs,
               after);
...
void free_narrative_elements(struct narrative_elements *elements)
{
  ...
  if (elements->dominant_mood)
    free(elements->dominant_mood);
  if (elements->primary_imagery)
    free(elements->primary_imagery);
  if (elements->active_elements)
    free(elements->active_elements);
  if (elements->sensory_details)
    free(elements->sensory_details);
  if (elements->temporal_aspects)
    free(elements->temporal_aspects);
...
int get_weather_code_from_string(const char *weather_condition)
{
  if (!weather_condition)
    return 0;
  if (strcmp(weather_condition, "clear") == 0)
    return 1;
  if (strcmp(weather_condition, "cloudy") == 0)
    return 2;
...
int get_time_code_from_string(const char *time_category)
{
  if (!time_category)
    return 0;
  if (strcmp(time_category, "dawn") == 0)
    return 1;
  if (strcmp(time_category, "morning") == 0)
    return 2;
  if (strcmp(time_category, "midday") == 0)
    return 3;
  if (strcmp(time_category, "afternoon") == 0)
    return 4;
  if (strcmp(time_category, "dusk") == 0)
    return 5;
  if (strcmp(time_category, "evening") == 0)
    return 6;
  if (strcmp(time_category, "night") == 0)
    return 7;
  if (strcmp(time_category, "midnight") == 0)
    return 8;
  return 0; /* Unknown */
}
...
int get_hint_category_from_string(const char *category_str)
{
  ...
  if (strcmp(category_str, "atmosphere") == 0)
    return HINT_ATMOSPHERE;
  if (strcmp(category_str, "fauna") == 0)
    return HINT_FAUNA;
  if (strcmp(category_str, "flora") == 0)
    return HINT_FLORA;
  if (strcmp(category_str, "geography") == 0)
    return HINT_GEOGRAPHY;
  if (strcmp(category_str, "weather_influence") == 0)
    return HINT_WEATHER_INFLUENCE;
  if (strcmp(category_str, "resources") == 0)
    return HINT_RESOURCES;
  if (strcmp(category_str, "landmarks") == 0)
    return HINT_LANDMARKS;
  if (strcmp(category_str, "sounds") == 0)
    return HINT_SOUNDS;
  if (strcmp(category_str, "scents") == 0)
    return HINT_SCENTS;
  if (strcmp(category_str, "seasonal_changes") == 0)
    return HINT_SEASONAL_CHANGES;
  if (strcmp(category_str, "time_of_day") == 0)
    return HINT_TIME_OF_DAY;
  if (strcmp(category_str, "mystical") == 0)
    return HINT_MYSTICAL;
                                ...
                                const char *time_category, int x, int y)
{
  ...
  env_context.weather =
      wilderness_weather_raw; /* Use raw weather value for intensity calculations */
  ...
  log("DEBUG: Environmental context - wilderness_weather=%d (%s), time_of_day=%d, season=%d, "
      "coords=(%d,%d)",
  ...
  {
    int selected_index = select_contextual_weighted_hint(hints, atmosphere_hints, atmosphere_count,
                                                         &env_context, regional_characteristics);
  ...
  if (!weather_added && env_context.weather > 32)
  { // Only add weather effects if significant weather
    ...
    for (i = 0; hints && hints[i].hint_text && weather_count < 20; i++)
    {
      if (hints[i].hint_category == HINT_WEATHER_INFLUENCE ||
          hints[i].hint_category == HINT_SOUNDS || hints[i].hint_category == HINT_ATMOSPHERE ||
          hints[i].hint_category == HINT_FLORA)
      {
        ...
        if (!already_used)
        {
          ...
          double weather_relevance =
              calculate_weather_relevance_for_hint(&hints[i], env_context.weather);
          if (weather_relevance > 1.2)
          { // Only include hints that are enhanced by weather
    ...
    if (weather_count > 0)
    {
      int selected_index = select_contextual_weighted_hint(hints, weather_hints, weather_count,
                                                           &env_context, regional_characteristics);
      ...
      log("DEBUG: Enhanced weather hint added (weather=%d, relevance=%.2f): %s",
          env_context.weather,
          calculate_weather_relevance_for_hint(&hints[selected], env_context.weather),
  ...
  if (!wildlife_added &&
      (strcmp(time_category, "morning") == 0 || strcmp(time_category, "evening") == 0 ||
       strcmp(time_category, "night") == 0))
  {
    ...
    if (strcmp(time_category, "night") != 0 || (rand() % 2 == 0))
    {
      ...
      if (fauna_count > 0)
      {
        int selected_index = select_contextual_weighted_hint(
            hints, fauna_hints, fauna_count, &env_context, regional_characteristics);
  ...
  { // 66% chance
    ...
    if (flora_count > 0)
    {
      int selected_index = select_contextual_weighted_hint(hints, flora_hints, flora_count,
                                                           &env_context, regional_characteristics);
  ...
  { // 50% chance
    ...
    if (sensory_count > 0)
    {
      int selected_index = select_contextual_weighted_hint(hints, sensory_hints, sensory_count,
                                                           &env_context, regional_characteristics);
  ...
  { // 50% chance
    ...
    if (seasonal_count > 0)
    {
      int selected_index = select_contextual_weighted_hint(hints, seasonal_hints, seasonal_count,
                                                           &env_context, regional_characteristics);
  ...
  if (sentence_count < 5 && (strcmp(time_category, "evening") == 0 ||
                             strcmp(time_category, "morning") == 0 || (rand() % 3 == 0)))
  { // Always for transitions, 33% for others
    int time_hints[10];
    int time_count = 0;
    int already_used, j;
    ...
    if (time_count > 0)
    {
      int selected_index = select_contextual_weighted_hint(hints, time_hints, time_count,
                                                           &env_context, regional_characteristics);
  ...
  { // 33% chance
    ...
    if (mystical_count > 0)
    {
      int selected_index = select_contextual_weighted_hint(hints, mystical_hints, mystical_count,
                                                           &env_context, regional_characteristics);
                                          ...
                                          zone_rnum zone, int x, int y)
{
  ...
  struct regional_transition *transitions =
      calculate_regional_transitions(x, y, 10, &transition_count);
  ...
  enhanced_desc = layer_hints_on_base_description(base_description, hints, weather_condition,
                                                  time_category, x, y);
                                      ...
                                      int x, int y)
{
  ...
  {
    log("DEBUG: layer_hints_on_base_description called with null parameters - base_description: "
        "%s, hints: %s",
  ...
  narrative_debug_log(1, "Starting semantic integration for location (%d, %d) with %d hints", x, y,
                      hint_count);
  ...
  narrative_debug_log(1, "Applying semantic integration with weight %.2f",
                      elements->integration_weight);
  ...
  if (elements->temporal_aspects || elements->sensory_details)
  {
    ...
    inject_temporal_and_sensory_elements(components, elements->temporal_aspects,
                                         elements->sensory_details);
  }
...
char *simple_hint_layering(char *base_description, struct region_hint *hints, int x, int y)
{
  ...
  if (fauna_count > 0 && (rand() % 3 != 0))
  { // 66% chance to include fauna
    int selected_index = select_contextual_weighted_hint(hints, fauna_hints, fauna_count,
                                                         &env_context, regional_characteristics);
    int selected = fauna_hints[selected_index];
    if (strlen(hint_additions) > 0)
      safe_strcat(hint_additions, " ");
  ...
  log("DEBUG: Simple layering with regional weighting applied %d hints to base description",
      used_count);
...
char *create_unified_wilderness_description(zone_rnum zone, int x, int y)
{
  ...
  unified_description =
      weave_unified_description(NULL, hints, weather_condition, time_category, x, y);
                                              ...
                                              int x, int y)
{
  ...
  narrative_debug_log(1, "No regional enhancement available, using base description for (%d, %d)",
                      x, y);
  return base_desc;
}
