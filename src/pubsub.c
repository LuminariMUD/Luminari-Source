                        ...
                        const char *creator_name)
{
  ...
  pubsub_info("Created topic '%s' (ID: %d) by player %s", name, topic->topic_id,
...
int pubsub_delete_topic_by_name(const char *name, const char *deleter_name)
{
  ...
  snprintf(query, sizeof(query), "DELETE FROM pubsub_topics WHERE topic_name = '%s'", escaped_name);

  if (mysql_query(conn, query))
  {
    pubsub_error("Failed to delete topic '%s' from database: %s", name, mysql_error(conn));
    return PUBSUB_ERROR_DATABASE;
  }
  ...
  if (topic->creator_name)
    free(topic->creator_name);
  free(topic);
...
int pubsub_subscribe(struct char_data *ch, int topic_id, const char *handler)
{
  ...
  if (topic->max_subscribers > 0 && topic->subscriber_count >= topic->max_subscribers)
  {
  ...
  pubsub_info("Player %s subscribed to topic '%s' (ID: %d)", GET_NAME(ch), topic->name, topic_id);
                   ...
                   int priority)
{
  ...
  pubsub_info("Published message to topic '%s' (ID: %d), queued for %d players", topic->name,
              topic_id, processed);
                                    ...
                                    int priority)
{
  ...
  snprintf(spatial_data, sizeof(spatial_data), "%d,%d,%d,%d", source_x, source_y, source_z,
           max_distance);
  ...
  for (target = character_list; target; target = target->next)
  {
    ...
    distance = sqrt(pow(X_LOC(target) - source_x, 2) + pow(Y_LOC(target) - source_y, 2) +
    ...
    {
      pubsub_debug("Player %s at distance %.1f (too far from max %d)", GET_NAME(target), distance,
                   max_distance);
    }
  }
  ...
  pubsub_info("Published wilderness audio from (%d,%d,%d), delivered to %d players", source_x,
              source_y, source_z, processed);
...
int pubsub_call_handler(struct char_data *ch, struct pubsub_message *msg, const char *handler_name)
{
  ...
  {
    handler->usage_count++;
    pubsub_debug("Handler '%s' executed successfully for player %s", handler_name, GET_NAME(ch));
  }
  else
  {
    pubsub_error("Handler '%s' failed for player %s: %s", handler_name, GET_NAME(ch),
...
const char *pubsub_error_string(int error_code)
{
  ...
  case PUBSUB_ERROR_DATABASE:
    return "Database error";
  case PUBSUB_ERROR_MEMORY:
    return "Memory allocation error";
  case PUBSUB_ERROR_NOT_FOUND:
    return "Not found";
  ...
  case PUBSUB_ERROR_TOPIC_FULL:
    return "Topic at subscriber limit";
  case PUBSUB_ERROR_HANDLER_NOT_FOUND:
    return "Handler not found";
  case PUBSUB_ERROR_INVALID_MESSAGE:
    return "Invalid message";
  case PUBSUB_ERROR_QUEUE_INIT:
    return "Queue initialization failed";
  case PUBSUB_ERROR_QUEUE_FULL:
    return "Message queue is full";
  case PUBSUB_ERROR_QUEUE_DISABLED:
    return "Queue processing disabled";
  case PUBSUB_ERROR_INVALID_PARAMETER:
    return "Invalid parameter";
  default:
...
void pubsub_process_message_queue(void)
{
  ...
  pulse_count++;
  if (pulse_count < 3)
  { /* Process every 3 pulses (~0.75 seconds) */
    return;
  }
  pulse_count = 0;
...
{
  return (message_type >= PUBSUB_MESSAGE_TYPE_SIMPLE && message_type <= PUBSUB_MESSAGE_TYPE_STATUS);
}
                                             ...
                                             int message_category, int priority)
{
  ...
  {
    pubsub_error("pubsub_create_message: Invalid type/category combination %d/%d", message_type,
                 message_category);
    return NULL;
  }
...
void pubsub_list_topics_for_player(struct char_data *ch)
{
  ...
  for (topic = topic_list; topic; topic = topic->next)
  {
    if (topic->is_active)
    {
      ...
      send_to_char(ch, "%-20s %-40s %-8d\r\n", topic->name, desc_display, topic->total_messages);
      count++;
    }
  }
                        ...
                        int message_type, int category)
{
  ...
  msg = pubsub_create_message(topic->topic_id, sender_name, content, message_type, category,
