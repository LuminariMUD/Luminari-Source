...
static size_t ai_curl_write_callback(void *contents, size_t size, size_t nmemb, void *userp)
{
  ...
  if (!ptr)
  {
    AI_DEBUG("ERROR: Failed to realloc buffer from %zu to %zu bytes", mem->size,
             mem->size + realsize + 1);
    return 0;
  }
  ...
  AI_DEBUG("Response chunk: %.100s%s", (char *)contents, realsize > 100 ? "..." : "");
...
void init_ai_service(void)
{
  ...
  AI_DEBUG("  Current counts: minute=%d, hour=%d", ai_state.limiter->current_minute_count,
  ...
  AI_DEBUG("  Cache initialized: head=%p, size=%d, max_size=%d", (void *)ai_state.cache_head,
...
void shutdown_ai_service(void)
{
  ...
  for (entry = ai_state.cache_head; entry; entry = next)
  {
    ...
    AI_DEBUG("  Freeing cache entry: key='%s', response_len=%zu",
             entry->key ? entry->key : "(null)", entry->response ? strlen(entry->response) : 0);
    if (entry->key)
      free(entry->key);
    if (entry->response)
      free(entry->response);
...
void load_ai_config(void)
{
  ...
  ai_state.config->cache_expire_seconds =
      get_env_int("AI_CACHE_EXPIRE_SECONDS", DEFAULT_AI_CACHE_EXPIRE_TIME);
  AI_DEBUG("  Cache expire: %d seconds", ai_state.config->cache_expire_seconds);
  ...
  {
    log("AI Service: Timeout_ms %d exceeds maximum, clamping to 300000ms",
...
static char *make_api_request_single(const char *prompt)
{
  ...
  AI_DEBUG("JSON content: %.200s%s", json_request, strlen(json_request) > 200 ? "..." : "");
  ...
  if (res == CURLE_OK)
  {
    ...
    if (http_code == 200)
    {
      ...
      AI_DEBUG("Raw response: %.500s%s", response.data, response.size > 500 ? "..." : "");
...
bool ai_check_rate_limit(void)
{
  ...
  AI_DEBUG("Current counts: minute=%d/%d, hour=%d/%d", ai_state.limiter->current_minute_count,
           ai_state.limiter->requests_per_minute, ai_state.limiter->current_hour_count,
           ai_state.limiter->requests_per_hour);
      ...
      ai_state.limiter->current_hour_count >= ai_state.limiter->requests_per_hour)
  {
    AI_DEBUG("Rate limit exceeded: minute=%s, hour=%s",
             ai_state.limiter->current_minute_count >= ai_state.limiter->requests_per_minute ? "YES"
                                                                                             : "NO",
             ai_state.limiter->current_hour_count >= ai_state.limiter->requests_per_hour ? "YES"
                                                                                         : "NO");
    return FALSE;
  }
...
static char *build_json_request(const char *prompt)
{
  ...
  snprintf(json_buffer, sizeof(json_buffer),
           ...
           "\"temperature\": %.2f"
           "}",
           ai_state.config->model, escaped_prompt, ai_state.config->max_tokens,
           ai_state.config->temperature);

  AI_DEBUG("JSON request built, total length: %zu", strlen(json_buffer));
  AI_DEBUG("JSON structure: %.200s%s", json_buffer, strlen(json_buffer) > 200 ? "..." : "");
...
static char *parse_json_response(const char *json_str)
{
  ...
  while (*content_end)
  {
    if (*content_end == '"')
    {
      ...
      AI_DEBUG("  Found quote #%d at position %d with %d preceding backslashes", quotes_found,
  ...
  while (src < content_end)
  {
    ...
    if (*src == '\\' && src + 1 < content_end)
    {
      src++; /* Skip backslash */
      escape_sequences++;
      AI_DEBUG("  Escape sequence #%d at position %d: \\%c", escape_sequences, chars_processed,
               *src);
      if (*src == '"')
        *dst++ = '"';
      else if (*src == '\\')
  ...
  *dst = '\0';
  ...
  AI_DEBUG("Unescaping complete: processed %d chars, found %d escape sequences", chars_processed,
           escape_sequences);
                        ...
                        const char *backend, bool from_cache)
{
  ...
  if (ch && npc && response)
  {
    log("AI [%s%s]: %s -> %s: %s", backend ? backend : "Unknown", from_cache ? "/CACHED" : "",
        GET_NAME(ch) ? GET_NAME(ch) : "Unknown", GET_NAME(npc) ? GET_NAME(npc) : "Unknown",
        response);
  }
}
...
static bool warmup_ollama_model(void)
{
  ...
  if (test_response)
  {
    log("AI Service: Ollama model %s warmed up successfully (response length: %zu)", model,
...
static char *build_ollama_json_request(const char *prompt)
{
  ...
  written = snprintf(json_buffer, sizeof(json_buffer),
                     ...
                     "}"
                     "}",
                     model, system_prefix, escaped_prompt, system_suffix, num_predict, temperature,
                     top_k, top_p);
...
static char *parse_ollama_json_response(const char *json_str)
{
  ...
  AI_DEBUG("Ollama response parsed: '%.100s%s'", result, strlen(result) > 100 ? "..." : "");
...
char *ai_generate_response_async(const char *prompt, int request_type, int retry_count)
{
  ...
  else
  {
    ...
    if (retry_count < max_retries)
    {
      ...
      log("AI Service: Request type %d failed, retry %d/%d pending", request_type, retry_count + 1,
          max_retries);
    }
    else
    {
