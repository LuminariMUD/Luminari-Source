...
#define PERSIST_CHARACTER_EVENT(event_id)                                                          \
  ...
  PERSIST_CHARACTER_EVENT(eSLA_FARSEE);
  PERSIST_CHARACTER_EVENT(eSLA_STONESKIN);
  PERSIST_CHARACTER_EVENT(eSLA_LIGHTNING_BOLT);
  PERSIST_CHARACTER_EVENT(eSLA_FIRE_SHIELD);
  PERSIST_CHARACTER_EVENT(eSLA_FIRE_STORM);
  PERSIST_CHARACTER_EVENT(eSLA_SHADOW_JUMP);
  PERSIST_CHARACTER_EVENT(eSLA_PLANE_SHIFT);
  PERSIST_CHARACTER_EVENT(eSLA_PSIONIC_BLAST);
  PERSIST_CHARACTER_EVENT(eSLA_SCARE);
  PERSIST_CHARACTER_EVENT(eSLA_HASTE);
  PERSIST_CHARACTER_EVENT(eSLA_FIREBALL);
  PERSIST_CHARACTER_EVENT(eSLA_MASS_DISPEL);
  PERSIST_CHARACTER_EVENT(eSLA_FROST_BREATH);
  PERSIST_CHARACTER_EVENT(eSLA_WEB);
  PERSIST_CHARACTER_EVENT(eSUMMON_WARG);
  PERSIST_CHARACTER_EVENT(eSUMMON_HORDE);
  PERSIST_CHARACTER_EVENT(eRACIAL_FLURRY);
...
MUD_EVENT_CALLBACK(event_countdown)
{
  ...
  case EVENT_REGION:
    ...
    /* log("LOG: EVENT_REGION case in MUD_EVENT_CALLBACK(event_countdown): Region VNum %d, RNum %d", *regvnum, regrnum); */
    break;
  default:
    break;
  }
  ...
  case eQUEST_COMPLETE:
    if (ch == NULL)
      break;
    qvnum = parse_int((char *)pMudEvent->sVariables);
