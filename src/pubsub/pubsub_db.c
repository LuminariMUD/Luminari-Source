...
int pubsub_db_save_topic(struct pubsub_topic *topic)
{
  ...
  snprintf(query, sizeof(query),
           ...
           escaped_name, topic->description ? escaped_desc : "NULL", topic->category,
           topic->access_type, topic->min_level, topic->total_messages, topic->subscriber_count,
           topic->max_subscribers, topic->message_ttl, topic->is_persistent ? 1 : 0,
...
void pubsub_cache_player_subscriptions(const char *player_name)
{
  ...
  if (mysql_query(conn, query))
  {
    pubsub_error("Failed to load subscriptions for player %s: %s", player_name, mysql_error(conn));
    return;
  }
  ...
  if (cache->subscription_count > 0)
  {
    ...
    while ((row = mysql_fetch_row(result)))
    {
      ...
      i++;
      pubsub_debug("Cached subscription: player %s -> topic %s", player_name, row[5]);
    }
  }
  ...
  pubsub_debug("Cached %d subscriptions for player %s", cache->subscription_count, player_name);
}
...
int pubsub_get_player_subscription_count(const char *player_name)
{
  ...
  snprintf(query, sizeof(query),
           ...
           "WHERE player_name = '%s' AND status = 0",
           escaped_name);

  if (mysql_query(conn, query))
  {
    pubsub_error("Failed to get subscription count for player %s: %s", player_name,
                 mysql_error(conn));
    return 0;
  }
...
int pubsub_get_max_subscriptions(const char *player_name)
{
  ...
  if (mysql_query(conn, query))
  {
    pubsub_debug("Failed to get max subscriptions for player %s: %s", player_name,
                 mysql_error(conn));
    return PUBSUB_MAX_SUBSCRIPTIONS_DEFAULT;
  }
...
int pubsub_db_create_v3_tables(void)
{
  ...
  snprintf(query, sizeof(query),
           ...
           "reference_count INT DEFAULT 1, "
           "FOREIGN KEY (topic_id) REFERENCES pubsub_topics(topic_id) ON DELETE CASCADE, "
           "FOREIGN KEY (parent_message_id) REFERENCES pubsub_messages_v3(message_id) ON DELETE "
           "SET NULL, "
           "INDEX idx_topic_created (topic_id, created_at), "
           "INDEX idx_type_category (message_type, message_category), "
           "INDEX idx_thread (thread_id, sequence_number), "
...
int pubsub_db_populate_data(void)
{
  ...
  for (i = 0; default_topics[i][0] != NULL; i++)
  {
    ...
    if (mysql_query(conn, query))
    {
      pubsub_error("Failed to insert default topic '%s': %s", default_topics[i][0],
                   mysql_error(conn));
...
int pubsub_db_save_message(struct pubsub_message *msg)
{
  ...
  snprintf(query, sizeof(query),
           ...
           msg->topic_id, escaped_sender, msg->sender_id, msg->message_type, msg->message_category,
           msg->priority, msg->content ? escaped_content : "",
           msg->content_type ? msg->content_type : "text/plain",
           msg->content_encoding ? msg->content_encoding : "utf-8", msg->content_version,
           msg->spatial_data ? msg->spatial_data : "NULL", parent_id_str, thread_id_str,
           msg->sequence_number);
...
int pubsub_db_save_metadata(int message_id, struct pubsub_message_metadata *metadata)
{
  ...
  snprintf(query, sizeof(query),
           ...
           "processing_notes) VALUES (%d, ",
           message_id);

  /* sender_real_name */
  if (metadata->sender_real_name)
  {
    mysql_real_escape_string(conn, temp_str, metadata->sender_real_name,
                             strlen(metadata->sender_real_name));
    strcat(query, "'");
  ...
  {
    mysql_real_escape_string(conn, temp_str, metadata->sender_title,
                             strlen(metadata->sender_title));
    strcat(query, "'");
    strcat(query, temp_str);
    strcat(query, "'");
  }
  ...
  if (metadata->sender_class)
  {
    ...
    mysql_real_escape_string(conn, escaped_class, metadata->sender_class,
                             strlen(metadata->sender_class));
    strcat(query, "'");
  ...
  if (metadata->sender_race)
  {
    ...
    mysql_real_escape_string(conn, escaped_race, metadata->sender_race,
                             strlen(metadata->sender_race));
    strcat(query, "'");
  ...
  if (metadata->origin_area_name)
  {
    ...
    mysql_real_escape_string(conn, escaped_area, metadata->origin_area_name,
                             strlen(metadata->origin_area_name));
    strcat(query, "'");
  ...
  snprintf(temp_str, sizeof(temp_str), ", %d, %d, %d, ", metadata->origin_x, metadata->origin_y,
           metadata->origin_z);
  strcat(query, temp_str);
  ...
  if (metadata->context_type)
  {
    ...
    mysql_real_escape_string(conn, escaped_context, metadata->context_type,
                             strlen(metadata->context_type));
    strcat(query, "'");
  ...
  if (metadata->trigger_event)
  {
    ...
    mysql_real_escape_string(conn, escaped_trigger, metadata->trigger_event,
                             strlen(metadata->trigger_event));
    strcat(query, "'");
  ...
  if (metadata->related_object)
  {
    ...
    mysql_real_escape_string(conn, escaped_object, metadata->related_object,
                             strlen(metadata->related_object));
    strcat(query, "'");
  ...
  if (metadata->handler_chain)
  {
    ...
    mysql_real_escape_string(conn, escaped_chain, metadata->handler_chain,
                             strlen(metadata->handler_chain));
    strcat(query, "'");
  ...
  if (metadata->processing_notes)
  {
    ...
    mysql_real_escape_string(conn, escaped_notes, metadata->processing_notes,
                             strlen(metadata->processing_notes));
    strcat(query, "'");
