...
static struct pubsub_queue_node *
create_queue_node(struct pubsub_message *msg, struct char_data *target, const char *handler_name);
                         ...
                         const char *handler_name)
{
  ...
  pubsub_debug("Queued message ID %d with priority %d for player %s", msg->message_id, priority,
               GET_NAME(target));
...
int pubsub_queue_process_batch(int max_messages)
{
  ...
  for (priority = PUBSUB_PRIORITY_CRITICAL;
       priority >= PUBSUB_PRIORITY_LOW && processed < max_messages; priority--)
  {
    struct pubsub_queue_node *node;
