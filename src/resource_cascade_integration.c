...
ACMD(do_enhanced_survey)
{
  ...
  else if (!str_cmp(arg1, "cascade"))
  {
    if (!*arg2)
    {
      ...
      send_to_char(ch, "Available resources: vegetation, minerals, water, herbs, game, wood, "
                       "stone, crystal, clay, salt\r\n");
      return;
    }
  ...
  else
  {
    ...
    {
      send_to_char(ch, "Invalid survey option. Use: resources, ecosystem, cascade, relationships, "
                       "impact, terrain, debug, map, detail\r\n");
    }
  }
}
...
ACMD(do_enhanced_harvest)
{
  ...
  if (!*arg)
  {
    ...
    send_to_char(
        ch, "Note: Use 'mine' for minerals/crystals/stone, 'gather' for specific resources\r\n");
    return;
  }
  ...
  if (resource_type != RESOURCE_VEGETATION && resource_type != RESOURCE_HERBS &&
  ...
  {
    send_to_char(ch, "\trThis ecosystem is in critical condition. Harvesting here may cause "
...
int attempt_wilderness_harvest_with_cascades(struct char_data *ch, int resource_type)
{
  ...
  if (result > 0)
  {
    ...
    {
      send_to_char(ch,
                   "\tyYour harvesting has contributed to ecosystem stress in this area.\tn\r\n");
    }
  }
...
void show_resource_relationships(struct char_data *ch, room_rnum room)
{
  ...
  while ((row = mysql_fetch_row(result)))
  {
    ...
    if (source_resource >= 0 && source_resource < NUM_RESOURCE_TYPES && target_resource >= 0 &&
        target_resource < NUM_RESOURCE_TYPES)
    {
...
void show_conservation_impact(struct char_data *ch, room_rnum room)
{
  ...
  const char *score_desc = conservation_score >= 0.8   ? "Excellent Steward"
                           ...
                           : conservation_score >= 0.2 ? "Heavy Harvester"
                                                       : "Ecosystem Destroyer";

  send_to_char(ch, "Your Conservation Score: %s%.2f\tn (%s%s\tn)\r\n", score_color,
               conservation_score, score_color, score_desc);
...
ACMD(do_resourceadmin_cascade)
{
  ...
  if (!*arg1)
  {
    ...
    send_to_char(ch, "========================================\r\n");
    send_to_char(ch,
                 "resourceadmin cascade ecosystem [room]   - Show detailed ecosystem debug\r\n");
