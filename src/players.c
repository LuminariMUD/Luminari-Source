  ...
  if (mode != 1)
  {
    ...
    else
    {
      ...
      if ((pMudEvent = char_has_mud_event(ch, eINVISIBLE_ROGUE)))
        BUFFER_WRITE("%d %ld %d\n", pMudEvent->iId, mud_event_remaining(pMudEvent),
      ...
      if ((pMudEvent = char_has_mud_event(ch, eINNER_FIRE)))
        BUFFER_WRITE("%d %ld %d\n", pMudEvent->iId, mud_event_remaining(pMudEvent),
                     get_daily_uses(ch, FEAT_INNER_FIRE) -
                         daily_uses_remaining(ch, FEAT_INNER_FIRE));
      ...
      if ((pMudEvent = char_has_mud_event(ch, eEMPTYBODY)))
        BUFFER_WRITE("%d %ld %d\n", pMudEvent->iId, mud_event_remaining(pMudEvent),
                     get_daily_uses(ch, FEAT_EMPTY_BODY) -
                         daily_uses_remaining(ch, FEAT_EMPTY_BODY));
      ...
      if ((pMudEvent = char_has_mud_event(ch, eC_MOUNT)))
        BUFFER_WRITE("%d %ld %d\n", pMudEvent->iId, mud_event_remaining(pMudEvent),
                     get_daily_uses(ch, FEAT_CALL_MOUNT) -
                         daily_uses_remaining(ch, FEAT_CALL_MOUNT));
      ...
      if ((pMudEvent = char_has_mud_event(ch, eSMITE_EVIL)))
        BUFFER_WRITE("%d %ld %d\n", pMudEvent->iId, mud_event_remaining(pMudEvent),
                     get_daily_uses(ch, FEAT_SMITE_EVIL) -
                         daily_uses_remaining(ch, FEAT_SMITE_EVIL));
      if ((pMudEvent = char_has_mud_event(ch, eSMITE_GOOD)))
        BUFFER_WRITE("%d %ld %d\n", pMudEvent->iId, mud_event_remaining(pMudEvent),
                     get_daily_uses(ch, FEAT_SMITE_GOOD) -
                         daily_uses_remaining(ch, FEAT_SMITE_GOOD));
