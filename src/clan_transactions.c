...
bool add_clan_trans_operation(struct clan_transaction *trans, int type, clan_rnum clan,
                              long player_id, int old_val, int new_val)
{
...
bool add_clan_trans_treasury(struct clan_transaction *trans, clan_rnum clan, long old_amount,
                             long new_amount)
{
  struct trans_operation *op;
...
static void log_transaction(struct clan_transaction *trans, const char *action)
{
  ...
  mudlog(NRM, LVL_STAFF, TRUE, "CLAN TRANS: %s transaction %d (%s) by %s", action, trans->id,
         trans->description, trans->initiator ? GET_NAME(trans->initiator) : "SYSTEM");
}
