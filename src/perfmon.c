                                  ...
                                  int is_total)
{
  ...
  if (is_total)
  {
    return snprintf(buf, n, "%-20s|%12lu|%12ld|%11.2f%%|%19.2f%%\n\r", sect->id, enter_count,
                    usec_total, percent, (100.0 * usec_max) / USEC_PER_PULSE);
  }
  else
  {
    return snprintf(buf, n, "%-20s|%12lu|%12lu|%12ld|%11.2f%%|%19.2f%%\n\r", sect->id, enter_count,
                    sect->pulse_exit_count, usec_total, percent,
...
size_t PERF_prof_repr_pulse(char *out_buf, size_t n)
{
  ...
  written = snprintf(
      ...
      "Section name        |Enter Count |Exit Count  |usec total  |pulse %%    |max pulse %% (1 "
      "entry)\n\r"
      "--------------------------------------------------------------------------------\n\r");
