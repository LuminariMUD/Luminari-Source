...
struct region_proximity_list* get_nearby_regions(zone_rnum zone, int x, int y, int r) {
  ...
  sprintf(buf, "select * from (select " 
               ...
               x, y, ROUND((r*.5 + x)),  ROUND((r*.87 + y)), ROUND((r*.87 + x)), ROUND((r*.5 + y)), x, y,    /* ne */
               x, y, ROUND((r*.87 + x)), ROUND((r*.5 + y)), ROUND((r*.87 + x)), ROUND((r*-.5 + y)), x, y,    /* e */           
               x, y, ROUND((r*.87 + x)), ROUND((r*-.5 + y)), ROUND((r*.5 + x)), ROUND((r*-.87 + y)), x, y,   /* se */         
