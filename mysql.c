...
void load_paths() {
 ...
 if ( (numrows = mysql_num_rows(result)) < 1) 
  ...
  while ((row = mysql_fetch_row(result))) { 
    ...
    path_table[i].glyphs[GLYPH_TYPE_PATH_NS]  = parse_at(strdup(row[7]));
    path_table[i].glyphs[GLYPH_TYPE_PATH_EW]  = parse_at(strdup(row[8]));
    path_table[i].glyphs[GLYPH_TYPE_PATH_INT] = parse_at(strdup(row[9]));
