  ...
  if (!IS_NPC(k)) {
    ...
    for (i = 0; i < MAX_CLASSES; i++) {
      if (CLASS_LEVEL(k, i)) {
        if (counter)
          send_to_char(ch, " / ");
        send_to_char(ch, "%d %s", CLASS_LEVEL(k, i), class_abbrevs[i]);
        counter++;
      }
    }
    send_to_char(ch, "\r\n");
  ...
  for (num = 0; num <= top_of_objt; num++) {
    ...
    if (obj_proto[num].obj_flags.type_flag == itemtype) {
      ...
      if (r_num > 0) {
        ...
        switch (itemtype) {
          case ITEM_LIGHT:
            it = (obj_proto[num].obj_flags.value[2]);
          ...
          case ITEM_WAND:
          case ITEM_STAFF:
            it = (obj_proto[num].obj_flags.value[1]);
            it2 = obj_index[num].vnum;
          ...
          case ITEM_WEAPON:
            it = ((obj_proto[num].obj_flags.value[2] + 1) / 2)*(obj_proto[num].obj_flags.value[1]);
            it2 = obj_index[num].vnum;
            it3 = (obj_proto[num].obj_flags.value[3]);
          ...
          case ITEM_ARMOR:
            it = (obj_proto[num].obj_flags.value[0]);
            it2 = obj_index[num].vnum;
        ...
        extract_obj(obj); /* Finished with the object - remove it from the MUD */
      }
    }
  }
...
void perform_affect_list(struct char_data * ch, char *arg) {
  int num, i, j, apply, bob, bob2, it, it2, found = 0, r_num;
  int lst[50][3];
  int bob3, it3, iSpell;
  struct obj_data *obj;
  ...
  if (!(apply > 0 && apply < NUM_APPLIES)) {
  ...
  else if (apply == APPLY_CLASS) /* Weapon Damage - afflist 7 (Jamdog - 19th April 2006) */ {
    for (num = 0; num <= top_of_objt; num++) {

      if (obj_proto[num].obj_flags.type_flag == ITEM_WEAPON) {
        it = ((obj_proto[num].obj_flags.value[2] + 1) / 2)*(obj_proto[num].obj_flags.value[1]);
        it2 = obj_index[num].vnum;
        it3 = MAX(0, (obj_proto[num].obj_flags.value[0] - 250)); /* Get the spell number */

        r_num = real_object(it2);
        obj = read_object(r_num, REAL);
        /* sprintf(buf, "Weapon found: %d (%s)", it2, obj->short_description ); */

        for (j = 0; j < 50; j++) /* Put in list of 1 to 30 */ {
          if (it > lst[j][0]) {
            bob = lst[j][0];
            bob2 = lst[j][1];
            bob3 = lst[j][2];
    ...
    send_to_char(ch, "Average per hit for Weapons\r\n");

    for (i = 0; i < 50; i++) {
      r_num = real_object(lst[i][1]);
      if (r_num > 0) {
        obj = read_object(r_num, REAL);
        if (lst[i][2] == 0) {
          send_to_char(ch, "\tn%d. \tc[\ty%8d\tc] \tY%3d \tn%s\tn\r\n", ++found, lst[i][1], lst[i][0], obj->short_description);
        }
        else {
          iSpell = lst[i][2];
          ...
          if (iSpell > 10) {
            ...
            send_to_char(ch, "\tn%d. \tc[\ty%8d\tc]\tY %3d \tn%s\tn (casts \tG%s\tn)\r\n", ++found, lst[i][1], lst[i][0], obj->short_description, skill_name(iSpell));
          }
        }
        extract_obj(obj); /* Finished with the object - remove it from the MUD */
      }
  ...
  else if (apply == APPLY_LEVEL) /* AC-Apply for Armor only (afflist 8) */ {
    for (num = 0; num <= top_of_objt; num++) {
      if (obj_proto[num].obj_flags.type_flag == ITEM_ARMOR) {
        it = obj_proto[num].obj_flags.value[0];
        it2 = obj_index[num].vnum;
        it3 = obj_proto[num].obj_flags.value[1];
        for (j = 0; j < 50; j++) {
    ...
    send_to_char(ch, "AC Apply for Armor\r\n");
    ...
    for (i = 0; i < 50; i++) {
      ...
      if (r_num > 0) {
        ...
        if (lst[i][2] == 0) {
          ...
          /* Check for a valid spell of skill - disallow !UNUSED! */
          if ((iSpell > 215) || (str_cmp(skill_name(iSpell), "!UNUSED!") == 0)) {
            send_to_char(ch, "\tn%d. \tc[\ty%8d\tc]\tY %3d\tn %s\tn\r\n", ++found, lst[i][1], lst[i][0], obj->short_description);
          }
          else {
            send_to_char(ch, "\tn%d. \tc[\ty%8d\tc]\tY %3d\tn %s\tn (casts \tG%s\tn)\r\n", ++found, lst[i][1], lst[i][0], obj->short_description, skill_name(iSpell));
    ...
    for (i = 0; i < MAX_OBJ_AFFECT; i++) {
      if (obj_proto[num].affected[i].modifier) {
        if (obj_proto[num].affected[i].location == apply) {
          it = obj_proto[num].affected[i].modifier;
          it2 = obj_index[num].vnum;
          for (j = 0; j < 50; j++) {
            if (it > lst[j][0]) {
              bob = lst[j][0];
  ...
  for (i = 0; i < 50; i++) {
    ...
    if (r_num > 0) {
      obj = read_object(r_num, REAL);
      send_to_char(ch, "\tn%d. \tc[\ty%8d\tc]\tY %3d\tn %s\r\n", ++found, lst[i][1], lst[i][0], obj->short_description);
      extract_obj(obj); /* Finished with the object - remove it from the MUD */
    }
  }
}
...
ACMD(do_typelist) {
  ...
  one_argument(argument, arg);
  if (!*arg) {
    send_to_char(ch, "What item type do you want to list?\r\n");
    for (i = 0; i < NUM_ITEM_TYPES; i++) {
      send_to_char(ch, "%s%2d%s-%s%-14s%s", CBGRN(ch, C_NRM), i, CCNRM(ch, C_NRM), CCYEL(ch, C_NRM), item_types[i], CCNRM(ch, C_NRM));
      if (!((i + 1) % 4)) send_to_char(ch, "\r\n");
    }
    send_to_char(ch, "\r\n");
    send_to_char(ch, "Usage: %stypelist <num>%s\r\n", CBGRN(ch, C_NRM), CBGRN(ch, C_NRM));
    send_to_char(ch, "%sNOTE:%s Displays ALL objects of that type - this could be\r\n", CBRED(ch, C_NRM), CBYEL(ch, C_NRM));
    send_to_char(ch, "      thousands of items, so please use for rarer types\r\n");
    send_to_char(ch, "      if possible%s\r\n", CCNRM(ch, C_NRM));

    return;
  }
  perform_type_list(ch, arg);
}
...
ACMD(do_afflist) {
  ...
  one_argument(argument, arg);
  if (!*arg) {
    send_to_char(ch, "Which item affect do you want to list?\r\n");
    for (i = 0; i < NUM_APPLIES; i++) {
      if (i == APPLY_CLASS) /* Special Case 1 - Weapon Dam */
        send_to_char(ch, "%s%2d%s-%s%-14s%s", CBGRN(ch, C_NRM), i, CCNRM(ch, C_NRM), CCYEL(ch, C_NRM), "Weapon Dam", CCNRM(ch, C_NRM));
      else if (i == APPLY_LEVEL) /* Special Case 2 - Armor AC Apply */
        send_to_char(ch, "%s%2d%s-%s%-14s%s", CBGRN(ch, C_NRM), i, CCNRM(ch, C_NRM), CCYEL(ch, C_NRM), "AC Apply", CCNRM(ch, C_NRM));
      else
        send_to_char(ch, "%s%2d%s-%s%-14s%s", CBGRN(ch, C_NRM), i, CCNRM(ch, C_NRM), CCYEL(ch, C_NRM), apply_types[i], CCNRM(ch, C_NRM));
      if (!((i + 1) % 4)) send_to_char(ch, "\r\n");
    }
    send_to_char(ch, "\r\n");
    send_to_char(ch, "Usage: %safflist <num>%s\r\n", CBGRN(ch, C_NRM), CBGRN(ch, C_NRM));
    send_to_char(ch, "Displays the top 50 items with the selected affect\r\n\r\n");
    send_to_char(ch, "%sNOTE:%s Displays only the top 50 objects with the specified affect.\r\n", CBRED(ch, C_NRM), CBYEL(ch, C_NRM));
    send_to_char(ch, "      If there are a lot of overstatted eq, then remove some of the\r\n");
    send_to_char(ch, "      top ones to see more,%s\r\n", CCNRM(ch, C_NRM));

    return;
  }
  perform_affect_list(ch, arg);
}
